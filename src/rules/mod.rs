...
fn assess_treatment_failure(
    ...
    let threshold_level =
        initial_level * store.globals.treatment_failure_threshold;
            ...
            if !individual.restart_window_assessed[bacteria_idx] {
                ...
                if let Some(cessation_level) = individual.bacteria_level_at_drug_cessation[bacteria_idx] {
                    ...
                    let threshold_multiplier =
                        store.globals.restart_bacteria_level_threshold;
                    ...
                    if (bacteria_worsened || bacteria_still_high) && individual.level[bacteria_idx] > 0.1 {
                        ...
                        let return_probability =
                            store.globals.restart_window_probability;
...
fn start_restart_treatment(
    ...
    if let Some(prev_drug_idx) = stopped_drug_idx {
        ...
        if avail >= 0.01 && intro_ok && !individual.cur_use_drug[prev_drug_idx] {
            ...
            let bacteria_idx_for_cache = bacteria_indices.get(bacteria_name).unwrap_or(&0);
            let potency = store.drug_bacteria.potency(*bacteria_idx_for_cache, prev_drug_idx);
    ...
    for (drug_idx, &drug_name) in DRUG_SHORT_NAMES.iter().enumerate() {
        ...
        // Base potency score
        let bacteria_idx_for_cache = bacteria_indices.get(bacteria_name).unwrap_or(&0);
        let potency = store.drug_bacteria.potency(*bacteria_idx_for_cache, drug_idx);
        if potency >= minimal_potency_threshold {
            score += potency;
        }
...
pub fn apply_rules(
    ...
    let transfer_prob = store.globals.microbiome_resistance_transfer_probability_per_day;
    ...
    let double_dose_probability = store.globals.double_dose_probability_if_identified_infection;
    ...
    for &bacteria in BACTERIA_LIST.iter() {
        ...
        if current_level > 0.0 {
            ...
            if !individual.sepsis[b_idx] {
                ...
                let sepsis_baseline_log_odds = store
                    .bacteria
                    .sepsis_baseline_log_odds(b_idx);
                let log_odds_infection_level = store
                    .bacteria
                    .sepsis_log_odds_infection_level(b_idx);
                let log_odds_infection_duration = store
                    .bacteria
                    .sepsis_log_odds_infection_duration(b_idx);
                ...
                let region_log_odds = match individual.region_living {
                    ...
                    Region::NorthAmerica | Region::Europe | Region::Oceania => store.globals.log_odds_sepsis_region_a, // Higher resource regions
        ...
        for &vaccine in &BACTERIAL_VACCINES {
            if let Some(vaccine_idx) = crate::config::VaccinationParameters::vaccine_index(vaccine)
            {
                ...
                if targets_bacteria && !individual.vaccination_status[b_idx] {
                    let daily_prob = store
                        .vaccination
                        .daily_probability(vaccine_idx, age_idx);
    ...
    for &syndrome_id in individual.infectious_syndrome.iter() {
        if syndrome_id > 0 {
            let multiplier = store
                .syndrome
                .initiation_multiplier(syndrome_id as usize);
            syndrome_administration_multiplier =
                syndrome_administration_multiplier.max(multiplier);
        }
    }
    ...
    for drug_idx in 0..DRUG_SHORT_NAMES.len() {
        if individual.cur_use_drug[drug_idx] {
            ...
            if !relevant_infection_active_for_this_drug {
                ...
                let random_cessation_if_no_infection =
                    store
                        .globals
...
let available_drugs: Vec<usize> = DRUG_SHORT_NAMES.iter().enumerate()
    ...
    if num_drugs_currently_used + drugs_initiated_this_time_step < 3 && available_drugs_count > 0 {
        ...
        if individual.immunodeficiency_type.is_some() {
            start_any_antibiotic_prob *= store
                .globals
        ...
        if rng.gen_bool(start_any_antibiotic_prob) {
            ...
            for &drug_idx in &available_drugs {
                ...
                for b_idx in 0..BACTERIA_LIST.len() {
                    if individual.level[b_idx] > 0.001 {
                        ...
                        let specific_multiplier = store
                            .drug_bacteria
                            .initiation_multiplier(b_idx, drug_idx);
                ...
                if !has_any_identified_infection {
                    ...
                    let very_high_threshold = store.globals.regional_resistance_threshold_very_high;
                    let high_threshold = store.globals.regional_resistance_threshold_high;
                    let moderate_threshold = store.globals.regional_resistance_threshold_moderate;
                ...
                if has_any_identified_infection {
                    let targeted_narrow_bonus = store.globals.targeted_therapy_narrow_spectrum_bonus;
                    let targeted_broad_penalty = store.globals.targeted_therapy_broad_spectrum_penalty;
                    let ineffective_drug_penalty = store.globals.targeted_therapy_ineffective_drug_penalty;
                    let effective_potency_threshold = store.globals.effective_potency_threshold_for_targeted_therapy;
                    ...
                    let empiric_ineffective_penalty = store.globals.empiric_therapy_ineffective_penalty;
                    let effective_potency_threshold = store.globals.effective_potency_threshold_for_empirical_therapy;
            ...
            if !drug_scores.is_empty() {
                ...
                if total_weight > 0.0 && total_weight.is_finite() {
                    ...
                    if has_any_identified_infection && rng.gen_bool(double_dose_probability) {
                        let double_dose_multiplier = store.drug.double_dose_multiplier(chosen_drug_idx);
    ...
    let half_life_years = store.globals.mortality_improvement_half_life_years;
        ...
        if age_years > 80.0 {
            let log_odds_age_squared = store
                .globals
        ...
        total_log_odds += store
            .region
            .mortality_log_odds(individual.region_living);
        ...
        if has_sepsis {
            ...
            let mut sepsis_death_risk = store
                .globals
        ...
        let toxicity_death_risk_per_day = store
            .globals
        ...
        for &bacteria in BACTERIA_LIST.iter() {
            ...
            if individual.sepsis[b_idx] {
                ...
                if sepsis_duration >= minimum_duration {
                    ...
                    let bacteria_level_coefficient = store
                        .globals
                        .sepsis_log_odds_bacteria_level;
                    ...
                    if individual.hospital_status.is_hospitalized() {
                        let hospital_coefficient = store
                            .globals
                            .sepsis_log_odds_in_hospital;
                    ...
                    if individual.immunodeficiency_type.is_some() {
                        let immunosuppressed_coefficient = store
                            .globals
                            .sepsis_log_odds_immunosuppressed;
                    ...
                    total_log_odds += store
                        .region
                        .sepsis_log_odds(individual.region_living);
    ...
    for (b_idx, &bacteria) in BACTERIA_LIST.iter().enumerate() {
        ...
        if !is_infected {
            ...
            if rng.gen_bool(acquisition_probability.clamp(0.0, 1.0)) {
                ...
                let max_resistance_level = store.globals.max_resistance_level;
                let mut env_majority_r_level = get_global_param("environmental_majority_r_level_for_new_acquisition").unwrap_or(0.0);
                ...
                for (drug_index, _use_drug) in individual.cur_use_drug.iter().enumerate() { 
                    ...
                    if resistance_data.any_r < 0.0001 { // Check if any_r is effectively zero
                        ...
                        if drug_current_level > 0.0 && current_bacteria_level > 0.0001 { 
                            ...
                            let bacteria_level_effect_multiplier = store
                                .globals
                                .resistance_emergence_bacteria_level_multiplier;
                            let any_r_emergence_level_on_first_emergence = store
                                .globals
                            ...
                            let drug_initial_level_for_normalization = store.drug.initial_level(drug_index);
                            ...
                            if active_drug_count >= multi_drug_penalty_threshold {
                                ...
                                let drugs_affected_by_this_resistance = if let Some(cross_resistance_groups) = 
                                ...
                                if drugs_affected_by_this_resistance < active_drug_count {
                                    ...
                                    if drugs_affected_by_this_resistance == 1 {
                                        ...
                                        multi_drug_penalty_factor =
                                            store.globals.resistance_development_inhibition_partial_cross;
                                    }
                                }
                    ...
                    if drug_current_level > 0.0 && current_bacteria_level > 0.001 {
                        ...
                        let base_potency = store
                            .drug_bacteria
                            .potency(bacteria_full_idx, drug_index);
            ...
            if !on_any_drug {
                ...
                for (mechanism_idx, _) in ResistanceMechanism::all().iter().enumerate() {
                    if individual.resistance_mechanisms[b_idx][mechanism_idx] {
                        let mechanism_reversion_rate = store
                            .resistance_mechanism
                            .reversion_rate(mechanism_idx);
...
fn calculate_testing_probability(
    ...
    let region_multiplier = store
        .region
        .testing_multiplier(individual.region_cur_in);
