...
pub struct InfectionJourneySnapshot {
    ...
    pub microbiome_resistance_major: Vec<(String, f64)>, // (bacteria_name, majority_level)
...
impl JourneyLogger {
    ...
    pub fn check_individual(&mut self, individual: &Individual, time_step: usize) {
        ...
        let has_active_infection = individual.level.iter().any(|&level| level > INFECTION_EPS);
    ...
    fn should_terminate_journey(&mut self, individual: &Individual, _time_step: usize) -> bool {
        if let Some(journey) = self.active_journeys.get_mut(&individual.id) {
            ...
            if primary_bacteria_level <= INFECTION_EPS {
    ...
    fn create_snapshot(
        ...
        let previous_primary_level = previous_snapshot
            ...
            .unwrap_or(0.0);
        let current_primary_level = individual.level[primary_bacteria_idx];
        let bacteria_cleared_this_step =
            previous_primary_level > INFECTION_EPS && current_primary_level <= INFECTION_EPS;
        ...
        let mut resistance_any_r: Vec<(String, f64)> = DRUG_SHORT_NAMES
            .iter()
            .enumerate()
            .filter(|(idx, _)| {
        ...
        if bacteria_cleared_this_step && !resistance_any_r.iter().any(|(_, value)| *value > 0.0) {
        ...
        let mut resistance_activity_r: Vec<(String, f64)> = DRUG_SHORT_NAMES
            .iter()
            .enumerate()
            .filter(|(idx, _)| {
                // Log activity_r when drug has measurable level, even if no longer actively prescribed.
                // This captures residual exposure that might still influence clearance risk.
                individual.cur_use_drug[*idx] || individual.cur_level_drug[*idx] > INFECTION_EPS
            })
            .map(|(idx, &drug_name)| {
                (
                    drug_name.to_string(),
        ...
        {
            if let Some(snapshot) = previous_snapshot {
                if !snapshot.resistance_activity_r.is_empty() {
                    resistance_activity_r = snapshot.resistance_activity_r.clone();
                }
            }
        }
        ...
        let mut microbiome_resistance_minor: Vec<(String, f64)> = Vec::new();
        let mut microbiome_resistance_major: Vec<(String, f64)> = Vec::new();
        ...
        for (idx, &present) in individual.presence_microbiome.iter().enumerate() {
            ...
            if let Some(resistances) = individual.resistances.get(idx) {
                let mut minority_max = 0.0;
                ...
                for resistance in resistances {
                    ...
                    } else if value > 0.0 && value > minority_max {
                        minority_max = value;
                    }
                }
                ...
                if minority_max > 0.0 {
                    microbiome_resistance_minor
                        .push((BACTERIA_LIST[idx].to_string(), minority_max));
                }

                if majority_max > 0.0 {
                    microbiome_resistance_major
                        .push((BACTERIA_LIST[idx].to_string(), majority_max));
                }
            }
        }
            ...
            >= 0
        {
            ...
            let scores: Vec<(String, f64)> = DRUG_SHORT_NAMES
                .iter()
                .enumerate()
                ...
                .map(|(idx, &drug_name)| {
                    (
                        drug_name.to_string(),
                        individual.drug_score_on_selection_day[idx],
                    )
                })
                .collect();
        ...
        InfectionJourneySnapshot {
            ...
            perceived_penicillin_allergy: individual.perceived_penicillin_allergy,
            ...
            toxicity_level: individual.current_toxicity_hazard,
            ...
            infection_has_caused_symptoms: individual.infection_has_caused_symptoms
                [primary_bacteria_idx],
    ...
    fn detect_drug_supported_clearance(
        ...
        if let Some(prev_snapshot) = previous {
            if prev_snapshot.primary_bacteria_level > INFECTION_EPS
                && current.primary_bacteria_level <= INFECTION_EPS
    ...
    fn snapshot_to_csv(snapshot: &InfectionJourneySnapshot) -> String {
        ...
        let selected_drug_str = snapshot
            .selected_drug
            .as_ref()
            .unwrap_or(&String::new())
            .clone();

        let resolution_str = snapshot
            .resolution_type
            .as_ref()
            .unwrap_or(&String::new())
            .clone();
        ...
        columns.push(snapshot.day_of_journey.to_string());
        columns.push(snapshot.age_at_onset.to_string());
        columns.push(snapshot.sex.clone());
        columns.push(snapshot.region_living.clone());
        columns.push(snapshot.region_current.clone());
        columns.push(snapshot.immunodeficiency.clone());
        columns.push(snapshot.primary_bacteria.clone());
        columns.push(format!("{:.6}", snapshot.primary_bacteria_level));
        ...
        columns.push(format!("{:.6}", snapshot.background_mortality_risk));
    ...
    pub fn get_stats(&self) -> (usize, u32, u32) {
        (
            self.active_journeys.len(),
            self.total_journeys_started,
            self.total_snapshots_logged,
        )
    }
