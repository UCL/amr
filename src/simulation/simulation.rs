...
use crate::rules::{apply_rules, DrugAvailabilityCache, RuleEvents};
...
/// Controls the simulation horizon, alternate-policy continuations, and summary-output density.
///
/// * `None`: retain every baseline row, run configured alternate policies from 2027, and
///   collect the additional diagnostics gated to this mode.
/// * `Partial`: retain every baseline row while skipping alternate-policy continuations.
/// * `Partial25Counterfactual`: retain every baseline row and run the no-resistance
///   continuation from 2022; alternate-policy output begins at the branch point.
/// * `FullMinimal`: retain only 2022-2025 rows and use the lean calibration field profile.
/// * `Full`: retain only 2022-2025 rows and use the standard calibration field profile.
/// * `Full25Counterfactual`: use the standard calibration field profile for the 2022-2025
///   baseline and no-resistance continuation.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CalibrationMode {
    None,
    /// Skip alternate-policy continuations while retaining the full baseline time series.
    #[allow(dead_code)]
    Partial,
...
/// Controls which optional groups are retained in each [`TimeStepSummary`] row. Collection is
/// also skipped for disabled groups where the hot path supports it. Disabled vector groups are
/// empty; scalar and bacterium-by-drug fields not governed by a flag remain present.
///
...
pub struct SummaryContentFlags {
    /// Core per-bacterium infection stock, death association, sepsis, treatment activity,
    /// acquisition, outcome, and carrier-stratum arrays.
    pub per_bacteria: bool,
    /// Additional per-bacterium treatment-prevention, fifth-day course, and resistance-
    ...
    /// Regional population, hospital, age, drug-use, death, and hospital-acquisition breakdowns.
    pub regional: bool,
    /// Syndrome-level active-infection, acquisition, sepsis-stock, and death-association counts.
    /// Some fields count people once; bacterium-specific and sepsis-by-region fields may record
    /// more than one syndrome entry for the same person.
    pub syndrome: bool,
    /// Infection resolution pathway counts by bacterium.
    pub resolution: bool,
    /// Diagnostic coverage stocks by bacterium, plus aggregate entry-cohort cascade stages split
    /// only by community/hospital entry setting.
    pub testing: bool,
    /// Drug-use tracking at the configured post-infection evaluation day in unabridged runs
    /// (legacy field names still contain `day_7`).
    pub day7: bool,
}
...
impl SummaryContentFlags {
    ...
    ///
    /// Retains the unconditional scalar/drug/bacterium-by-drug fields and enables the core
    /// per-bacterium group. Detail, split-burden, microbiome, regional, syndrome, resolution,
    /// testing, and evaluation-day groups remain disabled.
    ...
    /// Standard calibration-summary profile for `CalibrationMode::Full`.
    ///
    /// Enables core per-bacterium, split-burden, serious-R care-setting, microbiome,
    /// regional, syndrome, and testing groups. Per-bacterium detail, microbiome detail,
    /// resolution-pathway, and evaluation-day groups remain disabled.
...
impl CalibrationMode {
    ...
    /// Policy IDs configured by default for this run mode.
    ///
    /// Policy 0 denotes the separately executed baseline and is discarded when the actual
    /// alternate-policy continuation list is built.
...
pub(crate) struct PolicyAdjustments {
    ...
    /// When true, use hard-coded North America default adjustments for regional testing
    /// (`1.1`), cessation (`0.85`), and initiation (`0.0` log-odds).
...
impl PolicyAdjustments {
    ...
    fn alternate_example(globals: &config::GlobalScalars) -> Self {
        ...
        Self {
            ...
            minimal_potency_threshold_for_drug_selection: None,
            bacterial_testing_rate_multiplier: Some(1.5), // 1.5× bacterial-ID testing rate
            resistance_testing_rate_multiplier: Some(1.5), // 1.5× AST testing rate
            counterfactual_resistance_multiplier: None,
            clear_all_resistance_on_branch_start: false,
    ...
    fn perfect_diagnostics(globals: &config::GlobalScalars) -> Self {
        ...
        Self {
            ...
            drug_selection_temperature: Some(adjusted_temperature),
            minimal_potency_threshold_for_drug_selection: None,
            bacterial_testing_rate_multiplier: Some(20.0), // Very high bacterial-ID testing rate
            resistance_testing_rate_multiplier: Some(20.0), // Matched very high AST testing rate
            counterfactual_resistance_multiplier: None,
            clear_all_resistance_on_branch_start: false,
            reserve_drug_penalty_multiplier: None,
            drug_initiation_rate_multiplier: None,
    ...
    /// Policy 4: hard-coded equal-access regional adjustments
    ///
    /// Uses North America default constants for regional testing, cessation, and initiation;
    /// it does not read the configured North America entries. Drug availability, introduction
    /// dates, and other regional model inputs are unchanged.
...
fn targeted_course_started_for_bacterium(rule_events: &RuleEvents, bacteria_idx: usize) -> bool {
    rule_events
        .targeted_course_start
        .is_some_and(|event| event.includes_bacterium(bacteria_idx))
}

fn diagnostic_targeted_treatment_recordable(
    individual: &Individual,
    ...
    individual.diagnostic_cascade_bacterial_identification_recorded[bacteria_idx]
        ...
        && targeted_course_started_for_bacterium(rule_events, bacteria_idx)
}
...
/// Capped samples of complete mechanism profiles from active infections.
///
/// Profiles are stored as bitmasks by region, care setting, and bacterium. Retaining
/// complete sampled profiles preserves mechanism co-occurrence within each reservoir.
#[derive(Clone, Serialize, Deserialize)]
pub struct MechanismProfileCache {
    ...
    /// Number of profiles represented while collecting or merging; after daily blending this
    /// is reset to the retained slot length.
...
impl MechanismProfileCache {
        ...
        for r in 0..self.num_regions {
            for h in 0..2 {
                ...
                for b in 0..self.num_bacteria {
                    ...
                    // Normalize total_seen to the blended slot length for the next reservoir.
    ...
    /// then yields a uniform reservoir of the complete combined input stream, even when thread
...
/// - `peak_mechanism_prevalence`: the highest marginal prevalence measured in each eligible
///   regional community reservoir at an annual update. Used by the ratchet-floor mechanism
///   to support local exogenous community reseeding of selected persistent mechanisms.
///
/// - `drug_resistance_prevalence`: prevalence within the current sampled profile reservoir,
///   rebuilt after each daily refresh for constant-time prescribing lookups.
#[derive(Clone, Serialize, Deserialize)]
pub struct MechanismCache {
    /// Profile reservoir for profile-based acquisition sampling.
    pub profiles: MechanismProfileCache,
    /// Highest marginal mechanism prevalence measured in an eligible community reservoir at
    /// an annual update; this is not the all-time peak in the underlying population.
    ...
    /// Resistance prevalence within each sampled
    /// `[region x hospital/community x bacteria x drug]` reservoir, rebuilt after refreshes.
    drug_resistance_prevalence: Vec<f64>,
    /// First sampled-and-archived genotype carrying each locally established mechanism.
...
impl MechanismCache {
    ...
    /// Debug helper: seed every hospital reservoir whose bacterium has an applicable resistance
    /// mechanism with one such single-mechanism profile before day 0.
    ...
    /// stored annual maximum can increase.
    ///
    ...
    /// Sample a complete mechanism profile bitmask for a care-setting context.
    ///
    /// The local-persistence archive receives its configured first opportunity. Otherwise,
    /// `hospital` selects the preferred current reservoir, with fallback to the other care
    /// setting in the same region-bacterium slot when the preferred reservoir is empty.
    ...
    ///
    /// The hospital local-persistence archive receives its configured opportunity before the
    /// current-reservoir behavior described below.
    ///
    /// While the current hospital reservoir has no resistant genotype for this
    /// region×bacterium, hospital acquisition bootstraps from the current hospital and
    /// community reservoirs (or whichever is nonempty). When the current hospital reservoir
    /// contains a resistant profile, acquisition samples that reservoir alone.
    ///
    ...
    /// Return the prevalence calculated within the current sampled profile reservoir.
    #[inline]
    pub fn prevalence(
        &self,
        region_idx: usize,
...
pub struct TimeStepSummary {
    ...
    // Current row plus up to 364 preceding retained rows. These fields cover fewer than
    // 365 simulated days at simulation start, an alternate-continuation start, or the start of
    // a sparsely retained output window.
    pub deaths_past_year: usize,
    pub deaths_background_past_year: usize,
    pub deaths_sepsis_past_year: usize,
    pub deaths_infection_non_sepsis_past_year: usize,
    ...
    // Non-background deaths associated with every bacterium active at death. These are
    // co-occurrence counts, not causal attribution, and include drug-toxicity deaths.
    pub deaths_by_bacteria: Vec<usize>, // indexed by bacterium
    ...
    /// Potential treatment-option landscape across introduced drugs meeting the global potency
    /// threshold, irrespective of regional availability or prescribing.
    /// Numerator: potency multiplied by retained activity across those infection-drug pairs.
    pub potential_activity_existing_drugs_sum_by_bacteria: Vec<f64>,
    /// Denominator for that landscape: the same introduced, potency-qualified drug sum without
    /// resistance.
    pub max_possible_potential_activity_existing_drugs_sum_by_bacteria: Vec<f64>,
    /// Infection-course and treatment aggregates, indexed by bacterium.
    #[serde(default)]
    ...
    #[serde(default)]
    /// Same non-background death/active-bacterium association used by `deaths_by_bacteria`;
    /// despite the legacy name, this can include drug-toxicity deaths and is not causal.
    pub infection_death_count_by_bacteria: Vec<usize>,
    #[serde(default)]
    /// Legacy fifth-day course/positive-bacterium-level association count. It does not establish
    /// that the course targeted the bacterium and is not a failure event emitted by the rules.
    ...
    pub deaths_infected_by_bacteria_region: Vec<usize>, // all-cause death/active-bacterium associations by home region
    pub newly_infected_past_year: usize, // current plus up to 364 preceding retained rows
    ...
    // Post-rule active-infection stock split by the care location captured before today's rules,
    // not by infection acquisition route.
    ...
    // Legacy fifth-day course diagnostics; courses are not attributed to target bacteria.
    pub drug_failure_events_by_bacteria_region: Vec<usize>, // each day-5 active course replicated over bacteria with level > 0, by home region
    pub drug_treatment_day5_events_by_bacteria_region: Vec<usize>, // each course reaching day 5 replicated over every bacterium, by home region
    ...
    // Drug initiation tracking at the configured post-infection evaluation day. The field
    // names retain `day_7` for output compatibility even when another day is configured.
    pub day_7_evaluations_by_bacteria: Vec<usize>, // evaluations by bacterium
    pub day_7_drug_used_by_bacteria: Vec<usize>, // evaluations with a drug started since infection onset
    ...
    // Living post-rule stock classified by the effective region captured before today's rules.
    pub living_population_by_region: Vec<usize>, // six regions; excludes not-yet-born records

    // Living post-rule stock whose pre-rule care status was hospitalized, classified by the
    // pre-rule effective region.
    pub hospital_population_by_region: Vec<usize>,
    ...
    // Living post-rule stock by pre-rule effective region and current age group
    // (6 regions * 5 age groups = 30 values).
    ...
    // Post-rule septic bacterium records by syndrome and pre-rule effective region. One person
    // can contribute multiple records (10 syndromes * 6 regions = 60 values).
    // [syndrome_idx * 6 + region_idx] where syndrome_idx: 0-9 (syndromes 1-10), region_idx: 0-5
    pub syndrome_population_by_region: Vec<usize>,

    // Death/bacterium associations by syndrome and pre-rule effective region. A death can
    // contribute to multiple syndrome slots (10 syndromes * 6 regions = 60 values).
    // [syndrome_idx * 6 + region_idx] where syndrome_idx: 0-9 (syndromes 1-10), region_idx: 0-5
    pub syndrome_deaths_sepsis_by_region: Vec<usize>,
    pub syndrome_deaths_infection_non_sepsis_by_region: Vec<usize>,

    // Post-rule drug use classified by the effective region captured before today's rules
    // (6 regions * num_drugs values).
    pub currently_on_drug_by_region_drug: Vec<usize>,

    // Polypharmacy among records without a death date. Unlike `total_population`, this pass
    // does not exclude not-yet-born records.
    pub people_on_1_drug: usize,
    pub people_on_2_drugs: usize,
    pub people_on_3plus_drugs: usize,
...
impl TimeStepSummary {
    fn validate_regional_resistance_dimensions(&self) -> Result<(), std::io::Error> {
        ...
        for (name, actual, expected) in [
            (
                "infected counts",
                self.regional_resistance_infected_by_bacteria_region.len(),
                infected_len,
            ),
            (
                "positive counts",
    ...
    fn append_regional_resistance_csv_values(&self, row: &mut String) {
        row.push_str(if self.regional_resistance_collected {
            ",1"
        } else {
            ",0"
        });
        for region_idx in 0..REGION_COUNT {
            for bacteria_idx in 0..BACTERIA_LIST.len() {
                ...
                for drug_idx in 0..DRUG_SHORT_NAMES.len() {
                    let index = (bacteria_idx * DRUG_SHORT_NAMES.len() + drug_idx) * REGION_COUNT
                        + region_idx;
                    let (positive, any_r_sum) = if self.regional_resistance_collected {
                        (
                            self.regional_resistance_positive_by_bacteria_drug_region[index],
                            self.regional_resistance_any_r_sum_by_bacteria_drug_region[index],
                        )
                    } else {
                        (0, 0.0)
                    };
    ...
    fn validate_regional_death_dimensions(&self) -> std::io::Result<()> {
        ...
        for (name, actual, expected) in [
            (
                "region",
                self.deaths_by_region.len(),
                REGION_COUNT * NUM_DEATH_CAUSES,
            ),
            (
                "region and age",
                self.deaths_by_region_age.len(),
                REGION_COUNT * 5 * NUM_DEATH_CAUSES,
            ),
        ] {
            if actual != expected {
                return Err(std::io::Error::new(
                    std::io::ErrorKind::InvalidData,
                    format!(
                        "deaths by {name} at timestep {}: expected {expected} values, got {actual}",
                        self.time_step
                    ),
                ));
            }
...
pub struct Simulation {
    ...
    /// Baseline summary rows retained under the selected calibration mode.
    pub summary_log: Vec<TimeStepSummary>,
    /// One set of retained continuation summaries per completed alternate policy option.
    pub policy_branch_summary_log: Vec<PolicyBranchSummary>,
    /// Precomputed parameter values and lookup tables used by the rules hot path.
    pub param_cache: crate::rules::ParameterKeyCache,
    /// Precomputed potency values indexed by `[bacterium * num_drugs + drug]`.
    ...
    /// Journey logger for sampled infection episodes.
    pub journey_logger: JourneyLogger,
    /// Optional fixed RNG seed for deterministic runs.
    pub rng_seed: Option<u64>,
    /// Pseudo-random identifier regenerated by each `run()` call and written to summary rows
    /// and checkpoint metadata; uniqueness across independent runs is not guaranteed.
    pub run_id: u32,
    /// Suppresses baseline-only logging while an alternate-policy continuation is running.
    branch_active: bool,
    ...
    /// Whether the temporary branch snapshot should be stored on disk rather than in memory.
    use_disk_branch_checkpoint: bool,
    /// Directory used for temporary branch-checkpoint files; files are deleted when their
    /// checkpoint handles are dropped.
    branch_checkpoint_dir: PathBuf,
    /// Precomputed legacy mapping from each bacterium to drugs with potency above `0.01`.
    /// The current run loop retains this public data but does not consume it.
    pub relevant_drugs_by_bacteria: Vec<Vec<usize>>,
    /// Controls simulation horizon, continuation policies, retained years, and diagnostic detail.
    pub calibration_mode: CalibrationMode,
    /// Controls which summary field groups are stored per row. Initialized from the selected
    /// calibration mode; callers may set [`SummaryContentFlags::for_figures`] or
    /// [`SummaryContentFlags::none`] before `run()` for a specialized output.
    pub summary_content_flags: SummaryContentFlags,
}

impl Simulation {
    /// Create a simulation with an initialized population, caches, and lookup tables.
    ///
    /// The run identifier remains zero until [`Simulation::run`] is called.
    pub fn new(
        ...
        // Build the retained legacy bacterium-drug map, excluding pairs at the model's
        // negligible-activity placeholder. The current run loop does not consume this map.
    ...
    /// Replace the requested policy IDs for this mode's branch point.
    ///
    /// The baseline (policy 0) always runs independently; including ID 0 here is accepted but
    /// does not create another continuation. IDs 1-4 select alternate continuations. Invalid
    /// IDs emit a warning and are ignored. Call this after `Simulation::new()` and before `run()`.
    ///
    /// # Examples
    /// ```ignore
    /// // Run the baseline plus the stewardship continuation:
    /// simulation.set_active_policy_branches(&[1]);
    ///
    /// // Run the baseline plus all four alternate continuations:
    ...
    /// Store the branch-point snapshot in a temporary disk file instead of keeping it in memory.
    /// The file is removed when the checkpoint handle is dropped. When `directory` is `None`,
    /// the existing configured directory is retained (initially `amr_branch_checkpoints`).
    ...
    /// Keep the branch-point snapshot in memory instead of writing a temporary disk file.
    ...
    fn run_from(
        ...
        for t in start_step..self.time_steps {
            ...
            struct LocalTotals {
                ...
                /// Infection-course and treatment aggregates by bacterium. The death field is
                /// a non-background death/active-bacterium association, not causal attribution.
                ...
                /// active people assigned once to their first valid infection syndrome (1-10)
                infected_by_syndrome: Vec<usize>,
                /// active person-bacterium records by bacterium and syndrome
                infected_by_syndrome_by_bacteria: Vec<usize>,
                /// acquisition people assigned once to the first valid acquisition-event syndrome
                newly_infected_by_syndrome: Vec<usize>,
                /// post-rule living stock by pre-rule effective region
                living_population_by_region: Vec<usize>,
                /// post-rule living stock by pre-rule effective region and current age group
                age_distribution_by_region: Vec<usize>,
                /// deaths by pre-rule effective region and cause
                deaths_by_region: Vec<usize>,
                /// deaths by pre-rule effective region, current age, and cause
                deaths_by_region_age: Vec<usize>,
                /// post-rule drug use by pre-rule effective region
                currently_on_drug_by_region_drug: Vec<usize>,
                /// sepsis-death/septic-bacterium records by syndrome and pre-rule region
                syndrome_deaths_sepsis_by_region: Vec<usize>,
                /// non-sepsis-death/active-nonseptic-bacterium records by syndrome and pre-rule region
                syndrome_deaths_infection_non_sepsis_by_region: Vec<usize>,
                /// post-rule living stock with pre-rule hospitalization, by pre-rule region
                hospital_population_by_region: Vec<usize>,
                /// Configured post-infection evaluation-day counts (`CalibrationMode::None` only).
                day_7_evaluations_by_bacteria: Vec<usize>,
                /// Drug initiation since infection onset by that evaluation day
            ...
            impl LocalTotals {
                fn new(
                    ...
                    let bacteria_drug_len = if collect_summary_stats {
                    ...
                    Self {
                        ...
                        infected_and_on_any_drug_by_bacteria: if collect_summary_stats {
                            vec![0; num_bacteria]
                        } else {
                            Vec::new()
                        },
                        infections_by_bacteria: if collect_summary_stats {
                            vec![0; num_bacteria]
                        } else {
                            Vec::new()
                        },
                        ...
                        deaths_by_bacteria: if collect_summary_stats {
                            vec![0; num_bacteria]
                        } else {
                            Vec::new()
                        },
                        ...
                        currently_on_drug_by_drug: if collect_summary_stats {
                            vec![0; num_drugs]
                        } else {
                            Vec::new()
                        },
                        ...
                        new_sepsis_cases: 0,
                        number_with_sepsis_by_bacteria: if collect_summary_stats {
                            vec![0; num_bacteria]
                        } else {
                            Vec::new()
                        },
                        new_sepsis_cases_by_bacteria: if collect_summary_stats {
                            vec![0; num_bacteria]
                        } else {
                            Vec::new()
                        },
                        ...
                        newly_infected_by_bacteria_region: if collect_summary_stats {
                            vec![0; num_bacteria * REGION_COUNT]
                        } else {
                            Vec::new()
                        },
                        newly_infected_carrier_by_bacteria: if collect_summary_stats {
                            vec![0; num_bacteria]
                        } else {
                            Vec::new()
                        },
                        newly_infected_non_carrier_by_bacteria: if collect_summary_stats {
                            vec![0; num_bacteria]
                        } else {
                            Vec::new()
                        },
                        carrier_at_risk_person_days_by_bacteria: if collect_summary_stats {
                            vec![0; num_bacteria]
                        } else {
                            Vec::new()
                        },
                        non_carrier_at_risk_person_days_by_bacteria: if collect_summary_stats {
                            vec![0; num_bacteria]
                        } else {
                            Vec::new()
                        },
                        new_infections_in_carriers_by_bacteria: if collect_summary_stats {
                            vec![0; num_bacteria]
                        } else {
                            Vec::new()
                        },
                        new_infections_in_non_carriers_by_bacteria: if collect_summary_stats {
                            vec![0; num_bacteria]
                        } else {
                            Vec::new()
                        },
                        new_any_r_infections_in_carriers_by_bacteria: if collect_summary_stats {
                            vec![0; num_bacteria]
                        } else {
                            Vec::new()
                        },
                        new_any_r_infections_in_non_carriers_by_bacteria: if collect_summary_stats {
                            vec![0; num_bacteria]
                        } else {
                            Vec::new()
                        },
                        ...
                        deaths_infected_by_bacteria_region: if collect_summary_stats {
                            vec![0; num_bacteria * REGION_COUNT]
                        } else {
                            Vec::new()
                        },
                        ...
                        presence_microbiome_by_bacteria: if collect_summary_stats {
                            vec![0; num_bacteria]
                        } else {
                            Vec::new()
                        },
                        presence_microbiome_resistant_by_bacteria: if collect_summary_stats {
                            vec![0; num_bacteria]
                        } else {
                            Vec::new()
                        },
                        ...
                        infected_carrier_count_by_bacteria: if collect_summary_stats {
                            vec![0; num_bacteria]
                        } else {
                            Vec::new()
                        },
                        infected_non_carrier_count_by_bacteria: if collect_summary_stats {
                            vec![0; num_bacteria]
                        } else {
                            Vec::new()
                        },
                        resistant_infected_carrier_count_by_bacteria: if collect_summary_stats {
                            vec![0; num_bacteria]
                        } else {
                            Vec::new()
                        },
                        resistant_infected_non_carrier_count_by_bacteria: if collect_summary_stats {
                            vec![0; num_bacteria]
                        } else {
                            Vec::new()
                        },
                        ...
                        num_age_50_79: 0,
                        num_age_80plus: 0,
                        activity_r_sum_by_bacteria: if collect_summary_stats {
                            vec![0.0; num_bacteria]
                        } else {
                            Vec::new()
                        },
                        max_possible_activity_r_sum_by_bacteria: if collect_summary_stats {
                            vec![0.0; num_bacteria]
                        } else {
                            Vec::new()
                        },
                        activity_r_pure_sum_by_bacteria: if collect_summary_stats {
                            vec![0.0; num_bacteria]
                        } else {
                            Vec::new()
                        },
                        max_possible_activity_r_pure_sum_by_bacteria: if collect_summary_stats {
                            vec![0.0; num_bacteria]
                        } else {
                            Vec::new()
                        },
                        ...
                        new_active_infections_by_bacteria: if collect_summary_stats {
                            vec![0; num_bacteria]
                        } else {
                            Vec::new()
                        },
                        active_infection_days_by_bacteria: if collect_summary_stats {
                            vec![0; num_bacteria]
                        } else {
                            Vec::new()
                        },
                        treated_infection_days_by_bacteria: if collect_summary_stats {
                            vec![0; num_bacteria]
                        } else {
                            Vec::new()
                        },
                        effective_treated_infection_days_by_bacteria: if collect_summary_stats {
                            vec![0; num_bacteria]
                        } else {
                            Vec::new()
                        },
                        infection_resolution_count_by_bacteria: if collect_summary_stats {
                            vec![0; num_bacteria]
                        } else {
                            Vec::new()
                        },
                        infection_death_count_by_bacteria: if collect_summary_stats {
                            vec![0; num_bacteria]
                        } else {
                            Vec::new()
                        },
                        drug_failure_count_by_bacteria: if collect_summary_stats {
                            vec![0; num_bacteria]
                        } else {
                            Vec::new()
                        },
                        ...
                        infected_with_bacteria_and_mechanism: if collect_summary_stats {
                            vec![0; num_bacteria * ResistanceMechanism::all().len()]
                        } else {
                            Vec::new()
                        },
                ...
                fn merge(&mut self, other: Self) {
                    for (total, value) in self
                        ...
                        .iter_mut()
                        .zip(other.regional_resistance_infected_by_bacteria_region)
                    {
                        *total += value;
                    }
                    for (total, value) in self
                        ...
                        .iter_mut()
                        .zip(other.regional_resistance_positive_by_bacteria_drug_region)
                    {
                        *total += value;
                    }
                    for (total, value) in self
                        ...
                        .iter_mut()
                        .zip(other.regional_resistance_any_r_sum_by_bacteria_drug_region)
                    {
                        *total += value;
                    }
            ...
            let collect_summary_stats = calibration_mode.retains_summary_year(sim_year);
            ...
            let collect_regional_stats =
                collect_summary_stats && self.summary_content_flags.regional;
            let collect_syndrome_stats =
                collect_summary_stats && self.summary_content_flags.syndrome;
            let collect_resolution_stats =
                collect_summary_stats && self.summary_content_flags.resolution;
            let collect_per_bacteria_detail_stats =
                collect_summary_stats && self.summary_content_flags.per_bacteria_detail;
            let collect_split_burden_stats =
                collect_summary_stats && self.summary_content_flags.split_burden;
            let collect_serious_r_hc_stats =
                collect_summary_stats && self.summary_content_flags.serious_r_hc;
            let collect_microbiome_detail_stats =
                collect_summary_stats && self.summary_content_flags.microbiome_detail;
            ...
            let chunk_totals: Vec<Box<LocalTotals>> = self
                ...
                .map(|(chunk_idx, chunk)| {
                    ...
                    if individual.date_of_death.is_none() && individual.age >= 0 {
                        ...
                        let has_any_microbiome = collect_summary_stats
                            && individual
                                .presence_microbiome
                                .iter()
                                .enumerate()
                    ...
                    let mut pre_acquisition_carrier_at_risk = if collect_summary_stats {
                        vec![false; num_bacteria]
                    } else {
                        Vec::new()
                    };
                    let mut pre_acquisition_non_carrier_at_risk = if collect_summary_stats {
                        vec![false; num_bacteria]
                    } else {
                        Vec::new()
                    };
                    ...
                    let track_diagnostic_cascade_for_person_day = track_diagnostic_cascade
                        && vital_status.available_for_current_day_event_attribution;
                    if track_diagnostic_cascade_for_person_day {
                        for b_idx in 0..num_bacteria {
                            ...
                            if individual.diagnostic_cascade_open[b_idx] {
                                ...
                                if diagnostic_targeted_treatment_recordable(
                                    individual,
                                    &rule_events,
                                    b_idx,
                                ) {
                    ...
                    if track_diagnostic_cascade
                        && !track_diagnostic_cascade_for_person_day
                        && individual
                            .diagnostic_cascade_open
                            .iter()
                    ...
                    if collect_summary_stats {
                        if let Some(death_time) = individual.date_of_death {
                            ...
                            // Attribute the death to the effective region captured before today's rules.
                            ...
                            if let Some(ref cause) = individual.cause_of_death {
                                match cause.as_str() {
                                    ...
                                    "sepsis_related" => {
                                        ...
                                        // one death can increment multiple syndrome-region slots.
                                        if collect_syndrome_stats {
                                            for b_idx in 0..BACTERIA_LIST.len() {
                                    ...
                                    "infection_non_sepsis_related" => {
                                        ...
                                        // syndrome; one death can increment multiple slots.
                                        if collect_syndrome_stats {
                                            for b_idx in 0..BACTERIA_LIST.len() {
                                                if individual.level[b_idx] > INFECTION_EPS
                                                    && !individual.sepsis[b_idx]
                            ...
                            // Associate each non-background death, including a drug-toxicity
                            // death, with every bacterium active at death. This is co-occurrence,
                            ...
                            // Associate every death, including background mortality, with every
                            // active bacterium and the person's home region.
                            for b_idx in 0..num_bacteria {
                                if individual.level[b_idx] > INFECTION_EPS {
                    ...
                    if collect_summary_stats && vital_status.counts_as_living_stock {
                        // Population denominators exclude future cohort members not yet born.
                        lt.living_population += 1;

                        // Classify post-rule survivors by the effective region captured pre-rule.
                        let region_idx = effective_region_idx;
                        if collect_regional_stats {
                        ...
                        // Count post-rule drug use, using the pre-rule effective region for
                        // regional splits.
                        ...
                        // Legacy fifth-day course diagnostic without course-to-bacterium
                        // attribution: replicate each course reaching day 5 into every bacterial
                        // denominator, then count every level > 0 while that drug remains active.
                        ...
                        let mut individual_has_any_reportable_infection = false;
                        {
                            ...
                            for b_idx in 0..num_bacteria {
                                if individual.level[b_idx] > INFECTION_EPS {
                                    ...
                                    // Count this person once under the first valid active-infection syndrome.
                                    ...
                                    // Count every active person-bacterium record under its syndrome.
                                    let syndrome_id = individual.infectious_syndrome[b_idx];
                                    ...
                                    // Cross-bacterium duration summaries use the general clinical
                                    // reporting scope used by `total_currently_infected`.
                                    if bacterium_is_in_general_clinical_reporting_scope(b_idx)
                                    ...
                                    for d_idx in 0..num_drugs {
                                        ...
                                        if collect_summary_stats && any_r > 0.0 {
                        ...
                        // Hospital stock uses the care setting captured before today's rules.
                        if is_hospitalized {
                        ...
                        // Count post-rule sepsis stock by bacterium.
                        for b_idx in 0..num_bacteria {
                            if individual.sepsis[b_idx] {
                    ...
                    // For living-stock records, consume and clear these transient event buffers,
                    // including on discarded-output days, so events cannot leak into a later row.
                    if vital_status.counts_as_living_stock {
                            ...
                            .enumerate()
                        {
                            ...
                            for (cat_idx, count) in category_counts.iter_mut().enumerate() {
                                if *count > 0 {
                                    if collect_summary_stats
                                        && !lt.cleared_any_r_microbiome_categories.is_empty()
                                    {
                    ...
                    // The unabridged (`None`) mode retains these additional diagnostics.
                    if calibration_mode == CalibrationMode::None
                        && individual.date_of_death.is_none()
                        && individual.age >= 0
                    {
                        // Track drug use at the configured post-infection evaluation day; the
                        // output field names retain the legacy `day_7` label.
                        for b_idx in 0..num_bacteria {
                        ...
                        // Count each septic person-bacterium record by syndrome and the effective
                        // region captured before today's rules.
                    ...
                    // Reset per-timestep output buffers after aggregation. Because individual and
                    // journey logging runs after this population pass, those loggers observe the
                    // cleared resolution and infection-prevention buffers.
            ...
            // Update mechanism profile cache with freshly-collected profiles
            {
                ...
                // Annually update the ratchet's stored maximum from the refreshed sampled
                // community reservoir; intervening population peaks are not observed here.
                if t % 365 == 0 {
            ...
            // Construct a row only when the selected mode retains this year. In windowed modes
            // that means 2022-2025; model updates and in-pass buffer resets still run daily.
            if collect_summary_stats {
                ...
                // Sum the current row and at most 364 earlier retained rows. At the start of a
                // sparse output window or alternate continuation, this is shorter than 365
                // simulated days because earlier rows are unavailable in `summary_log`.
                ...
                let mut summary = TimeStepSummary {
                    ...
                    deaths_drug_toxicity,
                    drug_stops_due_to_toxicity,
                    // Current row plus up to 364 earlier retained summary rows.
                    ...
                    // The unabridged (`None`) mode populates these fields below.
                    people_on_1_drug: 0,
                    people_on_2_drugs: 0,
                    people_on_3plus_drugs: 0,

                    // These high-volume diagnostics are retained only in the unabridged mode.
                    infected_on_drug_with_previous_failure,
                    drug_selection_count_by_bacteria: if collect_summary_stats
                    ...
                    drug_score_sums_by_bacteria_drug: if collect_summary_stats
                        && self.calibration_mode == CalibrationMode::None
                    {
                        drug_score_sums_by_bacteria_drug
                    } else {
                ...
                // In unabridged mode this pass includes every record without a death date,
                // including not-yet-born records (normally in the zero-drug bin).
                ...
                // Windowed Full* modes emit only the rows consumed by calibration_summary.py for
                // the 2025 summary: 2022-2025 inclusive. Their other rows are dropped.
                // Partial, Partial25Counterfactual, and None keep every baseline row.
        ...
        // Flush records not written by a periodic journey-log flush.
        if self.journey_logger.enabled && !self.branch_active {
            let _ = self.journey_logger.finalize();
        ...
        P: AsRef<std::path::Path>,
    {
        ...
        for summary in self.summary_log.iter().chain(
            self.policy_branch_summary_log
                .iter()
                .flat_map(|branch| branch.summaries.iter()),
        ) {
        ...
        // Add non-background death/active-bacterium association columns (not causal attribution).
        for bacteria in BACTERIA_LIST.iter() {
            header.push(',');
            header.push_str(&bacteria.replace(" ", "_"));
        ...
        // Add legacy fifth-day active-course/positive-level association columns by home region.
        for bacteria in BACTERIA_LIST.iter() {
            for region in &[
                "north_america",
                "south_america",
        ...
        // Add fifth-day course denominators replicated across every bacterium by home region.
        for bacteria in BACTERIA_LIST.iter() {
            for region in &[
                "north_america",
                "south_america",
        ...
        // Add all-cause death/active-bacterium association columns by home region.
        for bacteria in BACTERIA_LIST.iter() {
            for region in &regions {
                header.push(',');
                header.push_str(&bacteria.replace(" ", "_"));
        ...
        // Add post-rule drug-use columns by pre-rule effective region.
        ...
        // Add configured evaluation-day drug-initiation columns (legacy names say day 7).
        for bacteria in BACTERIA_LIST.iter() {
            header.push(',');
            header.push_str(&bacteria.replace(" ", "_"));
        ...
        // Add post-rule living-stock columns by pre-rule effective region.
        let region_names = [
            "north_america",
            "south_america",
            "africa",
        ...
        // Add post-rule living-stock columns with pre-rule hospitalization and region.
        for region_name in &region_names {
            header.push(',');
        ...
        // Add post-rule living age-distribution columns by pre-rule effective region.
        let age_group_names = [
            "prop_age_0_5",
            "prop_age_6_14",
            "prop_age_15_49",
        ...
        // Add age-specific death columns by pre-rule effective region.
        for region_name in &region_names {
            for age_group_name in &age_group_names {
                for death_type_name in &death_type_names {
                    header.push(',');
        ...
        // Add septic person-bacterium record columns by syndrome and pre-rule region.
        for syndrome_id in 1..=10 {
            // syndromes 1-10
            for region_name in &region_names {
                header.push(',');
        ...
        // Add sepsis-death/septic-bacterium association columns by syndrome and pre-rule region.
        for syndrome_id in 1..=10 {
            // syndromes 1-10
            for region_name in &region_names {
                header.push(',');
        ...
        // Add non-sepsis-death/active-bacterium association columns by syndrome and pre-rule region.
        for syndrome_id in 1..=10 {
            // syndromes 1-10
            for region_name in &region_names {
                header.push(',');
        ...
        // Add concurrent-drug histogram labels. The exported population includes unborn records.
        ...
        // Write data with pre-built strings (baseline followed by alternate continuations).
        ...
        for summary in combined_summaries {
            ...
            // Add legacy fifth-day active-course/positive-level association data.
            append_usize_slice_or_zeros(
                &mut row,
                &summary.drug_failure_events_by_bacteria_region,
                BACTERIA_LIST.len() * REGION_COUNT,
            );
            // Add fifth-day course denominators replicated across bacterial slots.
            append_usize_slice_or_zeros(
                &mut row,
            ...
            // Add configured evaluation-day drug-initiation data (legacy field names say day 7).
            append_usize_slice_or_zeros(
                &mut row,
            ...
            // Add post-rule living stock by pre-rule effective region.
            append_usize_slice_or_zeros(&mut row, &summary.living_population_by_region, 6);

            // Add post-rule living stock with pre-rule hospitalization and region.
            ...
            // Add post-rule age distribution by pre-rule effective region (as proportions).
            ...
            // Add deaths by pre-rule effective region (as counts).
            append_usize_slice_or_zeros(&mut row, &summary.deaths_by_region, 6 * NUM_DEATH_CAUSES);

            // Add age-specific deaths by pre-rule effective region (as counts).
            append_usize_slice_or_zeros(
                &mut row,
            ...
            // Add septic person-bacterium records by syndrome and pre-rule region.
            append_usize_slice_or_zeros(&mut row, &summary.syndrome_population_by_region, 10 * 6);

            // Add sepsis-death/septic-bacterium associations by syndrome and pre-rule region.
            append_usize_slice_or_zeros(
                &mut row,
            ...
            // Add non-sepsis-death/active-bacterium associations by syndrome and pre-rule region.
            append_usize_slice_or_zeros(
                &mut row,
            ...
            // Add the non-dead-record drug-count histogram, including not-yet-born records.
...
mod tests {
    use super::{
        ...
        diagnostic_effective_targeted_recordable, diagnostic_targeted_treatment_recordable,
        has_active_reportable_infection, infection_death_has_model_scope_contributor,
        ...
        DIAGNOSTIC_CASCADE_STAGE_COUNT, DIAGNOSTIC_CASCADE_TARGETED_TREATMENT_IDX,
    ...
    fn regional_resistance_fixture() -> Simulation {
        let mut simulation =
            Simulation::new(8, 1, false, Some(7_315_440), CalibrationMode::Partial);
        ...
        for (index, individual) in simulation.population.individuals.iter_mut().enumerate() {
            ...
            individual.resistances[0][0].any_r =
                store_float(if index % 2 == 0 { 0.5 } else { 0.0 });
        }
    ...
    fn model_scope_death_fixture() -> Simulation {
        ...
        let mut simulation =
            Simulation::new(1, day + 1, false, Some(654_321), CalibrationMode::Partial);
        ...
        let included = BACTERIA_LIST
            .iter()
            .position(|&name| name == "escherichia_coli")
            .unwrap();
        let included_second = BACTERIA_LIST
            .iter()
            .position(|&name| name == "staphylococcus_aureus")
            .unwrap();
        let excluded = ["helicobacter_pylori", "mdr_mycobacterium_tuberculosis"].map(|name| {
            BACTERIA_LIST
                .iter()
                .position(|&bacteria| bacteria == name)
                .unwrap()
        });
        let level = crate::config::parameter_store()
            .globals
            .infection_non_sepsis_minimum_bacteria_level
            .max(1.0)
            + 1.0;
                ...
                for age in ages {
                    for cause in ["sepsis_related", "infection_non_sepsis_related"] {
                        for cohort in 0..4 {
                            ...
                            person.region_cur_in = if cohort % 2 == 0 {
                                Region::Home
                            } else {
                                *region
                            };
    ...
    fn model_scope_deaths_existing_region_and_age_fields_reconcile_with_scoped_headlines() {
        for flags in [
            ...
            for cause in [
                super::DEATH_CAUSE_SEPSIS_IDX,
                super::DEATH_CAUSE_INFECTION_NON_SEPSIS_IDX,
            ] {
                for region in 0..super::REGION_COUNT {
                    let regional_count =
                        summary.deaths_by_region[region * super::NUM_DEATH_CAUSES + cause];
                    ...
                    for (age, expected) in [4, 4, 4, 4, 2].into_iter().enumerate() {
                        ...
                        assert_eq!(
                            count,
                            expected * (region + 1),
                            "region {region}, age {age}, cause {cause}"
                        );
                        age_total += count;
                    }
                ...
                assert_eq!(
                    summary
                        .deaths_by_region
                        .chunks_exact(super::NUM_DEATH_CAUSES)
                        .map(|region| region[cause])
                        .sum::<usize>(),
                    378
                );
            }
            ...
            assert_eq!(
                summary.deaths_by_region.iter().sum::<usize>(),
                summary.deaths_background
                    + summary.deaths_drug_toxicity
                    + summary.deaths_sepsis_model_scope
                    + summary.deaths_infection_non_sepsis_model_scope
            );
    ...
    fn model_scope_deaths_ignore_noncontributors_and_old_deaths_preserve_other_causes() {
        ...
        let included = BACTERIA_LIST
            ...
            .position(|&name| name == "escherichia_coli")
            .unwrap();
        let excluded = BACTERIA_LIST
            .iter()
            .position(|&name| name == "helicobacter_pylori")
            .unwrap();
        let threshold = crate::config::parameter_store()
            .globals
            .infection_non_sepsis_minimum_bacteria_level;
        for cause in ["sepsis_related", "infection_non_sepsis_related"] {
            for case in 0..3 {
                ...
                person.level[included] = if case == 0 {
                    if cause == "sepsis_related" {
                        INFECTION_EPS
                    } else {
                        threshold
                    }
                } else {
                    threshold.max(1.0) + 1.0
                };
        ...
        for cause in [
            Some("background_mortality"),
            Some("drug_toxicity_related"),
            Some("unknown"),
            None,
        ] {
            let mut person = template.clone();
        ...
        assert_eq!(
            summary.deaths_by_region[super::DEATH_CAUSE_BACKGROUND_IDX],
            3
        );
        assert_eq!(
            summary.deaths_by_region[super::DEATH_CAUSE_DRUG_TOXICITY_IDX],
            1
        );
    ...
    fn model_scope_deaths_disabled_profile_clears_existing_fields_without_changing_state() {
        ...
        disabled
            ...
            .unwrap();
        assert_eq!(
            bincode::serialize(&collected.create_branch_snapshot()).unwrap(),
        ...
        for (column, name) in headers.iter().enumerate() {
            if super::REGIONAL_RESISTANCE_REGION_NAMES
                .iter()
                .any(|region| name.starts_with(&format!("{region}_")) && name.contains("_deaths_"))
            {
    ...
    fn model_scope_deaths_export_preserves_layout_and_rejects_incomplete_arrays() {
        ...
        simulation
            ...
            .unwrap();
        let path =
            Path::new(env!("CARGO_MANIFEST_DIR")).join("target/model_scope_deaths_fixture.csv");
        ...
        assert_eq!(
            ...
            format!(
                "regional_resistance_oceania_{}_{}_any_r_sum",
                BACTERIA_LIST.last().unwrap(),
                DRUG_SHORT_NAMES.last().unwrap()
            )
        );
        ...
        for (region_idx, region) in super::REGIONAL_RESISTANCE_REGION_NAMES.iter().enumerate() {
            for cause in ["deaths_sepsis", "deaths_infection_non_sepsis"] {
                assert!(!headers
                    .iter()
                    .any(|name| name == format!("{region}_{cause}_model_scope")));
                let column = headers
                    .iter()
                    .position(|name| name == format!("{region}_{cause}"))
                    .unwrap();
                ...
                for (age_idx, age) in age_names.iter().enumerate() {
                    ...
                    assert_eq!(
                        row[column].parse::<usize>().unwrap(),
                        [4, 4, 4, 4, 2][age_idx] * (region_idx + 1)
                    );
                }
            }
        }
    ...
    fn assert_aligned_infection_statistics(simulation: &Simulation, hospital_before: &[bool]) {
        ...
        for b in 0..BACTERIA_LIST.len() {
            let active: Vec<_> = simulation
                .population
                .individuals
                .iter()
                .enumerate()
                .filter(|(_, person)| {
                    person.date_of_death.is_none()
                        && person.age >= 0
                        && person.level[b] > INFECTION_EPS
                })
                .collect();
            assert_eq!(summary.infections_by_bacteria[b], active.len());
            assert_eq!(
                summary.infected_and_on_any_drug_by_bacteria[b],
                active
                    .iter()
                    .filter(|(_, p)| p.cur_use_drug.iter().any(|&on| on))
                    .count()
            );
            if !summary.infected_with_test_identified_by_bacteria.is_empty() {
                assert_eq!(
                    summary.infected_with_test_identified_by_bacteria[b],
                    active
                        .iter()
                        .filter(|(_, p)| p.test_identified_infection[b])
                        .count()
                );
                assert_eq!(
                    summary.infected_with_test_for_resistance_by_bacteria[b],
                    active
                        .iter()
                        .filter(|(_, p)| p.test_for_resistance[b])
                        .count()
                );
            }
            if !summary.infected_with_bacteria_and_mechanism.is_empty() {
                for m in 0..mechanism_count {
                    assert_eq!(
                        summary.infected_with_bacteria_and_mechanism[b * mechanism_count + m],
                        active
                            .iter()
                            .filter(|(_, p)| p.has_any_mechanism(b, m))
                            .count()
                    );
                }
            }
            for d in 0..drug_count {
                ...
                let positive = active
                    .iter()
                    .filter(|(_, p)| load_float(p.resistances[b][d].any_r) > 0.0)
                    .count();
                let sum: f64 = active
                    .iter()
                    .map(|(_, p)| load_float(p.resistances[b][d].any_r))
                    .sum();
                assert_eq!(
                    summary.infected_with_any_r_positive_by_bacteria_drug[index],
                    positive
                );
                ...
                let susceptible = active
                    .iter()
                    .filter(|(_, p)| {
                        load_float(p.resistances[b][d].any_r)
                            < simulation.mic_lt2_majority_r_thresholds[index]
                    })
                    .count();
                assert_eq!(
                    summary.infected_and_standardized_mic_lt2_by_bacteria_drug[index],
                    susceptible
                );
                let hospital_sum: f64 = active
                    .iter()
                    .filter(|(i, _)| hospital_before[*i])
                    .map(|(_, p)| load_float(p.resistances[b][d].any_r))
                    .sum();
                assert_eq!(
                    summary.any_r_sum_by_bacteria_drug_hospital[index],
                    hospital_sum
                );
                if !summary
                    .infected_with_any_r_positive_hospital_by_bacteria_drug
                    .is_empty()
                {
                    let hospital_positive = active
                        .iter()
                        .filter(|(i, p)| {
                            hospital_before[*i] && load_float(p.resistances[b][d].any_r) > 0.0
                        })
                        .count();
                    assert_eq!(
                        summary.infected_with_any_r_positive_hospital_by_bacteria_drug[index],
                        hospital_positive
                    );
                    assert_eq!(
                        summary.infected_with_any_r_positive_community_by_bacteria_drug[index],
                        positive - hospital_positive
                    );
                    assert!(
                        hospital_positive
                            <= summary.currently_infected_hospital_count_by_bacteria[b]
                    );
                    assert!(
                        positive - hospital_positive
                            <= summary.currently_infected_community_count_by_bacteria[b]
                    );
                }
                if summary.regional_resistance_collected {
                    let start = index * super::REGION_COUNT;
                    assert_eq!(
                        summary.regional_resistance_positive_by_bacteria_drug_region
                            [start..start + super::REGION_COUNT]
                            .iter()
                            .sum::<usize>(),
                        positive
                    );
    ...
    fn aligned_resistance_statistics_match_population_in_all_profiles() {
        for flags in [
            ...
            let before: Vec<_> = simulation
                ...
                .individuals
                .iter()
                .map(|p| p.hospital_status.is_hospitalized())
                .collect();
            let initial_sum: f64 = simulation
                .population
                .individuals
                .iter()
                .filter(|p| p.date_of_death.is_none() && p.level[0] > INFECTION_EPS)
                .map(|p| load_float(p.resistances[0][0].any_r))
                .sum();
            simulation.run_from(0, None).unwrap();
            assert!(
                initial_sum > 0.0,
                "fixture must exercise resistant observations"
            );
    ...
    fn aligned_resistance_statistics_exclude_same_day_clearance_and_death() {
        ...
        let before: Vec<_> = simulation
            ...
            .map(|p| p.hospital_status.is_hospitalized())
            .collect();
        let positive_before = simulation
            .population
            .individuals
            .iter()
            .filter(|p| {
                p.date_of_death.is_none()
                    && p.level[0] > INFECTION_EPS
                    && load_float(p.resistances[0][0].any_r) > 0.0
            })
            .count();
        ...
        assert_eq!(
            simulation.population.individuals[1].date_of_death,
            Some(1000)
        );
        assert!(
            simulation.summary_log[0].infected_with_any_r_positive_by_bacteria_drug[0]
                < positive_before,
            "fixture must distinguish pre-rule from post-rule counts"
        );
        assert_aligned_infection_statistics(&simulation, &before);
    }
    ...
    fn regional_resistance_observes_home_region_and_only_surviving_active_infections() {
        ...
        for individual in [
            ...
            super::record_regional_resistance_infection(
                individual,
                0,
                &mut infected,
                &mut positive,
    ...
    fn regional_resistance_runtime_export_matches_post_rule_population() {
        ...
        simulation
            .run_from(0, None)
            .expect("short regional run should succeed");
        let summary = &simulation.summary_log[0];
        assert!(summary.regional_resistance_collected);
            ...
            for (region_idx, region) in super::REGIONAL_RESISTANCE_REGION_NAMES.iter().enumerate() {
                ...
                for drug_idx in 0..DRUG_SHORT_NAMES.len() {
                    let values: Vec<_> = active
                        ...
                        .collect();
                    let index = (bacteria_idx * DRUG_SHORT_NAMES.len() + drug_idx)
                        * super::REGION_COUNT
                        + region_idx;
                    assert_eq!(
                        summary.regional_resistance_positive_by_bacteria_drug_region[index],
                        values.iter().filter(|&&value| value > 0.0).count()
                    );
                    assert_eq!(
                        summary.regional_resistance_any_r_sum_by_bacteria_drug_region[index],
                        values.iter().sum::<f64>()
                    );
                }
            }
        ...
        simulation
            ...
            .expect("regional CSV should export");
        ...
        let start = headers
            .iter()
            .position(|name| name == "regional_resistance_collected")
            .unwrap();
        assert_eq!(
            &headers[start - 1],
            "deaths_infection_non_sepsis_model_scope"
        ...
        assert_eq!(
            &headers[start + 1],
            format!(
                "regional_resistance_north_america_{}_infected_count",
                BACTERIA_LIST[0]
            )
        );
        assert_eq!(
            &headers[start + 2],
            format!(
                "regional_resistance_north_america_{}_{}_positive_count",
                BACTERIA_LIST[0], DRUG_SHORT_NAMES[0]
            )
        );
        assert_eq!(
            &headers[headers.len() - 1],
        ...
        for (bacteria_idx, bacteria) in BACTERIA_LIST.iter().enumerate() {
            for (region_idx, region) in super::REGIONAL_RESISTANCE_REGION_NAMES.iter().enumerate() {
                ...
                assert_eq!(
                    record[column].parse::<usize>().unwrap(),
                    summary.regional_resistance_infected_by_bacteria_region
                        [bacteria_idx * super::REGION_COUNT + region_idx]
                );
                for (drug_idx, drug) in DRUG_SHORT_NAMES.iter().enumerate() {
                    let index = (bacteria_idx * DRUG_SHORT_NAMES.len() + drug_idx)
                        * super::REGION_COUNT
                        + region_idx;
                    assert_eq!(
                        record[column + 1 + 2 * drug_idx].parse::<usize>().unwrap(),
                        summary.regional_resistance_positive_by_bacteria_drug_region[index],
                        "{field} {drug}"
                    );
                    assert_eq!(
                        record[column + 2 + 2 * drug_idx].parse::<f64>().unwrap(),
                        summary.regional_resistance_any_r_sum_by_bacteria_drug_region[index],
                        "{field} {drug}"
                    );
                }
    ...
    fn regional_resistance_collection_flags_preserve_model_state_and_mark_disabled_rows() {
        ...
        assert_eq!(
            bincode::serialize(&pruned).unwrap(),
    ...
    #[test]
    fn discarded_calibration_days_preserve_state_and_retained_output() {
        fn seed_boundary_spanning_episode(simulation: &mut Simulation, entry_step: usize) {
            ...
            individual.sepsis_episode_open[bacteria_idx] = true;
            individual.sepsis_episode_effective_at_onset[bacteria_idx] = false;
            individual.sepsis_episode_first_effective_day[bacteria_idx] = -1;
            individual.sepsis_episode_delay_bucket_recorded[bacteria_idx] = false;

            individual.diagnostic_cascade_open[bacteria_idx] = true;
            individual.diagnostic_cascade_entry_time_step[bacteria_idx] = entry_step as i32;
            individual.diagnostic_cascade_entry_hospitalized[bacteria_idx] = false;
            individual.diagnostic_cascade_bacterial_identification_recorded[bacteria_idx] = false;
            individual.diagnostic_cascade_resistance_testing_recorded[bacteria_idx] = false;
            individual.diagnostic_cascade_targeted_treatment_recorded[bacteria_idx] = false;
        ...
        fn clear_rolling_fields(summary: &mut super::TimeStepSummary) {
            summary.deaths_past_year = 0;
            summary.deaths_background_past_year = 0;
            summary.deaths_sepsis_past_year = 0;
            summary.deaths_infection_non_sepsis_past_year = 0;
            summary.deaths_drug_toxicity_past_year = 0;
            summary.newly_infected_past_year = 0;
        }

        let boundary_step = ((2022.0 - SIMULATION_START_YEAR) * DAYS_PER_YEAR).round() as usize;
        let first_step = boundary_step - 1;
        let end_step = boundary_step + 1;
        let seed = Some(7_315_337_u64);
        let mut partial = Simulation::new(8, end_step, false, seed, CalibrationMode::Partial);
        let mut full = Simulation::new(8, end_step, false, seed, CalibrationMode::Full);
        seed_boundary_spanning_episode(&mut partial, first_step);
        seed_boundary_spanning_episode(&mut full, first_step);

        assert_eq!(
            bincode::serialize(&partial.create_branch_snapshot()).unwrap(),
            bincode::serialize(&full.create_branch_snapshot()).unwrap(),
            "test fixtures must start with identical model state"
        );

        partial
            .run_from(first_step, None)
            .expect("partial boundary run should succeed");
        full.run_from(first_step, None)
            .expect("full boundary run should succeed");

        assert_eq!(
            bincode::serialize(&partial.create_branch_snapshot()).unwrap(),
            bincode::serialize(&full.create_branch_snapshot()).unwrap(),
            "discarding summary work must not alter population or mechanism-cache state"
        );
        assert_eq!(partial.summary_log.len(), 2);
        assert_eq!(full.summary_log.len(), 1);

        let mut expected = partial
            .summary_log
            .iter()
            .find(|summary| summary.time_step == boundary_step)
            .expect("partial mode should retain the boundary row")
            .clone();
        expected.apply_content_flags(SummaryContentFlags::calibration_full());
        let mut actual = full.summary_log[0].clone();
        // Partial mode includes the preceding discarded day in its rolling window; the daily
        // fields and all model state should otherwise be identical.
        clear_rolling_fields(&mut expected);
        clear_rolling_fields(&mut actual);
        assert_eq!(
    ...
    fn assert_death_day_diagnostic_cascade_counts(
        ...
        person_drug_date_is_today: bool,
...

        {
            ...
            individual.date_drug_initiated[drug_idx] = if person_drug_date_is_today {
                time_step as i32
            } else {
                time_step as i32 - 1
            };
    ...
    #[test]
    fn same_day_person_drug_date_without_start_event_does_not_record_targeted_treatment() {
        assert_death_day_diagnostic_cascade_counts(true, true, [1, 1, 0, 0, 0]);
    }

    #[test]
    fn targeted_course_start_event_preserves_the_selection_time_bacteria_snapshot() {
        let source_bacteria_idx = BACTERIA_LIST
            .iter()
            .position(|&name| name == "klebsiella_pneumoniae")
            .expect("K. pneumoniae must be modelled");
        let later_identified_bacteria_idx = BACTERIA_LIST
            .iter()
            .position(|&name| name == "escherichia_coli")
            .expect("E. coli must be modelled");
        let drug_idx = 0;
        let rule_events = RuleEvents {
            targeted_course_start: Some(TargetedCourseStartEvent::from_identified_bacteria(
                drug_idx,
                &[source_bacteria_idx],
            )),
            ..RuleEvents::default()
        ...
        assert!(targeted_course_started_for_bacterium(
            &rule_events,
            source_bacteria_idx
        ));
        assert!(!targeted_course_started_for_bacterium(
            &rule_events,
            later_identified_bacteria_idx
        ));

        let mut rng = SmallRng::seed_from_u64(73_150);
        let mut individual = Individual::new(1, 40 * 365, "female".to_string(), &mut rng);
        individual.diagnostic_cascade_bacterial_identification_recorded
            [later_identified_bacteria_idx] = true;
        individual.test_identified_infection[later_identified_bacteria_idx] = true;
        individual.cur_use_drug[drug_idx] = true;
        individual.drug_use_context[drug_idx] = AntibioticUseContext::Targeted;
        individual.resistances[later_identified_bacteria_idx][drug_idx].activity_r =
            store_float(1.0);

        // Identification after prescribing must not add the bacterium to the immutable event.
        assert!(!diagnostic_targeted_treatment_recordable(
            &individual,
            &rule_events,
            later_identified_bacteria_idx
        ));
        assert!(!diagnostic_effective_targeted_recordable(
            &individual,
            later_identified_bacteria_idx
        ));
    }
    ...
    #[test]
    fn targeted_course_start_event_can_cover_multiple_selection_time_bacteria() {
        let first_bacteria_idx = 0;
        let second_bacteria_idx = 1;
        let rule_events = RuleEvents {
            targeted_course_start: Some(TargetedCourseStartEvent::from_identified_bacteria(
                0,
                &[first_bacteria_idx, second_bacteria_idx],
            )),
            ..RuleEvents::default()
        };
    ...
    #[test]
    fn genuine_targeted_course_start_records_targeted_and_effective_stages_once() {
        let time_step = 33_580;
        let bacteria_idx = BACTERIA_LIST
            .iter()
            .position(|&name| name == "escherichia_coli")
            .expect("E. coli must be modelled");
        let drug_idx = 0;
        let mut rng = SmallRng::seed_from_u64(73_175);
        let mut individual = Individual::new(1, 40 * 365, "female".to_string(), &mut rng);
        individual.diagnostic_cascade_entry_time_step[bacteria_idx] = time_step as i32;
        ...
        let mut stage_counts = vec![1, 1, 0, 0, 0];
        let mut stage_counts_by_setting =
            vec![0; DIAGNOSTIC_CASCADE_STAGE_COUNT * DIAGNOSTIC_CASCADE_SETTING_COUNT];
        stage_counts_by_setting[diagnostic_cascade_stage_setting_index(
            DIAGNOSTIC_CASCADE_ELIGIBLE_IDX,
            DIAGNOSTIC_CASCADE_COMMUNITY_IDX,
        )] = 1;
        stage_counts_by_setting[diagnostic_cascade_stage_setting_index(
        ...
        for _ in 0..2 {
            if diagnostic_targeted_treatment_recordable(&individual, &rule_events, bacteria_idx) {
                record_diagnostic_cascade_stage(
                    0,
                    time_step,
                    &individual,
        ...
        assert_eq!(stage_counts, vec![1, 1, 0, 1, 1]);
    ...
    #[test]
    fn effective_stage_retains_regimen_level_activity_contract() {
        let mut rng = SmallRng::seed_from_u64(73_190);
        let mut individual = Individual::new(1, 40 * 365, "female".to_string(), &mut rng);
        let bacteria_idx = BACTERIA_LIST
            .iter()
            .position(|&name| name == "escherichia_coli")
            .expect("E. coli must be modelled");
        let attributed_start_drug_idx = 0;
        let different_effective_drug_idx = 1;

        individual.diagnostic_cascade_bacterial_identification_recorded[bacteria_idx] = true;
        individual.diagnostic_cascade_targeted_treatment_recorded[bacteria_idx] = true;
        individual.cur_use_drug[attributed_start_drug_idx] = false;
        individual.cur_use_drug[different_effective_drug_idx] = true;
        individual.drug_use_context[different_effective_drug_idx] = AntibioticUseContext::Targeted;
        individual.resistances[bacteria_idx][different_effective_drug_idx].activity_r =
            store_float(1.0);

        // The targeted-start stage is bacterium-attributed, but later effectiveness remains
        // a regimen-level exposure measure and need not come from the start-event drug.
        assert!(diagnostic_effective_targeted_recordable(
            &individual,
            bacteria_idx
        ));
