...
class InfectionJourneyAnalyzer:
    def __init__(self, csv_file=None, output_dir=None, verbose=True):
        """Initialize the analyzer with the journey data."""
        base_dir = Path(__file__).resolve().parent
        data_path = Path(csv_file) if csv_file is not None else base_dir / "infection_journeys.csv"
        self.output_dir = Path(output_dir) if output_dir is not None else base_dir / "infection_journey_analysis_outputs"

        self.verbose = verbose
        self.plot_counter = 0  # Counter for sequential plot numbering
        if self.verbose:
            print(f"Loading infection journey data from {data_path}...")
    ...
    def view_individual_journey(self, journey_id=None, individual_id=None, sequential_number=None, highlight_de_novo=False, highlight_sepsis_death=False):
        ...
        penicillin_allergy_text = "penicillin allergy flagged: not recorded"
        if 'perceived_penicillin_allergy' in journey_data.columns:
            raw_allergy_value = journey_data['perceived_penicillin_allergy'].iloc[0]
            if isinstance(raw_allergy_value, str):
                has_allergy = raw_allergy_value.strip().lower() in {"true", "1", "yes"}
            else:
                has_allergy = bool(raw_allergy_value)
            penicillin_allergy_text = f"penicillin allergy flagged: {'yes' if has_allergy else 'no'}"
    ...
    def generate_text_report(self):
        ...
        filename = self.output_dir / "infection_journeys_detailed_report.txt"
