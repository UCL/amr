...
Version 1 is deliberately behavior-neutral. The production calibration loader still reads the wide
matrices, while tests require the long-form file to reproduce every value, missing cell, and static
score-eligibility decision exactly.
...
matrices. `include_in_score` records target-side v1 eligibility after the existing target,
organism, and rifampicin exclusions. Model-potency filters and unavailable simulation denominators
can still exclude a row from a particular analysis. `evidence_weight` remains blank because evidence
quality has not yet been assessed; `score_row_weight` records equal target-side row weighting only.
