...
**Implementation order within a simulated day.** In the live `apply_rules()` implementation, the ordering is: age/living-state checks; reset daily microbiome flags; immunodeficiency onset/recovery; hospital admission or discharge; travel; sepsis onset or clearance for currently active infections; birth-cohort vaccination; antibiotic stopping, pharmacokinetic decay, and new drug initiation/selection; toxicity and treatment-failure/restart checks; mortality and sepsis recovery; then the per-bacterium loop for new acquisition, microbiome acquisition/clearance, resistance emergence/reversion/floors, infection growth, symptoms, clearance, testing, and finally HGT. This order matters: sepsis onset is evaluated before new infections acquired later that same day, and new acquisition is evaluated separately for each bacterium rather than skipped because the person already has another active infection.
...
The pathway-level inheritance probability is also scaled by the run-level `run_pathway_carrier_inheritance_multiplier`. If the pathway fires, only mechanisms already present in the person's carriage compartment are eligible, and copied mechanisms are written into `mechanism_any` before infection resistance is re-derived. Operationally, the code does not require a separate "endogenous acquisition" flag: this bridge is evaluated after any new infection is established, provided the person already carries the same organism in `presence_microbiome`. It is distinct from profile-cache sampling during acquisition (Section 3.4), because it copies mechanisms from the person's own carriage compartment rather than importing them from the regional community or hospital profile pool.
...
After every simulated day, the model refreshes a **profile reservoir** (`MechanismCache`) of up to 1000 complete resistance genotypes per combination of region × care setting (community / hospital) × bacteria.  The reservoir is built by **reservoir sampling** from all currently infected individuals, so every infected person has an equal probability of contributing a profile — including fully susceptible individuals. Infected individuals contribute to the pool corresponding to their **current location** (region, hospital or community) at the time of the daily cache update.

The reservoir blends retained profiles with newly collected profiles each day. In the live configuration, both `community_profile_cache_retention` and `hospital_profile_cache_retention` are 0.999, corresponding to a half-life of roughly 693 days. Hospital slots have one additional guard: if a hospital slot previously contained any resistant profile, the refresh step preserves one resistant exemplar rather than allowing that slot to become fully susceptible purely because of stochastic retention and refill. This gives the profile cache enough memory to represent persistent circulating strain ecology while still being refreshed by the current infected population.

Because the reservoir includes both resistant and susceptible profiles, the **prevalence** of resistance to any given drug can be computed directly by scanning the reservoir: for each profile, the framework checks whether any mechanism applicable to that drug is set, and the resistant fraction across all stored profiles gives the current prevalence estimate.  The profile reservoir is the source of both sampling and derived prevalence. This prevalence is used downstream by antibiotic prescribing logic (Section 6).

**Hospital enrichment levers.** Hospital-acquired active infections can be enriched through two profile-sampling levers. `hospital_resistance_prune_susceptible_percent` temporarily removes a configured fraction of mechanism-free profiles from the hospital candidate pool before sampling; resistant profiles are never pruned. `hospital_resistance_concentration_factor` optionally adds an additional weight of $f^k$, where $k$ is the number of mechanisms in a profile. In the current live configuration, the concentration factors are neutral (`f = 1.0`) and hospital enrichment is mainly carried by susceptible-profile pruning (default 50%, with higher organism-specific values for classic nosocomial pathogens). These levers change which already-observed genotype is sampled; they do not create synthetic combinations of mechanisms.

**Step 2 — Community dilution**

Clinical samples tend to over-represent resistant strains because susceptible infections are more likely to resolve quickly, generate less urgent microbiology, and be under-sampled in surveillance systems. To account for the fact that community bacteria are less resistant than those seen in clinics, the model applies a per-bacteria **community resistance dilution factor** (`community_resistance_dilution_factor`). A random draw determines whether the infection is sampled from the human circulating profile reservoir. If not, profile-cache sampling is skipped and only the exogenous floor path below can add resistance mechanisms. This dilution step is only applied to community-acquired infections; hospital-acquired infections use a human-reservoir probability of 1.0.
...
Rather than independently sampling each mechanism from a marginal prevalence estimate, the framework draws a **complete genotype profile** from the profile reservoir described in Step 1, assigning all mechanisms set in that profile to the newly infected individual simultaneously. The result is that newly acquired *E. coli*, for example, arrives with a resistance profile that mirrors an actual circulating strain — e.g., ESBL CTX-M together with fluoroquinolone resistance, as these co-occur on real plasmids (Partridge SR et al., 2018).

**Community-acquired infections** that pass the Step 2 dilution gate — meaning the model treats them as arising from the human circulating reservoir rather than from a wholly exogenous environmental exposure — draw a profile uniformly at random from the community reservoir for the relevant region and bacterium. Because the reservoir already includes susceptible as well as resistant profiles, this uniform draw reproduces the resistance prevalence already present in the community cache after the ecological dilution step has done its work. In the very early warm-up phase, if the requested community slot is still empty, the code temporarily falls back to the other stratum rather than inventing a synthetic profile.

**Hospital-acquired enriched sampling.** Hospital-acquired active infections use `sample_profile_hospital_enriched()` whenever either the concentration factor is above 1.0 or susceptible-profile pruning is configured. Before a hospital slot has ever accumulated a resistant genotype for that region and bacterium, the helper samples from a pooled hospital-plus-community regional reservoir, so early hospital acquisitions can bootstrap from resistant genotypes already circulating locally. Once the hospital slot contains a resistant profile of its own, sampling reverts to the hospital slot alone. The helper then optionally prunes all-zero profiles and optionally applies the $f^k$ weighting described above. If pruning would remove every candidate, the unpruned pool is used instead. The sampled profile is therefore always an existing stored profile.

Carriage acquisition (Section 8.2) is related but not identical. Hospitalized carriage acquisitions can use the same $f^k$ weighted sampler when `hospital_resistance_concentration_factor > 1`, but they do not use the active-infection-specific susceptible-profile pruning helper. Carriage also has its own acquisition gate, `microbiome_resistance_multiplier_on_acquisition`, plus hospital and community dilution modifiers.
...
*M. tuberculosis* is defined as multi-drug-resistant (MDR) by its rifampicin resistance — any isolate labelled MDR-TB will already carry a rifampicin-resistance mutation at the time of diagnosis (WHO, 2020). The model reflects this definitional requirement with a dedicated post-sampling step. For all MDR-TB acquisitions occurring in or after 1966 (when rifampicin was introduced), the current code hard-sets every resistance mechanism applicable to rifampicin for that bacterium — specifically the `rpoB` RNA-polymerase mutation — in both `mechanism_any` and `mechanism_majority`, regardless of what the profile-cache step assigned. `propagate_mechanism_resistance()` is then called again in raise-only mode so that the rifampicin `any_r` is updated to reflect this guaranteed mechanism. The acquisition provenance for rifampicin is stamped with `ResistanceAcquisitionType::AtInfectionTB` rather than `AtInfectionCommunity`, which allows the model to track this pathway separately in output statistics. The parameter `mdr_mycobacterium_tuberculosis_guaranteed_rifampicin_resistance` is currently used as an on/off gate: any positive value enables the hard-set rule, so the live default 0.90 behaves as "enabled" rather than as a 90% Bernoulli probability.

Note: after these six steps, the carrier-inheritance bridge described in Section 3.3 may apply additional mechanisms from `mechanism_microbiome` if the individual is already a carrier of the same organism. That step is separate from regional profile-cache acquisition: it is evaluated for any newly established same-organism infection with pre-existing carriage and is governed by `carrier_resistance_inheritance_probability`, `run_pathway_carrier_inheritance_multiplier`, and the per-mechanism `infection_from_microbiome_dampening` transfer probability.

#### Resistance appearance and expression summary

The three mechanism bitmasks have different meanings. `mechanism_any` is the active-infection mechanism set and is the source of `any_r`. `mechanism_majority` is the subset treated as the dominant/circulating strain; it is what active infections contribute back into the `MechanismCache`, and it also affects HGT donor strength. `mechanism_microbiome` is the carriage compartment and is the source of `microbiome_r`. Mechanism acquisition, majority evolution, and phenotype propagation are therefore separate operations.

| Pathway | When it runs | State changed | Interpretation |
|---------|--------------|---------------|----------------|
| Human-reservoir profile sampling | New infection passes the human-reservoir draw | Sets `mechanism_any` and `mechanism_majority` from a stored profile | Acquisition of an already-circulating strain genotype |
| Hospital-enriched profile sampling | Hospital-acquired active infection with pruning or weighting configured | Sets `mechanism_any` and `mechanism_majority` from an enriched stored profile | Oversamples resistant ward genotypes through bootstrap, pruning, and optional $f^k$ weighting |
| Community dilution | Community-acquired infection before profile sampling | Gates whether profile sampling runs | A failed draw does not itself add resistance; it sends the acquisition to the exogenous/no-profile path |
| Static environmental floors | Community acquisition not sampled from the human reservoir | Sets `mechanism_any` and `mechanism_majority` per mechanism | Explicit exogenous reseeding for configured bacteria-mechanism pairs |
| Dynamic ratchet floors | Same exogenous path, for low-reversion mechanisms | Sets `mechanism_any` and `mechanism_majority` per mechanism | Prevents historically established low-cost mechanisms from collapsing below stepped peak-prevalence floors |
| Universal extinction guard | Every new infection, per introduced drug class | Sets one applicable already-emerged mechanism in `mechanism_any` and `mechanism_majority` | Weak 1% guard against stochastic extinction, not a source of novel mechanisms |
...
| Same-person microbiome inheritance | New infection when the person already carries the same organism | Copies eligible `mechanism_microbiome` bits into `mechanism_any` only | Carrier-to-infection bridge; copied mechanisms start as active-infection minority mechanisms |
| De novo infection emergence | Existing active infection under drug pressure | Adds new bits to `mechanism_any` | Within-host emergence under selection; majority status can follow later under drug pressure |
| Majority-strain evolution | Existing active infection with drug pressure against a minority mechanism | Promotes `mechanism_any` bits into `mechanism_majority` | Converts a minority resistant subpopulation into the dominant/circulating strain |
| Infection-microbiome transfer | Same organism present in both compartments and masks differ | Unions `mechanism_any` and `mechanism_microbiome` | Within-host spillover between active infection and carriage |
| HGT | After the per-bacterium loop, between co-present bacteria | Adds transferable donor mechanisms to recipient `mechanism_any`; also `mechanism_microbiome` if recipient carriage exists | Inter-species mechanism transfer, scaled by context and donor majority/minority status |
| Mechanism-to-drug propagation | After mechanism changes | Recomputes `any_r` and/or `microbiome_r` from mechanism flags | Phenotype derivation across every drug a mechanism affects; not a new acquisition event |
...
The helper `serious_resistance_marker_drugs` defines one or more sentinel drug-resistance results for each organism. These markers are used when interpreting serious-resistance burden and in admission logic once a relevant resistant test result is known.
...
| Immunodeficiency | +0.2 | weak positive trigger in isolation | In the current model, immunodeficiency alone is not usually a stand-alone treatment indication; symptoms, sepsis, test confirmation, and the constrained prophylaxis pool drive most starts |
...
**Immunodeficiency prophylaxis.** A separate constrained prophylaxis path is evaluated when a person has immunodeficiency but no symptomatic infection. It is not a general empiric-treatment pool. The only candidate drugs are `trim_sulf` (score 0.8), `azithromycin` (3.0), `ciprofloxacin` (2.0), and `levofloxacin` (1.5). Reserve drugs are excluded, broad-spectrum drugs are penalised, lower-spectrum drugs receive a modest bonus, and fluoroquinolone prophylaxis is blocked for people younger than 18 years. This rule is why the small positive initiation effect for immunodeficiency does not translate into broad untreated prophylaxis with the full antibiotic catalogue.
...
The exact empiric score templates are source-of-truth in `src/config.rs` under `empiric_syndrome_templates`. They change during calibration and are no longer duplicated here in full, because stale copied tables are more misleading than useful. The current UTI template is shown below because it is often referenced when interpreting drug-share outputs:
...
These are scores before availability, age, syndrome restrictions, regional access, resistance surveillance, targeted-therapy modifiers, reserve-drug gates, toxicity avoidance, and the temperature-scaled weighted draw are applied.
...
Reserve agents are gated separately. In targeted therapy, reserve drugs are strongly discouraged unless there is recent treatment failure or a severe hospitalised Gram-negative context. In empiric therapy, reserve use is stricter: recent failure or severe hospitalised Gram-negative disease must be present, and population resistance pressure can still block escalation. After this gate, the global reserve penalty is applied (`reserve_drug_score_penalty = 0.35` in the live default configuration).

Drug choice also remembers recent harm. If toxicity stops a drug, that drug receives an approximately 1000-fold avoidance penalty for the configured avoidance window. When targeted therapy starts, ineffective existing drugs are stopped if they have no activity against any identified active infection. In non-severe empiric starts, a new empiric drug generally replaces existing empiric therapy rather than stacking indefinitely; septic patients can still retain combination therapy.
...
There are 46 mechanisms × 39 drug classes = 1,794 individual values. The table below shows the **current global default** multiplier for the major mechanisms discussed most often in the text (used when a specific per-class value has not been configured):
...
| `run_pathway_infection_de_novo_multiplier` / `counterfactual_resistance_multiplier` | Run-level and scenario-level scaling applied on top of the organism-specific baseline | Allows calibrated pathway-wide or policy-scenario changes without rebuilding the mechanism table |
...
The microbiome pathway is simpler. While on antibiotic exposure, microbiome emergence uses the same organism-mechanism baseline table and applies `run_pathway_microbiome_de_novo_multiplier` (multiplied by the compatibility scalar `microbiome_de_novo_multiplier`, currently 1.0) and counterfactual scaling, but it does not use the infection-burden or drug-window terms described above.
...
Since fitness costs mean resistant bacteria often replicate more slowly than susceptible competitors in the absence of antibiotic pressure (Andersson DI et al., 2010), resistance can gradually decline when drug use is reduced. The model assigns each mechanism a daily **reversion rate** — the probability of losing resistance per day when no antibiotic pressure is present. Higher rates mean the mechanism is "expensive" and lost quickly; lower rates mean it is nearly cost-free and persists indefinitely. In the live rules engine, per-mechanism reversion rates are scaled by `run_pathway_reversion_rate_multiplier` (default 1.0), with additional community-specific reversion multipliers for some bacteria.
...
*Note: The system reserves one remaining placeholder variable (`as_yet_unknown`, baseline rate `0.001`) designated for future empirical calibration. `mutation_pbp_mosaic` has been activated as **PBP mosaic mutations** (chromosomal target modification affecting penicillins, cephalosporins, aztreonam, and aztreonam-avibactam — NOT carbapenems), and `efflux_mtr_cde` as **mtrCDE-type broad efflux** (chromosomal efflux affecting macrolides, penicillins, tetracyclines, and chloramphenicol). Neither is HGT-transferable.*
...
**Motivation.** Even with long profile-cache retention, a finite stochastic profile reservoir can still under-represent historically established resistance after current selection pressure falls. The ratchet addresses a different problem from the environmental floor: it represents mechanisms that have already become established in the human circulating pool and remain there because the biological cost of carrying them is low, or because effective reversion is very weak.
...
1. **Ratchet:** use when a mechanism plausibly built up historically under human selection, has sufficiently low effective fitness cost, and should remain common once established.
...
4. **Co-selection / linked-package enrichment:** use when the missing feature is not the amount of resistance generated, but the fact that mechanisms tend to arrive together on lineages, integrons, cassettes, or plasmids. The recent *S. aureus* acquisition-side package completion logic is the template for this class of solution.

In the current rifampicin review we deliberately stop short of implementing lever 3 for non-target gut and enteric organisms. The model's policy focus is on comparisons in prescribing, diagnostics, and access for the explicitly modelled infections; for most of those comparisons, failure to reconstruct the full background rifampicin exposure generated by TB treatment is unlikely to be decision-changing. We therefore treat under-reproduction of some rifampicin resistance prevalences outside directly treated indications as an accepted limitation unless a future use case specifically depends on those pathways.

**Which lever to prefer.** The intended order of thought is: first ask whether the mechanism is simply a historically sticky low-cost background (ratchet); if not, ask whether it is repeatedly imported from outside the modelled human pool (environmental floor); if not, ask whether the model is missing a real source of selection pressure (additional off-model drug use); and only then ask whether the main problem is missing correlation structure between mechanisms (co-selection or lineage/package completion). This keeps the model aligned with its purpose as a policy model rather than pushing it toward unnecessary molecular detail.
...
| Mechanism-level reversion | `run_pathway_reversion_rate_multiplier` | 1.0 | Per-mechanism reversion operates in the microbiome compartment using the same rates and pathway multiplier as in the infection compartment (Section 7.4). Each mechanism can only revert when no selecting antibiotic is present. |
| De-novo emergence under treatment | `run_pathway_microbiome_de_novo_multiplier` | 1.0 | When antibiotics exert selective pressure on carried bacteria, resistance mechanisms can emerge in the microbiome via the microbiome emergence pathway (Section 7.3), scaled by this multiplier and the compatibility scalar `microbiome_de_novo_multiplier`. Emergence writes directly to `mechanism_microbiome`; transfer into infection occurs through the separate bridge pathways below. |
...
| HGT into the microbiome | (see Section 9) | — | When a horizontal gene transfer event fires and the recipient bacterium is also present in the microbiome, the transferred mechanism is written to `mechanism_microbiome` as well as `mechanism_any` |
...
Each day, after the per-bacterium updates, the model evaluates potential gene transfer events among bacteria co-present in the same person, whether as active infection, microbiome carriage, or both. The model evaluates HGT dynamically per distinct resistance mechanism, allowing independent plasmids (e.g., KPC and *mcr-1*) to transmit independently rather than as a single all-or-nothing block. Furthermore, bacteria do not restrict plasmid donation to only the dominant strain; minority resistance populations can donate, but face a transfer penalty.

When an HGT event fires, the transferred mechanism is written to the recipient bacterium's `mechanism_any` (infection compartment). If that recipient bacterium is also present in the microbiome, the mechanism is simultaneously written to `mechanism_microbiome`, ensuring the carriage reservoir stays consistent with the infection compartment. All HGT rates are scaled by the run-level `run_pathway_hgt_multiplier` (default 1.0) and by `counterfactual_resistance_multiplier`.
...
|-----------|----------------|-------------------|
| *S. aureus* | −7.4 | Aggressive bloodstream pathogen; 20–30% mortality in bacteraemia (Tong SYC et al., 2015) |
| *P. aeruginosa* | −5.9 | High mortality in ICU infections; often in immunocompromised hosts (Bassetti M et al., 2018) |
| *S. agalactiae* | −6.4 | Neonatal and pregnancy-associated sepsis (Seale AC et al., 2010) |
| *S. pyogenes* | −5.9 | Invasive GAS disease including necrotising fasciitis and toxic shock; STSS case-fatality 30–70% places invasive GAS among the most lethal bacterial syndromes (Carapetis JR et al., 2005) |
| *N. meningitidis* | −7.3 | Meningococcal disease; rapid sepsis progression with purpura fulminans and DIC (Stephens DS et al., 2007) |
| *E. faecium* | −6.4 | Hospital-acquired bloodstream infections, especially VRE |
| *K. pneumoniae* | −6.9 | Gram-negative sepsis; carbapenem-resistant strains carry >40% mortality (Xu L et al., 2017) |
| *E. faecalis* | −6.9 | Endocarditis and line-related bacteraemia |
| *E. coli* | −8.9 | Most common Gram-negative bloodstream isolate; UTI-source sepsis usually less severe (Poolman JT et al., 2016) |
| *S. Paratyphi A* | −8.4 | Enteric fever with occasional septic complications |
| *iNTS* | −8.6 | Invasive non-typhoidal salmonellosis; high mortality in sub-Saharan Africa (Stanaway JD et al., 2017) |
| *Y. enterocolitica* | −8.9 | Rare sepsis, mainly in iron-overload or immunosuppressed patients |
| *C. trachomatis* | −18.4 | STI — essentially never causes sepsis |
| *N. gonorrhoeae* | −23.4 | Disseminated gonococcal infection is exceedingly rare |
| *H. pylori* | −250.0 | Gastric pathogen — deaths from cancer, not sepsis; the implementation also forces zero sepsis risk when this is the sole active infection |
| Fallback default | −12.0 | Used for any organism without an explicit override |
...
The primary analytical goal of the initial model application is to estimate the number of deaths attributable to antimicrobial resistance. This is achieved by running a **counterfactual experiment**: a resistance-suppressed branch is simulated in parallel with the observed (baseline) trajectory over the same period, and the difference in mortality between the two branches provides an estimate of the burden caused by resistance itself.
...
| **Counterfactual** | A hypothetical world in which resistance is removed at the branch point. Resistance is wiped from all active infections and microbiome carriage (`clear_all_resistance_on_branch_start = true`), the mechanism cache is reset, and `counterfactual_resistance_multiplier` is set to 0.0. In the current rules this blocks profile-sampled resistance, de novo emergence, microbiome-acquisition seeding, and HGT transfer; acquisition-time floors and the MDR-TB rifampicin rule are separate pathways described in Section 3.4. |
...
| microbiome_de_novo_multiplier | 1 compatibility scalar multiplied into the microbiome de novo pathway |
...
- scalar columns for population state, mortality, treatment, policy branch, and calendar time
