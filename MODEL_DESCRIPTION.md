...
After every simulated day, the model refreshes a **stored library of circulating resistance profiles** (`MechanismCache` in the code) containing up to 1000 complete resistance profiles for each combination of region × care setting (community / hospital) × bacterium. The library is built by randomly retaining a sample of profiles from all currently infected people, so every infected person has an equal chance of contributing a profile — including fully susceptible individuals. Each person contributes to the group corresponding to their **current location** (region, hospital or community) at the time of the daily update.

The stored profile library blends retained profiles with newly collected profiles each day. In the live configuration, both `community_profile_cache_retention` and `hospital_profile_cache_retention` are 0.999, corresponding to a half-life of roughly 693 days. Hospital profile groups have one additional guard: if a hospital group previously contained any resistant profile, the refresh step preserves one resistant example rather than allowing that group to become fully susceptible purely because of chance loss during retention and refill. This gives the stored profile library enough memory to represent persistent circulating strain ecology while still being refreshed by the currently infected population.
...
Clinical samples tend to over-represent resistant strains because susceptible infections are more likely to resolve quickly, generate less urgent microbiology, and be under-sampled in surveillance systems. To account for the fact that community bacteria are less resistant than those seen in clinics, the model applies a per-bacterium **community resistance dilution factor** (`community_resistance_dilution_factor`). A random probability check determines whether the infection is treated as coming from the human circulating profile library. If not, sampling from that library is skipped and only the exogenous floor pathway described below can add resistance mechanisms. This dilution step is only applied to community-acquired infections; hospital-acquired infections are always treated as drawing from a human-associated circulating pool.
...
| Dynamic ratchet floors | Same exogenous path, for low-reversion mechanisms | Records one or more mechanisms as present in `mechanism_any` and `mechanism_majority` | Prevents historically established low-cost mechanisms from collapsing below stepped peak-prevalence floors |
| Universal extinction guard | Every new infection, per introduced drug class | Records one already-emerged mechanism as present in `mechanism_any` and `mechanism_majority` | Weak 1% guard against stochastic extinction, not a source of novel mechanisms |
...
**Mechanism-centred architecture.** For each person and bacterium, the model records which resistance mechanisms are present in the active infection, the dominant strain, and asymptomatic carriage. Drug-level resistance is then calculated from those mechanisms. In the code these three records are `mechanism_any`, `mechanism_majority`, and `mechanism_microbiome`, while the derived summary resistance measures are `any_r` and `microbiome_r`. A single stored profile library (`MechanismCache` in the code) holds up to 1000 complete clinical resistance profiles per region × care setting × bacterium for use in profile-based acquisition (Section 3.4). This library keeps separate hospital and community collections, retains profiles over time, preserves one resistant example in hospital groups that have previously contained resistance, derives prevalence directly from the stored profiles, applies per-bacterium hospital resistance concentration factors for organisms with strong nosocomial ecology, and maintains a `peak_mechanism_prevalence` table (see Section 7.8) that records the highest marginal prevalence ever reached for each bacteria-mechanism pair so that low-cost resistance can be sustained by the dynamic ratchet.
...
The microbiome pathway is simpler. While on antibiotic exposure, microbiome emergence uses the same organism-mechanism baseline table and applies `run_pathway_microbiome_de_novo_multiplier` (multiplied by the compatibility multiplier `microbiome_de_novo_multiplier`, currently 1.0) and counterfactual scaling, but it does not use the infection-burden or drug-window terms described above.
...
2. **Random probability check — does this acquisition receive the guard?** With `max_resistance_level = 1.0`, the current implementation is simply a 1% random probability check. Around 1% of acquisitions enter the mechanism-assignment path.

3. **Select a mechanism — subject to the causal correctness guard.** The code iterates through applicable resistance mechanisms for that drug class and organism. A candidate is only eligible if it has already emerged somewhere in the global stored profile library (`MechanismCache` in the code); otherwise it is skipped. Eligible candidates are then assigned with probability `mechanism_assignment_probability_on_any_r_gain = 0.8`, and one mechanism is sufficient.

4. **Derive `any_r` from the mechanism.** The extinction guard does not inject a drug-level resistance value directly. Instead, `propagate_mechanism_resistance()` converts the assigned mechanism into per-drug `any_r` using the same enhancement multipliers as every other resistance pathway.

**Design principle — the guard is deliberately weak.** The 1% value is intended only to prevent rare resistance mechanisms that have already entered global circulation from disappearing by chance in a finite simulation. It is far too small to reproduce the observed prevalences of common resistance phenotypes by itself. Larger persistence effects are handled elsewhere: through transmission of previously observed resistance profiles, explicit environmental floors, and the ratchet.
...
Since resistance to one agent in a class typically confers resistance to related agents — as when an ESBL in *E. coli* hydrolysing ceftriaxone also destroys cefazolin, cefuroxime, and unprotected penicillins through the same β-lactam ring cleavage — the model captures this by defining **cross-resistance groups**: bacteria-specific phenotype bundles applied at the level of the summary resistance measure `any_r`. In practice, once one drug in a configured group has a non-zero `any_r`, the model raises the other drugs in that group to the same group-maximum `any_r` for that bacterium. This layer therefore equalises the phenotype across related agents, but it does not force the underlying recorded mechanisms themselves to be acquired or lost in lockstep.
...
**Implementation.** Each simulated day, after updating the stored profile library, the model scans all stored profiles across all regions and both community and hospital strata to compute the current marginal prevalence of each (bacteria, mechanism) pair. The `peak_mechanism_prevalence[bacteria][mechanism]` table is then updated upward-only: once a peak has been reached, it is not allowed to fall.
...
| De-novo emergence under treatment | `run_pathway_microbiome_de_novo_multiplier` | 1.0 | When antibiotics exert selective pressure on carried bacteria, resistance mechanisms can emerge in the microbiome via the microbiome emergence pathway (Section 7.3), scaled by this multiplier and the compatibility multiplier `microbiome_de_novo_multiplier`. Emergence writes directly to `mechanism_microbiome`; transfer into infection occurs through the separate bridge pathways below. |
...
| **Counterfactual** | A hypothetical world in which resistance is removed at the branch point. Resistance is wiped from all active infections and microbiome carriage (`clear_all_resistance_on_branch_start = true`), the stored profile library is reset, and `counterfactual_resistance_multiplier` is set to 0.0. In the current rules this blocks resistance acquired from previously observed circulating profiles, de novo emergence, microbiome-acquisition seeding, and HGT transfer; acquisition-time floors and the MDR-TB rifampicin rule are separate pathways described in Section 3.4. |
