...
**Causal correctness guard.** Before the floor can assign a mechanism to a newly-infected individual, the model checks that the mechanism has already emerged somewhere in the simulation — specifically, that at least one currently stored profile in the `MechanismCache` (across all regions and both community and hospital strata) carries that mechanism's bitmask bit. This prevents the floor from introducing a resistance mechanism before any de novo emergence event has ever fired, ensuring the floor acts strictly as a *propagation amplifier* of an already-circulating strain pool rather than an origin of novel resistance. In practice the guard is dormant: once de novo emergence has seeded a mechanism into the cache, the guard permanently passes for that organism–mechanism pair for the remainder of the simulation.

**Current status.** The master switch `resistance_floor_feature_enabled` is set to 1.0 (on), but all per-organism `bacteria_{name}_resistance_floor_enabled` flags are currently **0.0 (off)**. Resistance floors are therefore globally inactive in Calibration A (see Section 7.5 for the planned Calibration B use).
...
**Design principle.** Resistance floors are appropriate only for *acquired* resistance. In the model, "resistance" always means acquired resistance; intrinsic non-susceptibilities are encoded as near-zero `potency_when_no_r` values and are therefore not subject to floor logic. The floor addresses a specific stochastic artefact: at 1,000,000-person scale, organisms with low infection counts may have too few concurrent active cases to maintain a continuously populated profile cache, causing resistance to stochastically collapse to zero even when globally-circulating resistant strains would always be present at some prevalence. The floor prevents this extinction artefact by guaranteeing that the minimum probability of acquiring a resistant strain at infection never falls below a specified level — provided that the mechanism has already emerged somewhere in the simulation (see causal correctness guard in Section 5.1, Step 4).

Floors do **not** model any specific real-world mechanism (importation, clonal spread, or environmental reservoir) explicitly. They represent a methodological acknowledgement that the simulated finite population is sampling from a larger global strain pool which the model cannot fully represent at this scale. The floor value for a given drug class should therefore be interpreted as the minimum resistance prevalence consistent with globally circulating strains of that organism at the relevant point in the antibiotic era.

**Calibration A — all floors disabled.** In the primary calibration, the master switch is on (`resistance_floor_feature_enabled = 1.0`) but all per-organism flags are **0.0 (off)**. Resistance is generated entirely bottom-up through de novo emergence, treatment selection, HGT, and profile-cache propagation. This calibration is appropriate when the simulated population is large enough to sustain resistance through the standard pathway for all modelled organisms.

**Calibration B — floors enabled for all bacteria (planned).** A second independent calibration is planned in which `bacteria_{name}_resistance_floor_enabled` is set to 1.0 for all organisms. In this calibration, per-drug-class floor targets and ramp periods are set to empirically grounded values reflecting globally observed resistance prevalence at the modern end of the simulation. Resistance emergence rates are then recalibrated against the same calibration targets as Calibration A, but with the floors providing a stable lower bound that prevents stochastic extinction. The two calibrations are expected to produce different predictions for the effect of antibiotic stewardship interventions: Calibration A predicts faster resistance decline when drug use is reduced (emergence-driven dynamics), while Calibration B predicts slower decline with an inertial floor (consistent with the literature showing resistance persists long after stewardship interventions). Running policy scenarios through both calibrations provides an honest uncertainty range on intervention efficacy.

**Stenotrophomonas maltophilia — no floor.** An earlier version of the model enforced floors for the L1/L2- and SmeDEF-driven intrinsic non-susceptibilities of this organism. These have been removed. *S. maltophilia* is constitutively non-susceptible to carbapenems, unprotected penicillins, 1st/2nd-generation cephalosporins, macrolides, and most aminoglycosides because every isolate encodes the L1 metallo-β-lactamase, L2 serine-β-lactamase, and SmeABC/SmeDEF efflux pumps chromosomally (Brooke JS, 2012; Crossman LC et al., 2008). This phenotype is now encoded directly as near-zero potency values (`potency_when_no_r` ≤ 0.05) for those drug classes, meaning clinicians in the simulation do not initiate those drugs for *Stenotrophomonas* infections, no treatment-selection pressure accumulates, and the output correctly shows these classes as simply not used — rather than artificially "resistant". Acquired resistance to TMP-SMX, fluoroquinolones, and tetracyclines (the drugs that are actually used) continues to emerge through the standard treatment-selection pathway. Floor values are pre-configured for *S. maltophilia* and will be activated in Calibration B.

**Helicobacter pylori — no floor in Calibration A.** *H. pylori* resistance to clarithromycin, metronidazole, fluoroquinolones, and amoxicillin is driven primarily by chromosomal mutation during treatment courses, not by circulating pre-resistant strains that seed infections independently of treatment history. In Calibration A the floor is disabled; resistance accumulates entirely through treatment-course selection. Incidental macrolide and fluoroquinolone exposure from courses prescribed for other indications (e.g. azithromycin for respiratory infections) also contributes, matching the well-documented link between community macrolide use and clarithromycin-resistant *H. pylori* prevalence (Savoldi A et al., 2018). Floor values are pre-configured and will be activated in Calibration B.

**Enterococcus faecium — pre-configured, disabled in Calibration A.** VRE clonal lineages (CC17) are globally disseminated hospital-adapted strains. At 1,000,000-person scale, the low total infection burden means the glycopeptide-resistance signal can stochastically collapse to zero between hospitalisation episodes. A moderate floor for glycopeptides (and related acquired drug-class resistances) prevents this artefact while still allowing acquired fluoroquinolone and macrolide resistance to fluctuate freely via treatment selection. The floor is pre-configured and will be activated in Calibration B.
