...
The pathway-level inheritance probability is also scaled by the run-level `run_pathway_carrier_inheritance_multiplier`. If the pathway fires, only mechanisms already present in the person's carriage compartment are eligible, and copied mechanisms are written into `mechanism_any` before infection resistance is re-derived. This is distinct from profile-cache sampling during acquisition (Section 3.4): it represents an endogenous infection arising from the person's own carried strain rather than importing a new strain from the regional community or hospital pool.



### 3.4 Resistance at acquisition

When a new infection is acquired in the community, the modelling framework determines whether (and, if so, what) resistance is present.  The framework uses a six-step process that reflects how resistance co-occurs in bacterial populations and how the simulation's policy branches interact with that process:
...
**Community-acquired infections** that pass the Step 2 dilution gate — meaning the model treats them as arising from the human or farm-animal associated circulating reservoir rather than from a wholly exogenous environmental exposure — draw a profile uniformly at random from the community reservoir for the relevant region and bacterium. Because the reservoir already includes susceptible as well as resistant profiles, this uniform draw reproduces the resistance prevalence already present in the community cache after the ecological dilution step has done its work. In the very early warm-up phase, if the requested community slot is still empty, the code temporarily falls back to the other stratum rather than inventing a synthetic profile.

**Hospital-acquired weighted sampling.** For hospital-acquired infections where the bacterium's `hospital_resistance_concentration_factor` $f > 1$, the model does not draw a uniform profile from the hospital pool. Instead it uses **weighted profile sampling** (`sample_profile_weighted`): each profile in the reservoir is assigned a sampling weight of $f^{k}$, where $k$ is the number of resistance mechanisms carried in that strain's profile. Profiles are then drawn with probability proportional to their weight. This means that more-resistant profiles (higher $k$) are over-sampled relative to susceptible profiles ($k = 0$, weight $= f^0 = 1$), reflecting the unmodelled ward-level cross-transmission, surface/device reservoirs, and HCW-mediated spread that an individual-based model cannot capture natively. Crucially, every sampled profile reflects one that has already been observed in the simulated population, so linked mechanisms on the same plasmid or lineage stay linked. In practice, a hospital-acquired *K. pneumoniae* infection with $f = 1.3$ and 8 distinct resistance mechanisms in its stored profile would receive a weight of $1.3^8 \approx 8.2$ compared with a fully susceptible profile. In plain clinical terms, that means the model makes this highly resistant hospital strain about eight times more likely to be the one acquired than an otherwise available susceptible strain from the same pool.
...
**Counterfactual gating.** Profile sampling is gated by the `counterfactual_resistance_multiplier`. Before writing the sampled genotype into the individual, the model makes a Bernoulli draw against that multiplier (default 1.0; set to 0.0 in the AMR-free counterfactual branch). If the draw fails, none of the sampled profile's mechanisms are written, so the newly infected individual remains susceptible through this pathway. This parameter is therefore the primary lever for constructing resistance-free counterfactual acquisitions without special-casing individual organisms.
...
Note: after these six steps, the carrier-derived infection bridge described in Section 3.3 may apply additional mechanisms from `mechanism_microbiome` if the individual is already a carrier of the same organism. That step is separate from regional profile-cache acquisition: it represents endogenous transition from carriage to infection and is governed by `carrier_resistance_inheritance_probability`, `run_pathway_carrier_inheritance_multiplier`, and the per-mechanism `infection_from_microbiome_dampening` transfer probability.
...
| counterfactual_resistance_multiplier | 1 by default; AMR-free branches set 0 |
