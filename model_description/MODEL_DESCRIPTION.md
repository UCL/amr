...
- The person's resistance-mechanism profile passes directly from carriage to infection and each resistance mechanism present in the microbiome compartment (`mechanism_microbiome`) is independently considered for transfer to the infection compartment (`mechanism_any`)
...
After every simulated day, the model refreshes a **stored library of circulating resistance-mechanism profiles** (`MechanismCache` in the code) containing up to 1000 complete resistance-mechanism profiles for each combination of region × care setting (community / hospital) × bacterium. Each eligible active infection contributes the resistance-mechanism profile of its predominant strain to that day's collection. If that strain carries none of the resistance mechanisms represented in the model, it is recorded as a resistance-mechanism profile with no modelled acquired resistance. Every eligible active infection-day has an equal chance of entering its daily region × setting × bacterium collection, but a persistent infection can contribute again on successive days. The retained resistance-mechanism profile library is therefore infection-day weighted rather than a sample of unique people, incident infections, or unique bacterial lineages. Resistance-mechanism profiles are assigned according to the person's **current location** (region and hospital/community status) at the time of collection.

The stored resistance-mechanism profile library blends retained resistance-mechanism profiles with newly collected resistance-mechanism profiles each day. Every stored resistance-mechanism profile has the same marginal probability of retention (0.999 for `community_profile_cache_retention` and `hospital_profile_cache_retention`). Hospital and community groups can both become all-susceptible when that is what retained and newly observed active resistance-mechanism profiles contain. Established resistance-mechanism profiles are represented separately in the bounded local mechanism-persistence archive rather than being forced into the active-prevalence library.
...
**Hospital enrichment lever.** Hospital-acquired active infections can be made more likely to draw already-observed resistance-mechanism profiles containing at least one represented mechanism through `hospital_resistance_prune_susceptible_percent`. Before a resistance-mechanism profile is drawn, the model temporarily removes the configured fraction of fully susceptible resistance-mechanism profiles from the candidate hospital pool; resistance-mechanism profiles containing at least one represented mechanism are never removed. The default is 50%, with higher organism-specific values for classic nosocomial pathogens. This changes the probability of selecting complete resistance-mechanism profiles already observed in the local library and does not create new mechanisms or artificial combinations of mechanisms.

**Step 2 — Community resistance-mechanism profile source**

Community infection and carriage acquisition hazards are determined independently of the resistance-mechanism profile assigned at acquisition. Conditional on a new community acquisition, the bacterium-specific `community_resistance_dilution_factor` controls whether the model attempts to inherit a complete resistance-mechanism profile from the local human circulating resistance-mechanism profile library. Despite the word `dilution` in its configuration name, the parameter is therefore the probability of using this resistance-mechanism profile source rather than a direct multiplier applied to resistance prevalence.
...
For a new active community infection, a successful probability check leads to resistance-mechanism profile sampling from the local community human-reservoir resistance-mechanism profile library, including its bounded local persistence archive. If the check is unsuccessful, that resistance-mechanism profile library is skipped and the infection follows the exogenous route, where configured environmental or dynamic ratchet probabilities may assign mechanisms without sampling a human-reservoir resistance-mechanism profile. Same-person carriage inheritance can add mechanisms after either route. For a new community carriage episode, the same factor controls whether the human-reservoir resistance-mechanism profile library is sampled, but the active-infection exogenous-floor assignment step is not applied; resistance can subsequently arise through the other carriage pathways. Hospital acquisitions do not use this community probability check.

The parameter consequently controls the mixture of resistance-mechanism profile pathways conditional on acquisition. Its effect on resistance is usually, but not necessarily, monotonic because the human-reservoir resistance-mechanism profile library contains resistance-mechanism profiles with and without modelled acquired resistance, and the exogenous active-infection route can itself assign resistance.
...
Rather than deciding each mechanism independently from a separate prevalence estimate, the framework draws a **complete resistance-mechanism profile** from the stored resistance-mechanism profile library described in Step 1, assigning all mechanisms recorded in that resistance-mechanism profile to the newly infected individual at the same time. The result is that newly acquired *E. coli*, for example, arrives with a resistance-mechanism profile that mirrors an actual circulating strain — e.g., ESBL CTX-M together with fluoroquinolone resistance, as these co-occur on real plasmids (Partridge SR et al., 2018).

**Community-acquired infections** that pass the Step 2 probability check — meaning the model treats them as arising from the human circulating reservoir rather than from a wholly exogenous environmental exposure — draw a resistance-mechanism profile uniformly at random from the community resistance-mechanism profile group for the relevant region and bacterium. Because that stored group already includes resistance-mechanism profiles with and without modelled acquired resistance, this uniform draw reproduces the resistance prevalence already present in the community library after the ecological dilution step has done its work. In the very early warm-up phase, if the requested community resistance-mechanism profile group is still empty, the model temporarily uses the other care-setting stratum rather than inventing a synthetic resistance-mechanism profile.

**Hospital-acquired enriched sampling.** Hospital-acquired active infections can, when configured, draw from an enriched version of the stored local hospital resistance-mechanism profile group. Early in a run, before the hospital resistance-mechanism profile group for that region and bacterium has accumulated an example containing at least one represented mechanism, the model temporarily samples from the combined local community-plus-hospital set, so resistance already circulating nearby can inform early hospital acquisitions. Once hospital resistance-mechanism profiles containing at least one represented mechanism exist, sampling returns to the hospital group alone. The model can temporarily remove a chosen fraction of fully susceptible resistance-mechanism profiles before the uniform draw, while never removing resistance-mechanism profiles containing at least one represented mechanism. If removing fully susceptible resistance-mechanism profiles would leave nothing to sample from, the original set is used instead. The chosen resistance-mechanism profile is therefore always one that already exists in the stored library; this step changes the probability of selecting previously observed resistance-mechanism profiles, but does not create artificial new resistance combinations.

Carriage acquisition (Section 8.2) is related but not identical. Hospitalised carriage acquisitions draw uniformly from the hospital resistance-mechanism profile stratum and do not use the removal step for fully susceptible resistance-mechanism profiles that is reserved for active hospital infections. For hospital carriage, the run-level `run_pathway_microbiome_acquisition_multiplier` controls whether resistance-mechanism profile sampling occurs. Community carriage applies both that control and the per-bacterium community dilution factor. A sampled carriage resistance-mechanism profile is recorded only in `mechanism_microbiome` and used to calculate `microbiome_r`; it does not create active-infection `mechanism_any` or `any_r` state. Transfer into a later same-organism infection occurs through the separate carrier-inheritance pathway.

If no resistance-mechanism profile exists in either relevant care-setting stratum for a given region and bacterium (early in the simulation, before enough infections have accumulated), no resistance-mechanism profile is assigned and the individual remains susceptible through the resistance-mechanism profile sampling pathway.

**Counterfactual scenario control.** The `counterfactual_resistance_multiplier` controls whether resistance-mechanism profile sampling occurs. Before assigning the sampled resistance-mechanism profile to the individual, the model makes a random probability check against that multiplier (default 1.0; set to 0.0 in the counterfactual scenario). If the check is unsuccessful, none of the sampled resistance-mechanism profile's mechanisms are assigned, so the newly infected individual remains susceptible through this pathway. The same multiplier also controls the de novo, microbiome-acquisition, and HGT pathways elsewhere in the model. It does not, however, control every acquisition-time rule: the exogenous floor pathway and the MDR-TB rifampicin rule are handled separately below.
...
|---------|--------------|---------------|----------------|
| Human-reservoir resistance-mechanism profile sampling | New infection passes the human-reservoir draw | Records the host-eligible subset of the sampled resistance-mechanism profile in `mechanism_any` and `mechanism_majority` | Acquisition of a resistance-mechanism profile already circulating in a strain |
| Hospital-enriched resistance-mechanism profile sampling | Hospital-acquired active infection with hospital enrichment configured | Records a hospital-weighted sampled resistance-mechanism profile in `mechanism_any` and `mechanism_majority` | Makes already-observed ward resistance-mechanism profiles more likely to be selected without creating new combinations |
| Community dilution | Community-acquired infection before resistance-mechanism profile sampling | Determines whether the acquisition draws from the human circulating resistance-mechanism profile library at all | A failed draw does not itself add resistance; it sends the acquisition to the exogenous path without human-reservoir resistance-mechanism profile sampling |
...
| Dynamic ratchet floors | Same exogenous path, for mechanisms below the configured reversion-rate threshold | Records one or more mechanisms as present in `mechanism_any` and `mechanism_majority` | Uses stepped historical peaks from the circulating resistance-mechanism profile library as exogenous reseeding probabilities; does not impose a hard prevalence bound |
...
- `ExcludedHost`: the pair is outside the current biological/model scope. The mechanism cannot emerge de novo, be received or donated by HGT, be assigned by an exogenous floor, be imported from a sampled resistance-mechanism profile, or contribute to the derived resistance phenotype for that bacterium.
- `EligibleNoDeNovo`: the host is eligible but the configured de novo rate is zero and the mechanism is not horizontally transferable. It can still be present in an inherited circulating resistance-mechanism profile or be assigned by an explicit host-compatible exogenous rule.
- `HgtOnly`: the host is eligible, the configured de novo rate is zero, and the mechanism is transferable. It can arrive through HGT where a compatible donor-recipient link exists, or through an already-circulating or exogenous resistance-mechanism profile, but receives no de novo emergence attempt.
...
This distinction is important because a zero emergence rate is not a synonym for an impossible host. The same classification is applied when mechanism-drug applicability is determined, when resistance-mechanism profiles are recorded or imported, when environmental or ratchet floors are evaluated, during carriage-to-infection and infection-to-carriage transfer, and for HGT donors and recipients. Exact host restrictions therefore cannot be bypassed by a different mechanism-acquisition pathway.
