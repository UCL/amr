...
If the pathway is applied, only mechanisms already present in the person's carriage compartment are eligible, and those mechanisms are copied into `mechanism_any` before infection resistance is recalculated. In practice, this link is evaluated after a new infection has been established, provided the person is already carrying the same organism in `presence_microbiome`. It is distinct from sampling from the regional stored profile library during acquisition (Section 3.4), because it uses the person's own carriage profile rather than importing a profile from the surrounding community or hospital pool.
...
After every simulated day, the model refreshes a **stored library of circulating resistance profiles** (`MechanismCache` in the code) containing up to 1000 complete resistance profiles for each combination of region × care setting (community / hospital) × bacterium. Each eligible active infection contributes the resistance-mechanism profile of its predominant strain to that day's collection. If that strain carries none of the resistance mechanisms represented in the model, it is recorded as a profile with no modelled acquired resistance. Before a profile is stored or assigned to a newly acquired infection, it is filtered to retain only mechanisms that the model permits for that bacterial species, so an excluded mechanism cannot persist through the profile library. Every eligible active infection-day has an equal chance of entering its daily region × setting × bacterium collection, but a persistent infection can contribute again on successive days. The retained library is therefore infection-day weighted rather than a sample of unique people, incident infections, or unique bacterial lineages. Profiles are assigned according to the person's **current location** (region and hospital/community status) at the time of collection.

The stored profile library blends retained profiles with newly collected profiles each day. Every previously stored profile has the same marginal probability of retention, implemented by stochastic rounding of the retained count followed by uniform removal; survival is therefore independent of profile position, resistance status, and care setting. In the current configuration, both `community_profile_cache_retention` and `hospital_profile_cache_retention` remain 0.999. For an existing stored profile this corresponds to an expected marginal half-life of roughly 693 daily refreshes. It is a retention parameter for the profile library, not a biological half-life for host carriage, a bacterial clone, or a resistance mechanism. Hospital and community groups can both become all-susceptible when that is what retained and newly observed active profiles contain; previously established resistant genotypes remain represented separately in the bounded local mechanism-persistence archive rather than being forced into the active-prevalence library.

Alongside the current circulating-profile library, the model keeps a permanent **local mechanism-persistence archive**. The first complete observed genotype carrying each mechanism is remembered separately for that region × care setting × bacterium. Distinct archived genotypes form a small, weighted representation of established strains in the much larger population outside the finite simulation. The archive is not included in the prevalence calculated from the current circulating-profile library and does not itself imply that a genotype is currently circulating among simulated infections. Section 7.5 describes its bounded contribution to profile sampling.
...
For a new active community infection, a successful probability check leads to profile sampling from the local community human-profile library, including its bounded local persistence archive. If the check is unsuccessful, that profile library is skipped and the infection follows the exogenous/no-profile route, where configured environmental or dynamic ratchet probabilities may assign mechanisms. Same-person carriage inheritance can add mechanisms after either route. For a new community carriage episode, the same factor controls whether the human-profile library is sampled, but the active-infection exogenous-floor assignment step is not applied; resistance can subsequently arise through the other carriage pathways. Hospital acquisitions do not use this community probability check.
...
**Community-acquired infections** that pass the Step 2 probability check — meaning the model treats them as arising from the human circulating reservoir rather than from a wholly exogenous environmental exposure — draw a profile uniformly at random from the community profile group for the relevant region and bacterium. Because that stored group already includes susceptible as well as resistant profiles, this uniform draw reproduces the resistance prevalence already present in the community library after the ecological dilution step has done its work. In the very early warm-up phase, if the requested community profile group is still empty, the model temporarily uses the other care-setting stratum rather than inventing a synthetic profile.

**Hospital-acquired enriched sampling.** Hospital-acquired active infections can, when configured, draw from an enriched version of the stored local hospital profile group. Early in a run, before the hospital profile group for that region and bacterium has accumulated a resistant example of its own, the model temporarily samples from the combined local community-plus-hospital set, so resistance already circulating nearby can inform early hospital acquisitions. Once resistant hospital profiles exist, sampling returns to the hospital group alone. The model can temporarily remove a chosen fraction of fully susceptible profiles before the uniform draw, while never removing resistant profiles. If removing susceptible profiles would leave nothing to sample from, the original set is used instead. The chosen profile is therefore always one that already exists in the stored library; this step changes the probability of selecting previously observed profiles, but does not create artificial new resistance combinations.

Carriage acquisition (Section 8.2) is related but not identical. Hospitalised carriage acquisitions draw uniformly from the hospital profile stratum and do not use the susceptible-profile removal step reserved for active hospital infections. For hospital carriage, the run-level `run_pathway_microbiome_acquisition_multiplier` controls whether profile sampling occurs. Community carriage applies both that control and the per-bacterium community dilution factor. A sampled carriage profile is recorded only in `mechanism_microbiome` and used to calculate `microbiome_r`; it does not create active-infection `mechanism_any` or `any_r` state. Transfer into a later same-organism infection occurs through the separate carrier-inheritance pathway.
...
**Counterfactual scenario control.** The `counterfactual_resistance_multiplier` controls whether profile sampling occurs. Before assigning the sampled resistance profile to the individual, the model makes a random probability check against that multiplier (default 1.0; set to 0.0 in the counterfactual scenario). If the check is unsuccessful, none of the sampled profile's mechanisms are assigned, so the newly infected individual remains susceptible through this pathway. The same multiplier also controls the de novo, microbiome-acquisition, and HGT pathways elsewhere in the model. It does not, however, control every acquisition-time rule: the exogenous floor pathway and the MDR-TB rifampicin rule are handled separately below.
...
When an infection or carriage acquisition samples the **local human reservoir**, the current circulating-profile library and the local historical archive act as two candidate profile pools. The archive has total virtual mass `K`, shared across all of its distinct complete genotypes. If the current candidate pool contains `N` profiles, the archive is selected with probability:

$$p_{archive} = \min\left(p_{max}, \frac{K}{N + K}\right).$$

The defaults are `K = 10` and `p_max = 0.10`. Thus the archive contributes about 0.99% of draws beside a full 1000-profile circulating library, 9.09% beside 100 current profiles, and never more than 10% in sparsely populated or empty strata. A successful draw selects one complete archived genotype in place of a profile from the current library. The draw is not repeated per mechanism or per drug, and archived mechanisms are never added independently to another sampled genotype.

The archive is enabled by `local_mechanism_persistence_enabled = 1.0`. Establishment and sampling are strictly local: an observation in one region or care setting does not seed another. Archived profiles remain eligible whether or not their mechanisms are also present in the current circulating-profile library; this continuous low-weight contribution prevents one restored profile from immediately disabling persistence support. The archive does not alter prevalence in the current library. It is sampled only on the human-reservoir pathway, follows the same counterfactual scenario control as ordinary profile inheritance, and can contribute to either active infection or carriage. Successful incorporations are reported separately for those two destinations.
...
The former universal per-drug acquisition rule has been removed. It repeated a nominal probability across all eligible drugs and could therefore impose strong pressure that depended on the number of drugs represented rather than on local extinction. The local archive replaces only its finite-population persistence purpose; explicit environmental probabilities and the ratchet remain separate exogenous pathways.
...
After the resistance profile has been recorded (by active or latent profile sampling, exogenous reseeding, or another explicit acquisition rule), the model uses `propagate_mechanism_resistance()` to translate the mechanisms recorded as present into the summary resistance measure `any_r` reported in outputs. For every drug, this calculation combines susceptibility multiplicatively across all active mechanisms:

$$\text{any\_r} = 1 - \prod_{m : \text{mechanism\_any}_m = \text{true}} (1 - e_m)$$

where $e_m$ is the enhancement multiplier for mechanism $m$ against that drug (see Section 7.2). At this stage resistance is recalculated from the full recorded mechanism profile (`raise_only = false` in the code), so the derived value replaces the previous value. If later steps add mechanisms, the calculation is repeated without allowing a decrease (`raise_only = true`), so `any_r` can rise when new mechanisms are added but is not reduced by that partial recalculation.

**Step 6 — MDR-TB guaranteed rifampicin resistance**

*M. tuberculosis* is defined as multi-drug-resistant (MDR) by its rifampicin resistance — any isolate labelled MDR-TB will already carry a rifampicin-resistance mutation at the time of diagnosis (World Health Organization drug-resistant TB treatment guidelines, 2020). The model reflects this definitional requirement with a dedicated post-sampling step. For all MDR-TB acquisitions occurring in or after 1966 (when rifampicin was introduced), the current model records every resistance mechanism applicable to rifampicin for that bacterium — specifically the `rpoB` RNA-polymerase mutation — in both `mechanism_any` and `mechanism_majority`, regardless of what the earlier profile-sampling step assigned. Resistance is then recalculated without allowing a decrease so that the rifampicin `any_r` reflects this guaranteed mechanism. The rifampicin acquisition pathway is classified as `ResistanceAcquisitionType::AtInfectionTB` rather than `AtInfectionCommunity`, allowing it to be tracked separately in output statistics. The parameter `mdr_mycobacterium_tuberculosis_guaranteed_rifampicin_resistance` acts as an enable/disable control: any positive value enables this fixed model rule, so the current default of 0.90 behaves as "enabled" rather than as a 90% random probability check.
...
| Dynamic ratchet floors | Same exogenous path, for mechanisms below the configured reversion-rate threshold | Records one or more mechanisms as present in `mechanism_any` and `mechanism_majority` | Uses stepped historical peaks from the circulating-profile library as exogenous reseeding probabilities; does not impose a hard prevalence bound |
| Local mechanism-persistence archive | Human-reservoir infection or carriage sampling after a complete genotype has been established locally | Samples one complete archived genotype for that region × setting × bacterium | One bounded profile-level draw shared across all archived genotypes; does not alter prevalence in the current circulating-profile library |
| MDR-TB rifampicin rule | MDR-TB acquisition from 1966 onward when the rule is enabled | Sets rifampicin-applicable mechanisms in `mechanism_any` and `mechanism_majority` | Enforces the definitional rifampicin resistance of MDR-TB |
...
**Mechanism-centred architecture.** For each person and bacterium, the model records which resistance mechanisms are present in the active infection and in any bacteria being carried / in the microbiome. Drug-level resistance is then calculated from those mechanisms. In the code these three records are `mechanism_any`, `mechanism_majority`, and `mechanism_microbiome`, while the derived summary resistance measures are `any_r` and `microbiome_r`. A single stored profile library (`MechanismCache` in the code) holds up to 1000 retained predominant-strain mechanism profiles per region × care setting × bacterium for use in profile-based acquisition (Section 3.4). It is assembled from active infection-days rather than unique isolates or people. This library keeps separate hospital and community collections, retains profiles over time, derives prevalence directly from the stored profiles, and supports configured removal of susceptible profiles from the candidate set for active hospital infections. It also maintains a regional `peak_mechanism_prevalence` table (Section 7.8) containing the highest annually sampled marginal prevalence in each region's community profile library for each bacterium-mechanism pair; stepped values derived from that table can support later local exogenous reseeding.

`MechanismCache` also stores a local historical establishment archive separately from its current profiles and prevalence estimates. This archive retains one complete first-observed genotype for each established mechanism in each region × care setting × bacterium stratum, retains only one copy of identical genotypes, and supplies the bounded representation of the wider population described in Section 7.5.
...
This distinction is important because a zero emergence rate is not a synonym for an impossible host. The same classification is applied when mechanism-drug applicability is determined, when profiles are recorded or imported, when environmental or ratchet floors are evaluated, during carriage-to-infection and infection-to-carriage transfer, and for HGT donors and recipients. Exact host restrictions therefore cannot be bypassed by a different mechanism-acquisition pathway.
...
The three organism-specific values of 3.0 are 30 times the ordinary 0.1 community multiplier, but only three times the same mechanism's hospital/base rate; they are not the former 200× settings and do not imply loss within a few days for the configured mechanism rates. These organism-level multipliers apply to every eligible mechanism for that bacterium, not only to carbapenemases, VanA/VanB, or *gyrA*. They are compressed ecological calibration terms used to preserve plausible setting contrasts while retaining the same mechanism-specific fitness-cost ordering (Andersson DI & Hughes D, 2010; San Millán A & MacLean RC, 2017; Arcilla MS et al., 2017).
...
*Implementation note: The previously unused category `as_yet_unknown` has been replaced by the explicit `mutation_siderophore_uptake` mechanism. This preserves the number of mechanism categories represented by the model while removing an uninterpretable calibration placeholder.*

### 7.5 Local finite-population mechanism persistence

At finite simulated population sizes, an uncommon but genuinely established mechanism can disappear from every active infection and every retained profile in a local reservoir by chance. The model represents established strains in the much larger population outside the simulation through a permanent local establishment archive rather than through the former universal per-drug acquisition rule. The archive contributes bounded sampling mass to the local human reservoir; it is not a hard lower bound on observed resistance prevalence.

| Parameter | Default | Function |
|-----------|--------:|----------|
| `local_mechanism_persistence_enabled` | 1.0 | Enables local historical establishment and pseudo-reservoir sampling |
| `local_mechanism_persistence_virtual_profile_mass` | 10.0 | Total virtual mass shared across all distinct archived genotypes in a local stratum |
...
**Establishment and archive composition.** At each daily refresh of the circulating-profile library, the first complete observed genotype carrying each mechanism is archived for that exact region × care setting × bacterium stratum. The archive is permanent but remains separate from the current profile library. Only one copy of each identical archived genotype is retained, so one observed genotype carrying several mechanisms is still one candidate. Later observations do not replace the first establishment genotype.
...
**Scope and interpretation.** Establishment does not cross regions, bacteria, or community/hospital strata. The pathway is not used for exogenous sources. It follows the same counterfactual scenario control as ordinary profile inheritance and can contribute to both active infection and carriage. Archived profiles do not count towards current circulating-profile prevalence, prescribing knowledge, or ratchet peak calculations. Once incorporated into an active infection, however, the genotype can re-enter the ordinary daily circulating-profile library through the same predominant-strain profile collection process as any other circulating strain.
...
The former `resistance_floor_*` configuration and repeated per-drug calculation `calculate_resistance_floor` have been removed. That calculation repeated a nominal probability across eligible drugs, was not local or triggered by extinction, and could therefore exert strong pressure unrelated to its finite-population rationale. Static environmental floors and the dynamic ratchet remain independent exogenous pathways (Sections 7.7 and 7.8).
...
Population-level persistence of *H. pylori* resistance instead arises through the same predominant-strain profile library, local historical archive, ratchet, inheritance, and repeated-acquisition processes used for other active infections. Drug-bacterium initiation multipliers favour drugs associated with *H. pylori* treatment once the organism is identified, but the drug-scoring calculation is a compressed representation and does not treat named triple or quadruple regimens as indivisible treatment units. Incidental exposure to applicable drugs prescribed for other indications can also contribute selection pressure while an *H. pylori* episode is active.
...
Era-specific `_before_{YYYY}` values allow the exogenous reservoir to change through time. Co-selection between organisms is represented through the model's explicit HGT and complete-profile pathways rather than a second static-floor suffix that would have the same effect.
...
So while the original motivation was agricultural resistance ecology, the current model is broader: it now represents any explicitly parameterised exogenous pathway that should continue to reseed resistance even when the local human circulating-profile library alone would be insufficient.
...
**Motivation.** Even with long retention of previously observed profiles, a finite stored profile library can under-represent historically established resistance after current selection pressure falls. The ratchet preserves a coarse regional memory of high prevalence previously sampled from the community profile library and uses that memory to support later local exogenous acquisition of mechanisms with sufficiently low configured reversion rates.
...
**Annual update.** Once per simulated year, after that day's circulating-profile library update, the model computes marginal mechanism prevalence separately in every region × bacterium community-profile stratum. Hospital profiles are excluded because the ratchet acts only on exogenous community acquisitions. A regional bacterium stratum must contain at least 100 retained community profiles before that annual observation can increase its permanent peak; evidence from different regions is not pooled to meet this threshold. Every retained profile in the qualifying local stratum contributes equally. The `peak_mechanism_prevalence[region][bacteria][mechanism]` table can increase, but not decrease, using values recorded at qualifying annual observations: once a local sampled peak has been recorded, it is not allowed to fall. Exogenous acquisition uses only the current region's peak. Genuinely global importation from outside the model must therefore be represented by an explicit static exogenous probability rather than inferred from another region's history.
...
capped at 0.50. Here `ratchet_floor` is a probability used when assigning mechanisms during exogenous acquisition. For example, a recorded local peak of 12% yields a 10% assignment probability and a peak of 23% yields a 20% probability in that region. The per-region 100-profile evidence threshold prevents a handful of retained profiles from establishing permanent memory. Annual sampling and 10-point steps further reduce sensitivity to small fluctuations, but they do not eliminate sampling noise; because a recorded peak cannot decrease, a qualifying sampled threshold crossing has a lasting effect.
...
In conceptual terms, a static environmental floor is a configured exogenous reseeding probability, whereas the ratchet is a history-derived exogenous reseeding probability based on prior prevalence in the circulating-profile library.

**Relationship to static floors.** Era-specific static environmental probabilities can provide exogenous seeding during configured historical periods, including periods when the finite circulating-profile library might otherwise build a resistant reservoir slowly. The ratchet has no universal takeover date. At each eligible exogenous acquisition, the model uses the larger of the static and ratchet probabilities. For example, if a *Shigella* mechanism has a current static probability of 0.45 and a recorded peak from the profile library produces a ratchet probability of 0.40, the effective assignment probability remains 0.45. Neither value implies 45% or 40% total resistance prevalence.
...
|---------|-----------|-------|---------------|
| Resistance profile sampling on acquisition | `run_pathway_microbiome_acquisition_multiplier` | 1.0 | Scales the probability that a new carriage episode samples a resistance profile from the local circulating-strain library. Hospital carriage uses this probability control directly; community carriage also applies the per-bacterium community dilution factor. A value of 1.0 is neutral rather than an independent 50% colonisation bottleneck. |
...
file-format definition, earlier wide-format matrices, Rust-derived potency matrix, and Rust-derived
...
| Bacteria | Sepsis base log-odds | Sepsis log-odds/level | Sepsis log-odds/day | Non-sepsis death log-odds | Organism-specific sepsis-death adjustment | Mechanismless reversion/day | Community human-profile probability | Hospital susceptible profiles removed (%) | Community mechanism-reversion multiplier |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
...
De novo emergence rate and modelled pathway status for every bacteria–mechanism pair. A zero rate does not necessarily exclude the host: transferable mechanisms can remain HGT-only, while non-transferable eligible mechanisms can still be inherited in an existing complete profile.
...
| `{bacteria}_{drug}_any_r`, `microbiome_r_positive`, and split hospital/community variants | Mechanism-derived resistance summaries. The summary CSV no longer exports `{bacteria}_{drug}_majority_r`; mechanism-majority state is used internally and exposed through mechanism-derived summaries rather than the old majority-r column group. |
...
| [`level[b]`](#rule-level) | Persistent value for each bacterium | Non-negative unitless infection intensity. | `0`; set to a bacterium-specific value at acquisition. | Reset to `0` on clearance. | Active infection |
...
| [`clearance_hazard[b]`](#rule-clearance-hazard) | Persistent value for each bacterium | Daily immune-clearance probability, from `0` to `1`. | `0`. | Recalculated during active infection; reset on clearance. | Active infection |
...
| [`local_profile_sampling_probability[b]`](#rule-local-profile-sampling-probability) | Temporary probability for each bacterium | Probability that a newly acquired carriage or infection profile is sampled from local persistent strain pressure. | Recalculated at acquisition. | Discarded after source selection. | Resistance persistence |
...
| [`new_bacteria_level[b]`](#rule-new-bacteria-level) | Temporary value for each bacterium | Proposed next infection intensity after growth and drug activity. | Recalculated for active infection. | Stored in `level[b]` or replaced by clearance. | Active infection |
...
| <a id="rule-resistance-microbiome-r"></a>`resistances[b][d].microbiome_r` | `mechanism_microbiome[b]`, drug class. | `resistance_mechanism_{mechanism}_enhancement_{drug_class}` with the mechanism-wide value used when no class-specific value is supplied; `max_resistance_level`. | Recomputed from the set of carriage mechanisms after carriage resistance changes. | Continuous susceptibility reduction among carriage profiles; zero does not mean the bacterium is absent. | Resistance recalculation functions in `rules`; `population::Resistance` |
...
| <a id="rule-mechanism-any"></a>`mechanism_any[b]` | Incoming infection profile, de novo emergence, HGT, minority promotion and reversion. | Mechanism applicability; de novo, HGT, promotion and reversion parameters. | The set is updated when any represented infection strain carries or loses a mechanism. | Includes minority mechanisms; `mechanism_majority` is the predominant-profile subset. | Resistance acquisition and evolution blocks in `rules::apply_rules` |
| <a id="rule-mechanism-majority"></a>`mechanism_majority[b]` | Incoming predominant-strain profile, minority promotion and predominant-strain reversion. | `majority_r_evolution_rate_per_day_when_drug_present`; reversion parameters. | Updated when mechanisms establish in, enter, or leave the represented predominant infection profile. | Must remain a subset of `mechanism_any[b]`. | Resistance acquisition and evolution blocks in `rules::apply_rules` |
| <a id="rule-mechanism-microbiome"></a>`mechanism_microbiome[b]` | Incoming carriage profile, HGT and carriage reversion. | Carriage profile-source, HGT and reversion parameter families. | Set at carriage acquisition and updated during carriage resistance evolution; reset at clearance. | Separate from the resistance-mechanism profiles of active infections. | Carriage acquisition, evolution and clearance blocks in `rules::apply_rules` |
| <a id="rule-how-resistance-acquired"></a>`how_resistance_acquired[b][d]` | Profile sampling, microbiome inheritance, de novo emergence and HGT events. | Fixed provenance categories; implementation setting `TRACK_RESISTANCE_ACQUISITION_PROVENANCE`, selected when the program is built. | Updated when resistance first enters or materially changes the active infection profile. | No values are retained when tracking is disabled; when enabled, the result is a coarse classification of the most recent event rather than full causal provenance. | Resistance acquisition and evolution blocks in `rules`; output aggregation in `simulation` |
...
| <a id="rule-incoming-infection-mechanism-mask"></a>`incoming_infection_mechanism_mask[b]` | Sampled local or exogenous profile, `mechanism_microbiome[b]`, current acquisition setting. | Circulating-profile library and ratchet parameters; `carrier_resistance_inheritance_probability`; `infection_from_microbiome_dampening`; mechanism applicability. | Calculated only after an infection-acquisition draw succeeds, then restricted to mechanisms permitted for the bacterium. | Discarded if existing therapy prevents establishment. | Infection profile assembly in `rules::apply_rules`; `simulation::ResistanceProfileCache` |
| <a id="rule-existing-therapy-prevention-probability"></a>`existing_therapy_prevention_probability[b]` | Current drug exposure, potency, syndrome penetration and resistance implied by the incoming mechanism profile. | `antibiotic_infection_prevention_efficacy`; potency, penetration and `max_resistance_level`. | Calculated for candidate acquisition under active therapy before prevention draws. | Prevents establishment; it does not reduce the externally specified acquisition hazard itself. | Infection-acquisition prevention block in `rules::apply_rules` |
