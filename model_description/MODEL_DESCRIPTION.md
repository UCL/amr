...
Carriage acquisition (Section 8.2) is related but not identical. Hospitalized carriage acquisitions draw uniformly from the hospital profile stratum and do not use the susceptible-profile removal step reserved for active hospital infections. Carriage gates profile sampling with `run_pathway_microbiome_acquisition_multiplier`, then applies the per-bacterium hospital boost or community dilution modifier. A sampled carriage profile is recorded only in `mechanism_microbiome` and projected to `microbiome_r`; it does not create active-infection `mechanism_any` or `any_r` state. Transfer into a later same-organism infection occurs through the separate carrier-inheritance pathway.
