...
We present a **framework** in the form of an **individual-based model** that simulates infection incidence, antibacterial use, resistance emergence, sepsis, and death. In the current configuration, we simulate a representative sample of the global population from 1930, before widespread antibacterial use, through 2025 (95 years) in **daily** time steps; full policy runs can extend through 2035 and beyond. We simulate the lives of 10 million people who are alive at some point during the 95 years. The framework code is open source, and we encourage others to use it to investigate how resistance has emerged over time, including through counterfactuals, and to predict the effects of potential policies on antibacterial resistance and infection-related mortality. These policies may aim directly to limit resistance, or may involve wider antibacterial use whose potential resistance costs need to be weighed against other benefits.  Users are free to make adaptations to the struture as well as the parameter values.
...
The framework dynamically models resistance at infection acquisition and how this depends on resistance in people concurrently infected in the same region and hospital / community setting, also accounting for movement between regions. Infection acquisition hazards are externally parameterised by bacterium, age, region, vaccination, carriage status, hospital status, sanitation, and selected organism-specific calendar-era effects.
...
**Implementation order within a simulated day.** The ordering of updating of variable values in a day is: age/living-state checks and birth-cohort vaccination; reset daily microbiome flags; immunodeficiency onset/recovery; hospital admission or discharge; travel; sepsis onset or clearance for infections already active at that point in the day; antibiotic stopping, pharmacokinetic decay, and new drug initiation/selection; toxicity and treatment-failure/restart checks; mortality and sepsis recovery; then a per-bacterium block that handles carriage acquisition/clearance and carriage resistance evolution, active-infection acquisition or progression, resistance profile assignment, testing/AST state, infection growth, symptoms, clearance, and resistance reversion/emergence. HGT is evaluated after the per-bacterium block. This order matters: sepsis onset is evaluated before new infections acquired later that same day, and new acquisition is evaluated separately for each bacterium rather than being skipped because the person already has another active infection.

**Stochastic processes.** The model calculates a *probability* for each event and then samples at random to determine whether that event occurs. Repeated runs therefore produce non-identical outputs.

**Calibration.** As laid out below, the framework contains thousands of parameters. In the current configuration we have just a single value for each parameter. Resistance calibration uses review-informed estimates drawing on surveillance systems and other studies (including WHO GLASS, ECDC EARS-Net, CDC AR Threats, and GRAM/GBD). In deriving review-informed estimates for resistance we take into consideration the fact that resistance data are based on cultured clinical isolates and may overrepresent infections that are tested, severe, persistent, invasive, or healthcare-associated relative to all simulated active-infection person-days.  We recognise that there is uncertainty over most of the parameter values, sometimes large uncertainty. Future users of the framework are likely to want to identify multiple sets of parameter values that produce an acceptable calibration in order to express parameter uncertainty when comparing future policy options.

**Resistance calibration quantities.** in figure 2 we define simulated
resistance prevalence as the proportion of active-infection person-days for which
the bacterium-drug `any_r` value is greater than zero (ie. whether any bacteria with acquired resistance are present in the infection or not). Resistance severity is
represented separately by the mean `any_r` value (on a scale of 0-1) among those positive infection-days.
The model does not assign drug-specific concentration units or attempt
...
| **4. Clinical Progression** | What happens once infected — bacteria level, symptoms, syndromes, sepsis |
...
The six regions and their approximate configured demographic weights determine the geographical distribution of both initially living people and future births:
...
| Africa | ~18% | UN DESA Population Division, 2024 |
| Europe | ~13% | UN DESA Population Division, 2024 |
| North America | ~7% | UN DESA Population Division, 2024 |
| South America | ~5% | UN DESA Population Division, 2024 |
...
The model adds together multiple log-odds ratio terms (e.g., a baseline term, an age term, a severity term) and then converts the total into a probability using the standard logistic function.

For example, the daily probability of starting antibiotics for a hospitalised septic immunocompromised patient with symptoms includes: *baseline log-odds (−5.5) + symptomatic infection (+6.2) + sepsis (+6.5) + hospitalised (+0.7) + immunodeficiency (+0.2) = +8.1*.
...
This is consistent with broad cross-country differences in service coverage and infrastructure documented by WHO's universal health coverage monitoring framework and the World Bank's hospital-bed indicator, which show persistent between-country variation in effective access to care and inpatient capacity even as global service coverage has improved over time (World Health Organization UHC fact sheet, 2025; World Bank, `SH.MED.BEDS.ZS`).
...
The acquisition risk function uses additive log-odds across six age bands (`infant`, `preschool`, `school`, `young_adult`, `middle_age`, and `elderly`). Each bacterium can have its own age profile, and each region can further replace the age adjustment for a particular bacterium. This preserves broad patterns such as childhood enteric disease, vulnerability at the extremes of age, and young-adult concentration of sexually transmitted infections without treating a generic syndrome template as an executable multiplier. Appendix B.6 lists the current bacterium-region-age values.
...
This pathway is governed by two biological parameters:

| Parameter | Value | Interpretation |
|-----------|-------|---------------|
| `carrier_resistance_inheritance_probability` | 0.50 | 50% chance that the carrier-inheritance pathway aries — when it does, individual mechanisms are copied from the microbiome to the infection compartment |
...
The effective person-level gate is scaled by `counterfactual_resistance_multiplier` before the draw. If the pathway is randomly selected to happen, only mechanisms already present in the person's carriage compartment are eligible, and those mechanisms are copied into `mechanism_any` before infection resistance is re-derived. In practice, this bridge is checked after a new infection has been established, provided the person is already carrying the same organism in `presence_microbiome`. It is distinct from sampling from the regional stored profile library during acquisition (Section 3.4), because it uses the person's own carriage profile rather than importing a profile from the surrounding community or hospital pool.
...
After every simulated day, the model refreshes a **stored library of circulating resistance profiles** (`MechanismCache` in the code) containing up to 1000 complete resistance profiles for each combination of region × care setting (community / hospital) × bacterium. Each eligible active infection contributes its current majority-mechanism profile to that day's collection, including a zero mask for a fully susceptible majority strain. Before recording or importing a profile, its mechanism mask is intersected with the bacterium's executable host-eligibility mask, so a mechanism excluded for that host cannot persist through the profile library. Every eligible active infection-day has an equal chance of entering its daily region × setting × bacterium reservoir, but a persistent infection can contribute again on successive days. The retained library is therefore infection-day weighted rather than a sample of unique people, incident infections, or unique bacterial lineages. Profiles are assigned according to the person's **current location** (region and hospital/community status) at the time of collection. Thread-local samples are combined using their full observation counts: an exact hypergeometric draw determines how many profiles to take from each local reservoir, followed by uniform sampling within each reservoir. Unequal processing-chunk sizes therefore do not bias which eligible infection-days are represented.

The stored profile library blends retained profiles with newly collected profiles each day. Every old profile has the same marginal probability of retention, implemented by stochastic rounding of the retained count followed by uniform removal; survival is therefore independent of profile position, resistance status, and care setting. In the live configuration, both `community_profile_cache_retention` and `hospital_profile_cache_retention` remain 0.999. For an existing cache entry this corresponds to an expected marginal half-life of roughly 693 daily refreshes. It is a memory parameter for retained cache entries, not a biological half-life for host carriage, a bacterial clone, or a resistance mechanism. Hospital and community groups can both become all-susceptible when that is what retained and newly observed active profiles contain; previously established resistant genotypes remain represented separately in the bounded local mechanism-persistence archive rather than being forced into the active-prevalence library.

Alongside the active library, the model keeps a permanent **local mechanism-persistence archive**. The first complete observed genotype carrying each mechanism is remembered separately for that region × care setting × bacterium. Deduplicated archived genotypes form a small pseudo-reservoir representing established strains in the much larger population outside the finite simulation. The archive is not included in active-cache prevalence and does not itself imply that a genotype is currently circulating among simulated infections. Section 7.5 describes its bounded contribution to profile sampling.
...
**Hospital-acquired enriched sampling.** Hospital-acquired active infections can, when configured, draw from an enriched version of the stored local hospital profile group. Early in a run, before the hospital profile group for that region and bacterium has accumulated a resistant example of its own, the model temporarily samples from the combined local community-plus-hospital set, so early hospital acquisitions can bootstrap from resistance already circulating nearby. Once resistant hospital profiles exist, sampling reverts to the hospital group alone. The model can temporarily remove a chosen fraction of fully susceptible profiles before the uniform draw, while never removing resistant profiles. If removing susceptible profiles would leave nothing to sample from, the unpruned set is used instead. The chosen profile is therefore always one that already exists in the stored library; this step changes the probability of selecting previously observed profiles, but does not create artificial new resistance combinations.

Carriage acquisition (Section 8.2) is related but not identical. Hospitalised carriage acquisitions draw uniformly from the hospital profile stratum and do not use the susceptible-profile removal step reserved for active hospital infections. The run-level `run_pathway_microbiome_acquisition_multiplier` is the profile-sampling gate for hospital carriage. Community carriage applies both that gate and the per-bacterium community dilution factor. A sampled carriage profile is recorded only in `mechanism_microbiome` and projected to `microbiome_r`; it does not create active-infection `mechanism_any` or `any_r` state. Transfer into a later same-organism infection occurs through the separate carrier-inheritance pathway.
...
**Counterfactual gating.** Policy branches can set `counterfactual_resistance_multiplier` to suppress represented resistance pathways. The default is 1.0. Policy option 2 sets it to 0.0 and also clears existing infection resistance, carriage resistance, AST-reported resistance, resistance provenance, `mechanism_any`, `mechanism_majority`, `mechanism_microbiome`, and the profile cache at branch start. The same multiplier gates or scales profile writes, local-persistence profile writes, exogenous static and ratchet floors, the MDR-TB rifampicin rule, same-person carriage inheritance, daily infection-carriage mechanism transfer, de novo emergence in both active infection and carriage, HGT, and AST reporting-error draws. Thus the 2025 resistance-suppressed counterfactual sends all represented resistance to zero rather than only blocking one profile-sampling route.


**Step 4 — Local persistence and exogenous reseeding**

The acquisition pathway has two distinct forms of resistance persistence. They operate on different source branches and should not be interpreted as prevalence floors.

When an infection or carriage acquisition samples the **local human reservoir**, the active cache and the local historical archive act as two candidate profile pools. The archive has total virtual mass `K`, shared across all of its deduplicated complete genotypes. If the active candidate pool contains `N` profiles, the archive is selected with probability:

$$p_{archive} = \min\left(p_{max}, \frac{K}{N + K}\right).$$

The defaults are `K = 10` and `p_max = 0.10`. Thus the archive contributes about 0.99% of draws beside a full 1000-profile cache, 9.09% beside 100 active profiles, and never more than 10% in sparse or empty slots. A successful draw selects one complete archived genotype in place of an active-cache profile. The draw is not repeated per mechanism or per drug, and archived mechanisms are never OR-ed independently onto another sampled genotype.

The archive is enabled by `local_mechanism_persistence_enabled = 1.0`. Establishment and sampling are strictly local: an observation in one region or care setting does not seed another. Archived profiles remain eligible whether or not their mechanisms are also present in the active cache; this continuous low-weight contribution is what prevents one restored profile from immediately disabling persistence support. The archive does not alter active-cache prevalence. It is sampled only on the human-reservoir branch, follows the same counterfactual gate as ordinary profile inheritance, and can contribute to either active infection or carriage. Successful incorporations are reported separately for those two destinations.

Separately, when a community active-infection acquisition falls into the **non-human / exogenous fraction** (`!from_human_reservoir`), the code rolls each mechanism against an effective floor equal to:

$$\text{assignment probability} = \max(\text{environmental floor}, \text{ratchet floor}) \times \text{counterfactual resistance multiplier}$$

The **environmental floor** is configured only for selected bacteria-mechanism pairs and represents resistance maintained outside the local human circulating pool, for example in agricultural, food-chain, wastewater, or other exogenous reservoirs (see Section 7.7). The **ratchet floor** is dynamic and is derived from stepped historical peak values in the stored profile library (see Section 7.8). Both are evaluated only when the mechanism's executable status permits that bacterial host, and the final draw is scaled by the scenario-level counterfactual multiplier. Despite the word “floor,” both values are Bernoulli mechanism-assignment probabilities on each exogenous acquisition. They provide reseeding pressure but do not mathematically guarantee that resistance prevalence across all infections remains above the same percentage.

The former universal per-drug acquisition guard has been removed. It repeated a nominal probability across all eligible drug entries and could therefore impose strong, drug-list-dependent pressure unrelated to local extinction. The local archive replaces only its finite-population persistence purpose; explicit environmental probabilities and the ratchet remain separate exogenous pathways.

**Linked resistance profiles.** Successful bacterial lineages can carry correlated sets of resistance mechanisms, including the familiar co-resistance patterns of MRSA lineages. The model represents this linkage by sampling and transmitting complete mechanism profiles from the region- and setting-specific profile reservoir. The local persistence archive likewise stores complete profiles. It does not independently add mechanisms after a profile draw merely because their marginal presence has been observed elsewhere: that would create combinations unsupported by the sampled lineage and would replace local joint structure with global marginal probabilities. This rule is the same for *S. aureus* and other bacteria. If future evidence shows that the finite profile reservoir does not adequately retain an important lineage structure, any extension should use empirically defined joint profiles or conditional lineage distributions rather than independent package completion.

**Step 5 — Converting recorded mechanisms into the summary resistance measure `any_r`**

After the resistance profile has been recorded (by active or latent profile sampling, exogenous reseeding, or another explicit acquisition rule), the model calls `propagate_mechanism_resistance()` to translate the mechanisms recorded as present into the summary resistance measure `any_r` reported in outputs. For every drug, this function computes the **multiplicative susceptibility** across all active mechanisms:
...
|---------|--------------|---------------|----------------|
| Human-reservoir profile sampling | New infection passes the human-reservoir draw | Records the host-eligible subset of the sampled mechanism profile in `mechanism_any` and `mechanism_majority` | Acquisition of an already-circulating strain genotype; subject to the scenario resistance gate |
...
| Static environmental floors | Community acquisition not sampled from the human reservoir | Records one or more host-eligible mechanisms as present in `mechanism_any` and `mechanism_majority` | Explicit exogenous reseeding for configured bacteria-mechanism pairs; scaled by the scenario resistance gate |
| Dynamic ratchet floors | Same exogenous path, for mechanisms below the configured reversion-rate threshold | Records one or more mechanisms as present in `mechanism_any` and `mechanism_majority` | Uses stepped historical cache peaks as exogenous reseeding probabilities; scaled by the scenario resistance gate; does not impose a hard prevalence bound |
| Local mechanism-persistence archive | Human-reservoir infection or carriage sampling after a complete genotype has been established locally | Samples one complete archived genotype for that region × setting × bacterium | One bounded profile-level pseudo-reservoir draw shared across all archived genotypes; does not alter active-cache prevalence |
| MDR-TB rifampicin rule | MDR-TB acquisition from 1966 onward when the effective gate is enabled | Sets rifampicin-applicable mechanisms in `mechanism_any` and `mechanism_majority` | Enforces the definitional rifampicin resistance of MDR-TB in ordinary branches; disabled when the scenario gate is zero |
| Same-person microbiome inheritance | New infection when the person already carries the same organism | Copies eligible mechanisms from `mechanism_microbiome` into `mechanism_any` only | Carrier-to-infection bridge; copied mechanisms start as active-infection minority mechanisms; scenario-gated |
| De novo infection emergence | Existing active infection under drug pressure | Adds newly emerged mechanisms to `mechanism_any` | Within-host emergence under selection; majority status can follow later under drug pressure; scaled by pathway and scenario multipliers |
| De novo microbiome emergence | Existing carriage with at least one positive-level active drug applicable to an absent mechanism | Gives each eligible mechanism one daily emergence roll and records successful transitions in `mechanism_microbiome` | Carriage-compartment emergence under binary drug pressure without multiplying the daily rate by the number of selecting drugs; scenario-scaled |
...
| Infection-microbiome transfer | Same organism present in both compartments and they contain different mechanisms | Copies mechanisms so `mechanism_any` and `mechanism_microbiome` contain the combined profile | Within-host spillover between active infection and carriage; scenario-gated |
| HGT | After the per-bacterium loop, between co-present bacteria | Adds mechanisms that are transferable and host-eligible for both donor and recipient to each recipient compartment where that organism is present: `mechanism_any` for active infection and `mechanism_microbiome` for carriage | Inter-species mechanism transfer, scaled by context, donor majority/minority status, run-level HGT multiplier, and scenario resistance gate |
...
- **Symptom onset**: Symptoms are sampled probabilistically once the infection has lasted at least the bacterium-specific `symptom_onset_delay_days` and its level is at least `symptom_onset_threshold_level`. The daily log-odds combine `symptom_onset_base_log_odds` with `symptom_onset_log_odds_per_level_unit` for burden above that threshold. The defaults are a 1-day delay, threshold 0.5, baseline log-odds −1.73, and +0.5 per additional level unit, with organism-specific overrides. Once symptoms begin, `infection_has_caused_symptoms` remains latched for that infection episode.
...
The exact empiric score templates are source-of-truth in `src/config.rs` under `empiric_syndrome_templates` and are generated into Appendix B.7. The current UTI template, for example, strongly favours nitrofurantoin and fosfomycin, while retaining lower scores for alternative oral and hospital agents. These are relative prescribing weights before availability, age, syndrome restrictions, regional access, resistance surveillance, targeted-therapy modifiers, reserve-drug gates, toxicity avoidance, and the final weighted probabilistic draw are applied.
...
The model gates antibiotics at configured introduction days. Sulfanilamide is the first available agent, nalidixic acid provides an early quinolone-era exposure before the later fluoroquinolones, and newer reserve agents enter only near the end of the historical horizon. This layering is essential for reproducing the sequential emergence of resistance over the 20th and early 21st centuries; the exact executable day offsets are generated in Appendix B.2.
...
Antibiotic availability is gated by a per-drug introduction day measured from the 1930 epoch. Before that day a drug cannot be selected; afterwards it remains subject to regional access and all other prescribing restrictions. Appendix B.2 is generated from `DRUG_INTRODUCTION_DATES` and lists the exact live day for all 62 drugs. Keeping that table generated is important because historical prescribing-era overrides do not themselves make a drug available before its configured introduction day.
...
**Mechanism-centred architecture.** For each person and bacterium, the model records which resistance mechanisms are present in the active infection and in any bacteria being carried / in the microbiome. Drug-level resistance is then calculated from those mechanisms. In the code these three records are `mechanism_any`, `mechanism_majority`, and `mechanism_microbiome`, while the derived summary resistance measures are `any_r` and `microbiome_r`. A single stored profile library (`MechanismCache` in the code) holds up to 1000 retained majority-mechanism profiles per region × care setting × bacterium for use in profile-based acquisition (Section 3.4). It is assembled from active infection-days rather than unique isolates or people. This library keeps separate hospital and community collections, retains profiles over time, derives cache prevalence directly from the stored profiles, and supports configured susceptible-profile pruning for active hospital infections. It also maintains a regional `peak_mechanism_prevalence` table (Section 7.8) containing the highest annually sampled marginal prevalence in each region's community cache for each bacterium-mechanism pair; stepped values derived from that table can support later local exogenous reseeding.

`MechanismCache` also stores a local historical establishment archive separately from its active profiles and prevalence estimates. This archive retains one complete first-observed genotype for each established mechanism in each region × care setting × bacterium slot, deduplicates identical genotypes, and supplies the bounded finite-population pseudo-reservoir described in Section 7.5.

The architecture is primarily mechanism-by-mechanism for **de novo** emergence under active drug pressure. Every bacterium-mechanism pair also has an executable route status, constructed once at startup from the reviewed host map, the explicit bacterium-mechanism emergence rate, and whether the mechanism is horizontally transferable:

- `ExcludedHost`: the pair is outside the current biological/model scope. The mechanism cannot emerge de novo, be received or donated by HGT, be assigned by an exogenous floor, be imported from a sampled profile, or contribute to the derived resistance phenotype for that bacterium.
- `EligibleNoDeNovo`: the host is eligible but the configured de novo rate is zero and the mechanism is not horizontally transferable. It can still be present in an inherited circulating profile or be assigned by an explicit host-compatible exogenous rule.
- `HgtOnly`: the host is eligible, the configured de novo rate is zero, and the mechanism is transferable. It can arrive through HGT where a compatible donor-recipient link exists, or through an already-circulating/exogenous profile, but receives no de novo emergence attempt.
- `DeNovo`: the pair has a positive configured de novo rate and an eligible host. Transfer by HGT remains possible as well when that mechanism is classified as transferable.

This distinction is important because a zero emergence rate is not a synonym for an impossible host. The same status is enforced when applicability is cached, when profiles are recorded or imported, when environmental or ratchet floors are evaluated, during carriage-to-infection and infection-to-carriage transfer, and for HGT donors and recipients. Exact host restrictions therefore cannot be bypassed by a different mechanism-entry pathway.
...
The table below lists every mechanism, the drugs it affects, and which bacterial groups can acquire it. It is intended as a reference table. The key point is that each mechanism has a defined scope: ESBL enzymes (rows 1–3) relate to `pen`, `c1_2g`, `c3g`, `c4g`, and related monobactam-active entries but not `carb_group1`/`carb_group2`, while KPC and NDM/VIM (rows 6–7) compromise the carbapenem classes as well, for example.
  ...
  | Narrow-spectrum Gram-negative penicillinase | `enzyme_narrow_spectrum_gram_negative_penicillinase` | Policy-scale inhibitor-susceptible route representing TEM-1 and, where explicitly assigned, related ROB/BRO enzymes | `penicillin_g`, `ampicillin`, `amoxicillin`, `piperacillin`, `ticarcillin` | Reviewed Enterobacterales/enteric hosts, *N. gonorrhoeae*, *H. influenzae*, *M. catarrhalis* |
   ...
   | AAC/APH/ANT | `enzyme_aac_aph` | Aminoglycoside-modifying enzymes | `gentamicin`, `tobramycin`, `amikacin` | Enterobacterales, Nonfermenters, Enteric Pathogens, Fastidious, Staphylococci, Streptococci |
...
Each mechanism a bacterium has **reduces** drug efficacy by a specific amount. The "enhancement multiplier" (0.0–1.0) represents by **how much** a drug's effectiveness is reduced:
...
There are 46 mechanisms × 39 drug classes = 1,794 raw mechanism-class values. A non-zero raw value affects a simulated bacterium-drug pair only when both the bacterium host gate and the drug-specific `mechanism_applies_to_drug` gate admit that mechanism. Non-applicable fallback values are inert. The table below shows the **global default** multiplier for the major mechanisms discussed most often in the text (used when a specific per-class value has not been directly specified):
...
De novo resistance emergence is parameterised directly at the **bacterium-mechanism** level. Each organism therefore has its own baseline emergence profile across the various mechanisms. The route status described above determines whether a zero means an excluded host, an HGT-only route, or an eligible non-transferable mechanism with no de novo attempt; the numerical rate alone is not used as a host-eligibility flag.
...
**Run-level pathway sensitivity controls.** Four neutral `run_pathway_*` multipliers are retained for diagnostic ablations and global sensitivity analysis without rewriting organism-specific baselines. Their defaults are all 1.0: `run_pathway_infection_de_novo_multiplier` scales de novo emergence during active infection; `run_pathway_hgt_multiplier` scales horizontal gene transfer; `run_pathway_reversion_rate_multiplier` scales mechanism loss in infection and carriage; and `run_pathway_microbiome_acquisition_multiplier` scales resistance-profile inheritance when new carriage is acquired. These controls are not a predetermined calibration parameter set; any future calibration role should be justified by the target review and an identifiability assessment. Separately, policy branches can set `counterfactual_resistance_multiplier` to 0.0. In the current rules engine, that scenario multiplier gates or scales profile-sampled resistance, local persistence, exogenous floors and ratchet reseeding, MDR-TB rifampicin seeding, same-person carriage inheritance, infection-carriage spillover, de novo emergence, microbiome-acquisition seeding, HGT transfer, and AST reporting-error draws.
...
At finite simulated population sizes, an uncommon but genuinely established mechanism can disappear from every active infection and every retained profile in a local reservoir by chance. The model represents established strains in the much larger off-model population through a permanent local establishment archive rather than through the former universal per-drug acquisition guard. The archive contributes bounded sampling mass to the local human reservoir; it is not a hard lower bound on observed resistance prevalence.

| Parameter | Default | Function |
|-----------|--------:|----------|
| `local_mechanism_persistence_enabled` | 1.0 | Enables local historical establishment and pseudo-reservoir sampling |
| `local_mechanism_persistence_virtual_profile_mass` | 10.0 | Total virtual mass shared across all deduplicated archived genotypes in a local slot |
| `local_mechanism_persistence_max_sampling_probability` | 0.10 | Maximum archive share of any one local human-reservoir profile draw |

**Establishment and archive composition.** At each cache refresh, the first complete observed genotype carrying each mechanism is archived for that exact region × care setting × bacterium slot. The archive is permanent but remains separate from the active profile reservoir. Identical archived genotypes are deduplicated, so one observed genotype carrying several mechanisms is still one candidate. Later observations do not replace the first establishment genotype.

**Pseudo-reservoir sampling.** Whenever an infection or carriage acquisition is already taking the human-reservoir profile-sampling path, the model computes one archive probability from the total virtual mass and the size of the active candidate pool. All archived genotypes share that single mass; on success, one candidate is selected uniformly and its complete genotype is returned. The model does not repeat the draw for every drug or mechanism and does not combine archived bits with an independently sampled active genotype. The probability cap prevents the archive from dominating very sparse local caches.

**Scope and interpretation.** Establishment does not cross regions, bacteria, or community/hospital strata. The pathway is not used on the exogenous source branch. It follows the same resistance-counterfactual gate as ordinary profile inheritance and can contribute to both active infection and carriage. Archived profiles do not count towards active-cache prevalence, prescribing knowledge, or ratchet peak calculations. Once incorporated into an active infection, however, the genotype can re-enter the ordinary daily cache through the same majority-profile collection process as any other circulating strain.

**Diagnostics.** `local_persistence_profile_incorporations_total` reports the sum of `local_persistence_profile_incorporations_infection` and `local_persistence_profile_incorporations_carriage` in each output timestep. The infection count is incremented only after the infection survives the existing-therapy prevention check; the carriage count is incremented after the archived profile is recorded in the carriage compartment. Counterfactual-rejected profiles are not counted.

The former `resistance_floor_*` configuration and `calculate_resistance_floor` per-drug loop have been removed. That loop repeated a nominal probability across eligible drug entries, was not local or extinction-triggered, and could therefore exert strong pressure unrelated to its finite-population rationale. Static environmental floors and the dynamic ratchet remain independent exogenous pathways (Sections 7.7 and 7.8).
...
**Motivation.** Even with long retention of previously observed profiles, a finite stored profile library can under-represent historically established resistance after current selection pressure falls. The ratchet preserves a coarse regional memory of high previously sampled community-cache prevalence and uses that memory to support later local exogenous acquisition of mechanisms with sufficiently low configured reversion rates.
...
**Implementation.** Once per simulated year, after that day's profile-cache update, the model computes marginal mechanism prevalence separately in every region × bacterium community-profile slot. Hospital profiles are excluded because the ratchet acts only on exogenous community acquisitions. A regional bacterium slot must contain at least 100 retained community profiles before that annual observation can increase its permanent peak; evidence from different regions is not pooled to meet this threshold. Every retained entry in the qualifying local slot contributes equally. The `peak_mechanism_prevalence[region][bacteria][mechanism]` table is updated upward-only from qualifying annual snapshots: once a local sampled peak has been recorded, it is not allowed to fall. Exogenous acquisition reads only the current region's peak. Genuinely global off-model importation must therefore be represented by an explicit static exogenous probability rather than inferred from another region's history.
...
capped at 0.50. Here `ratchet_floor` is a probability in the exogenous assignment loop. For example, a recorded local peak of 12% yields a 10% assignment probability and a peak of 23% yields a 20% probability in that region. The per-region 100-profile evidence threshold prevents a handful of retained profiles from establishing permanent memory. Annual sampling and 10-point steps further reduce sensitivity to small fluctuations, but they do not eliminate sampling noise; because the peak is upward-only, a qualifying sampled threshold crossing has a lasting effect.
...
In conceptual terms, a static environmental floor is a configured exogenous reseeding probability, whereas the ratchet is a history-derived exogenous reseeding probability based on prior cache prevalence. The effective draw in a policy branch is then multiplied by `counterfactual_resistance_multiplier`.

**Relationship to static floors.** Era-specific static environmental probabilities can provide exogenous seeding during configured historical periods, including periods when the finite profile cache might otherwise build a resistant reservoir slowly. The ratchet has no universal takeover date. At each eligible exogenous acquisition, the model uses the larger of the static and ratchet probabilities before applying scenario-level counterfactual scaling. For example, if a *Shigella* mechanism has a current static probability of 0.45 and a recorded cache peak that produces a ratchet probability of 0.40, the ordinary-branch assignment probability remains 0.45. Neither value implies 45% or 40% total resistance prevalence.

**Diagnostic switch.** `run_pathway_ratchet_enabled` is 1.0 by default and preserves the ratchet behaviour described above. Setting it to 0.0 disables only the dynamic ratchet contribution for component-ablation runs; static environmental floors and the independently configured local persistence archive remain active.
...
4. **Co-selection / linked profiles:** use when the missing feature is not the amount of resistance generated, but the fact that mechanisms tend to arrive together on lineages, integrons, cassettes, or plasmids. Complete profile sampling is the model's current representation of this joint structure. If that proves insufficient, an extension should preserve empirically supported joint or conditional profiles for the relevant organism and setting; independently completing a sampled profile from globally observed marginal mechanisms is not an appropriate calibration lever.

**Which lever to prefer.** The intended order of thought is: first ask whether the mechanism needs a history-derived exogenous reseeding probability (ratchet); if not, ask whether it is repeatedly imported from outside the modelled human pool (static environmental probability); if not, ask whether the model is missing a real source of selection pressure (additional off-model drug use); and only then ask whether the main problem is missing correlation structure between mechanisms (co-selection or linked profiles). This keeps the model aligned with its purpose as a policy model rather than pushing it towards unnecessary (and costly) molecular detail.
...
| Respiratory (latent proxy) | MDR *M. tuberculosis* | Uses the general respiratory carriage compartment as a compressed latent-reservoir proxy; the current code does not implement a separate LTBI reactivation hazard or detailed tuberculosis natural-history model |
...
|---------|-----------|-------|---------------|
| Resistance profile sampling on acquisition | `run_pathway_microbiome_acquisition_multiplier` | 1.0 | Scales the probability that a new carriage episode samples a resistance profile from the local circulating-strain library. Hospital carriage uses this gate directly; community carriage also applies the per-bacterium community dilution factor. A value of 1.0 is neutral rather than an independent 50% colonisation bottleneck. |
...
| Carrier → infection bridge | `carrier_resistance_inheritance_probability` | 0.50 | When a new same-organism infection is established in a carrier, each mechanism in `mechanism_microbiome` is independently considered for transfer to `mechanism_any` (see Section 3.3). The person-level gate is scaled by `counterfactual_resistance_multiplier`. |
| Infection ↔ microbiome transfer | `microbiome_resistance_transfer_probability_per_day` | 0.0001 | When both compartments are present and contain different mechanisms, a daily random probability check can trigger copying in both directions so that `mechanism_any` and `mechanism_microbiome` end up containing the combined profile. The drug-level resistance measures are then recalculated. The draw is scaled by `counterfactual_resistance_multiplier`. |
| HGT into the microbiome | (see Section 9) | — | When a horizontal gene transfer event fires, the mechanism is written only to recipient compartments where that bacterium is present: `mechanism_microbiome` for carriage and `mechanism_any` for active infection. HGT is scaled by `counterfactual_resistance_multiplier`. |
...
When an HGT event occurs, the mechanism must be classified as transferable and its executable host status must permit both the donor and recipient bacteria. A zero de novo emergence rate does not block receipt when the pair is `HgtOnly`; `ExcludedHost` does block both donation and receipt. Donor mechanism presence and active-infection majority status are snapshotted before the day's HGT pair evaluations begin. A mechanism received during that phase can therefore be donated from the following simulation day, but cannot cascade through further bacteria on the same day merely because of bacterium iteration order. The transferred mechanism is recorded in each compartment where the recipient organism is present: `mechanism_any` for an active infection and `mechanism_microbiome` for asymptomatic carriage. If the recipient is present in both, both records are updated. A carriage-only transfer does not create phantom active-infection mechanism state. All HGT rates are scaled by the run-level `run_pathway_hgt_multiplier` (default 1.0) and by `counterfactual_resistance_multiplier`.
...
| Minority donor penalty | hgt_minority_donor_multiplier | 0.20 (×0.20) | If an active-infection donor carries the mechanism outside its dominant strain, or if the mechanism is supplied only by carriage where no majority flag exists, its probability of successful conjugation is penalised by 80%. |
...
|--------------|-------------------------------|----------------|----------------------------------------------|------------------|
| Infection deaths | 6.4 million model-scope bacterial infection deaths per year | Ikuta KS et al., 2022; Murray CJL et al., 2022; GBD 2021 AMR Collaborators, 2024 | Derived calibration benchmark | Like-for-like target for the Figure 1 person-level numerator: sepsis plus non-sepsis infection deaths with at least one contributing infection other than H. pylori or MDR-TB. A concurrent excluded infection does not remove an otherwise in-scope death. The target is the rounded sum of per-organism mortality targets after those exclusions, so broader bacterial mortality estimates can be higher. |
...
| Per-bacteria mortality | Per-organism death targets in the calibration summary | Ikuta KS et al., 2022; Murray CJL et al., 2022; WHO Global Tuberculosis Report 2025; Plummer M et al., 2015; Troeger C et al., 2018; Guh AY et al., 2020 | Mix of direct estimates, modelled burden estimates, and derived benchmarks | Pathogen attribution differs between sources, and polymicrobial attribution can make summed organism deaths exceed headline infection deaths. |
...
| Bacteria/drug resistance benchmarks | Evidence-informed prevalence benchmarks and expert-assigned conditional mean-`any_r` model benchmarks | World Health Organization Global antibiotic resistance surveillance report, 2025; World Health Organization GLASS dashboard, accessed 2026; ECDC EARS-Net, 2024; CDC AR Threats, 2019; CDC COVID-19 Special Report, 2022; Murray CJL et al., 2022; WHO Global Tuberculosis Report 2025; Unemo M et al., 2021; WHO STI fact sheet, 2025 | Transformed calibration benchmarks and model-design benchmarks, not a matrix of direct observed estimates | The legacy prevalence matrix was informed by surveillance and organism-specific literature, but cell-level citations and harmonised denominator definitions were not retained. Routine clinical isolates also overrepresent tested, severe, and healthcare-associated infections compared with all simulated infections. |
...
automated tests can detect drift from the canonical target files; it does not replace those files
and its bounds do not enter the calibration score.

For the headline metrics, the first registry version uses 4.7-8.5 million model-scope infection
deaths, 70-150 million people taking antibiotics on an average day, 10-25% annual bacterial
infection incidence, and 20-47 million incident bacterial sepsis cases. The infection-death range
scales the published 5.7-10.2 million uncertainty interval for the 7.7 million GBD estimate to the
model's 6.4 million scope-adjusted central value. The sepsis range combines the published all-cause
sepsis interval with an explicit broad assumption about the bacterial share. The antibiotic-use
and all-bacteria-incidence bounds remain expert plausible ranges because their model numerators
have no direct global observed counterpart.

For target rows without a recoverable interval, the registry applies documented broad range tiers
and explicit organism-level overrides where the canonical target note contains a quantitative
...
ceftolozane/tazobactam; they replace legacy values that had simply duplicated the corresponding
prevalence targets. Five 0.60 values paired with zero prevalence benchmarks are explicit
...
reports configured, score-eligible, and run-usable row counts plus raw score weight by provenance
...
relative evidential confidence. A SHA-256 manifest binds the long-form targets, source table,
schema, legacy wide matrices, Rust-derived potency matrix, and Rust-derived resistance-
reachability matrix; analysis stops if those checked artifacts disagree.

The resistance target file already provides `uncertainty_lower` and `uncertainty_upper` fields,
...
presented as evidence-based uncertainty. Figure 2 therefore continues to show point calibration
...
acquisition utility are generated pattern datasets under this same contract; their names describe
the intended schema, not downloaded observations.
...
### 12.13 Residual high-emergence-rate mechanisms as reviewed coarse-graining
...
| Type | Description |
|------|-------------|
| `AtInfectionCommunity` | Complete resistance profile acquired with a new infection from the circulating or exogenous acquisition pathways |
...
`NorthAmerica`, `SouthAmerica`, `Africa`, `Asia`, `Europe`, `Oceania`, `Home` (fallback)
...
| enterococcus_faecium | -17.7 | -2 | 0.5 | 6 | 0.01 | 0.48 | 5 | 0.008 | 9.6 | yes |
...
| enterococcus_faecalis | -5.2 | 0.93 | 0.005 | 0 | 0 | 4e-4 | 0.8 | 75 | 0.1 |
| enterococcus_faecium | -4.2 | 0.93 | 0.005 | 0 | 0 | 4e-4 | 0.8 | 75 | 1.5 |
| escherichia_coli | -10.5 | 0.93 | 0.005 | 0 | 0 | 4e-4 | 0.75 | 50 | 0.1 |
| klebsiella_pneumoniae | -8.1 | 0.93 | 0.005 | 0 | 0 | 4e-4 | 0.45 | 75 | 0.1 |
| morganella_spp. | -7.1 | 0.93 | 0.005 | 0 | 0 | 4e-4 | 0.35 | 65 | 0.1 |
| proteus_spp. | -6.8 | 0.93 | 0.005 | 0 | 0 | 4e-4 | 0.35 | 65 | 0.1 |
...
| staphylococcus_aureus | -9.3 | 0.93 | 0.005 | 0 | 0.41 | 4e-4 | 0.8 | 65 | 0.1 |
| staphylococcus_epidermidis | -7.3 | 0.04 | 0.005 | -6 | 0 | 4e-4 | 0.5 | 65 | 0.1 |
| streptococcus_pneumoniae | -9.9 | 0.93 | 0.005 | 0 | 0 | 4e-4 | 0.95 | 50 | 0.1 |
...
| shigella_spp. | -22.5 | 0.93 | 0.005 | 1 | 0 | 4e-4 | 0.72 | 25 | 0.1 |
...
| mycoplasma_pneumoniae | -18 | 0.93 | 0.005 | -0.7 | 0 | 4e-4 | 1 | 50 | 0.1 |
...
| enterococcus_faecium | target_site_cfr | 30 | eligible; de novo enabled |
...
| streptococcus_pneumoniae | enzyme_cat | 15 | eligible; de novo enabled |
...
| streptococcus_pneumoniae | protection_tet_m | 0.25 | eligible; de novo enabled |
...
| streptococcus_pneumoniae | mutation_pbp_mosaic | 1e-7 | eligible; de novo enabled |
...
| streptococcus_pyogenes | mutation_gyra_primary | 0.1 | eligible; de novo enabled |
| streptococcus_pyogenes | mutation_gyra_parc_secondary | 0.1 | eligible; de novo enabled |
| streptococcus_pyogenes | protection_qnr | 0 | excluded host |
| streptococcus_pyogenes | enzyme_16s_rrmt | 0 | excluded host |
| streptococcus_pyogenes | target_site_erm_b | 0.05 | eligible; de novo enabled |
| streptococcus_pyogenes | target_site_cfr | 0.2 | eligible; de novo enabled |
...
| streptococcus_pyogenes | global_efflux_pump | 0.2 | eligible; de novo enabled |
| streptococcus_pyogenes | mutation_folate_pathway | 1 | eligible; de novo enabled |
...
| streptococcus_pyogenes | mutation_mpr_f | 0.01 | excluded host |
| streptococcus_pyogenes | mutation_liafsr_cls | 0 | excluded host |
| streptococcus_pyogenes | mutation_rpo_b | 0.03 | eligible; de novo enabled |
| streptococcus_pyogenes | protection_fus_b | 0.1 | excluded host |
| streptococcus_pyogenes | protection_tet_m | 0.003 | eligible; de novo enabled |
...
| streptococcus_pyogenes | mutation_23s_rrna | 0.1 | eligible; de novo enabled |
...
| streptococcus_agalactiae | mutation_gyra_primary | 0.3 | eligible; de novo enabled |
| streptococcus_agalactiae | mutation_gyra_parc_secondary | 0.3 | eligible; de novo enabled |
| streptococcus_agalactiae | protection_qnr | 0 | excluded host |
| streptococcus_agalactiae | enzyme_16s_rrmt | 0 | excluded host |
| streptococcus_agalactiae | target_site_erm_b | 15 | eligible; de novo enabled |
| streptococcus_agalactiae | target_site_cfr | 15 | eligible; de novo enabled |
...
| streptococcus_agalactiae | mutation_folate_pathway | 3 | eligible; de novo enabled |
...
| burkholderia_cepacia_complex | enzyme_esbl_ctx_m | 0.001 | eligible; de novo enabled |
| burkholderia_cepacia_complex | enzyme_esbl_tem | 0.001 | eligible; de novo enabled |
| burkholderia_cepacia_complex | enzyme_esbl_shv | 0.001 | eligible; de novo enabled |
| burkholderia_cepacia_complex | enzyme_kpc | 3e-4 | eligible; de novo enabled |
| burkholderia_cepacia_complex | enzyme_ndm_vim | 3e-4 | eligible; de novo enabled |
| burkholderia_cepacia_complex | enzyme_oxa_48 | 3e-4 | eligible; de novo enabled |
| burkholderia_cepacia_complex | enzyme_ampc_cmy | 0.001 | eligible; de novo enabled |
| burkholderia_cepacia_complex | enzyme_ampc_dha | 0.001 | eligible; de novo enabled |
...
| burkholderia_cepacia_complex | mutation_gyra_primary | 0.15 | eligible; de novo enabled |
...
| burkholderia_cepacia_complex | enzyme_16s_rrmt | 5 | eligible; de novo enabled |
...
| burkholderia_cepacia_complex | enzyme_cat | 5e-4 | eligible; de novo enabled |
...
| burkholderia_cepacia_complex | modification_mcr_1 | 0.015 | eligible; de novo enabled |
| burkholderia_cepacia_complex | mutation_polymyxin_regulatory | 0 | eligible; no de novo or HGT |
| burkholderia_cepacia_complex | global_efflux_pump | 0.05 | eligible; de novo enabled |
...
| burkholderia_cepacia_complex | enzyme_fos | 0.015 | eligible; de novo enabled |
...
| burkholderia_cepacia_complex | mutation_rpo_b | 0.005 | eligible; de novo enabled |
| burkholderia_cepacia_complex | protection_fus_b | 0 | excluded host |
| burkholderia_cepacia_complex | protection_tet_m | 5e-5 | eligible; de novo enabled |
| burkholderia_cepacia_complex | enzyme_aac_aph | 5 | eligible; de novo enabled |
...
| burkholderia_cepacia_complex | efflux_tet_abc | 5e-5 | eligible; de novo enabled |
...
| burkholderia_cepacia_complex | mutation_siderophore_uptake | 5e-5 | eligible; de novo enabled |
...
Every current file contains `simulation_summary_schema_version`; this document describes
version `1`. The Python analysis accepts only the schema version it was written for. An
unversioned or differently versioned file must be analysed with its matching repository
revision or regenerated, rather than interpreted through name-based fallbacks.
...
|--------|------|-------------|
| `time_step` | int | Simulation day (0-indexed) |
| `simulation_summary_schema_version` | int | Output contract version; currently `1` |
| `policy_option` | int | Policy branch identifier for the row; baseline rows use `0` |
| `run_id` | int | Numeric run identifier written into the summary file |
...
| `number_severely_immunosuppressed` | int | Living people currently in a temporary or chronic immunodeficiency state |
...
| `infected_10_days_count`, `infected_21_days_count` | int | Living people whose current infection duration has reached the named threshold |
| `total_with_resistance` | int | Living people with any active infection resistance flag |
| `currently_taking_drug_count` and `currently_taking_drug_count_*` | int | Living people receiving antibiotics, including empiric, targeted, prophylaxis, other, and other-context subsets |
| `currently_infected_and_on_drug_count` | int | Living people with a reportable infection and at least one active antibiotic |
| `taking_two_drugs_count` | int | Living people currently taking exactly two antibiotics |
...
| `infection_acquisition_people_with_any_r_count`, `infection_acquisition_people_with_serious_r_count`, `infection_acquisition_people_serious_r_marker_eligible_count` | int | Person-level acquisition counts stratified by acquisition-time resistance marker state |
| `infection_acquisition_people_past_year` | int | Rolling one-year count of people with successful infection acquisition |
| `local_persistence_profile_incorporations_total` | int | Complete archived local profiles incorporated into active infection or carriage acquisitions |
| `local_persistence_profile_incorporations_infection`, `local_persistence_profile_incorporations_carriage` | int | Local-profile incorporations split by active-infection and carriage acquisition pathways |
| `new_drug_initiations_count` | int | New antibiotic courses started during the day |
| `new_drug_initiations_with_active_infection_count` | int | New course starts in people with a reportable infection above `INFECTION_EPS` at the pre-rule snapshot |
| `diagnostic_cascade_*` | int | Person-level diagnostic and targeted-treatment cascade counts, split by all settings plus community/hospital subsets |
| `sepsis_onset_*` and `sepsis_effective_therapy_*` | int | Sepsis onset context and effective-therapy timing counters |
...
| `total_deaths`, `deaths_background`, `deaths_sepsis`, `deaths_infection_non_sepsis`, `deaths_drug_toxicity` | int | Mutually exclusive all-cause and cause-specific daily death counts |
| `deaths_past_year` and `deaths_*_past_year` | int | Rolling one-year all-cause and cause-specific death counts |
| `num_age_*` | int | Living population counts in exported age bands |
| `num_with_any_bacteria_microbiome` | int | Living people with at least one bacterium present in the carriage/microbiome state |
| `people_on_1_drug`, `people_on_2_drugs`, `people_on_3plus_drugs` | int | Current treatment-intensity stocks |
| `active_infection_on_drug_with_previous_failure_count` | int | Active infection and antibiotic exposure count among people with a previous drug-failure flag |
| `people_on_*_drugs_hist` | int | Histogram of current concurrent antibiotic counts |
...
| `{bacteria}_infections_prevented_by_drug` | Prevention events attributed to antibiotic prophylaxis/activity |
| `{bacteria}_deaths` and `{bacteria}_deaths_*` | Bacterium-attributed deaths and age/care-setting subsets |
...
| `{bacteria}_infection_acquisition_events_with_any_r_hospital` | Hospital-acquired events with any-R snapshotted at acquisition |
| `{bacteria}_infection_acquisition_events_with_any_r_community` | Community-acquired events with any-R snapshotted at acquisition |
| `{bacteria}_presence_microbiome`, `{bacteria}_presence_microbiome_resistant`, and living tier suffixes | Current carriage/microbiome stocks and resistance-tier summaries |
| `{bacteria}_microbiome_acquisitions_*`, `{bacteria}_microbiome_clearances_*`, and clearance-category suffixes | Carriage acquisition, clearance, and resistant-clearance event families |
| `{bacteria}_infected_carrier_count`, `{bacteria}_infected_non_carrier_count`, and resistant/location splits | Active infection stocks split by carrier status, current care setting, and resistance state |
| `{bacteria}_drug_failure_events_{region}` and `{bacteria}_drug_treatment_day5_events_{region}` | Region-stratified treatment failure and day-5 treatment evaluation events |
| `{bacteria}_infected_with_test_identified`, `{bacteria}_infected_with_test_for_resistance` | Active infection stocks with diagnostic identification and resistance-testing state |
| `{bacteria}_infected_and_on_any_drug` | Active infections under any antibiotic exposure |
| `{bacteria}_infected_with_{mechanism}` and mechanism-family vector cells | Mechanism-attributed active infection summaries |
| `{bacteria}_infection_resolution_*` | Resolution events by immune clearance, drug-assisted clearance, and death cause |
| `{bacteria}_day_7_evaluations`, `{bacteria}_day_7_drug_used` | Day-7 treatment-review counters |
| `{bacteria}_syndrome_{id}_infected` | Active infection stock by syndrome |
| `{bacteria}_drug_selection_count` | Count of drug-selection evaluations for the bacterium |
...
| `*_by_bacteria` vector cells | Semicolon-delimited vectors in `BACTERIA_LIST` order |
...
| Pattern | Description |
|---------|-------------|
| `{bacteria}_infected_and_mic_lt2_{drug}` and `{bacteria}_sum_mic_{drug}` | Legacy reciprocal-activity proxy count and sum by organism-drug pair |
| `{bacteria}_currently_on_drug_{drug}` | Drug exposure by organism-drug pair |
| `{bacteria}_microbiome_r_positive_{drug}` | Carriage/microbiome resistance positivity by organism-drug pair |
| `{bacteria}_sum_any_r_{drug}` and `{bacteria}_sum_any_r_hospital_{drug}` | Mechanism-derived any-R sums by organism-drug pair and hospital subset |
| `{bacteria}_infected_with_any_r_positive_{drug}` and hospital/community splits | Active infection resistance counts by organism-drug pair and current care setting |
| `{bacteria}_drug_score_sum_{drug}` | Accumulated drug-selection score contributions by organism-drug pair |
...
| Pattern | Description |
|---------|-------------|
| `{region}_population` | Living population in the region |
| `{region}_hospital_population` | Living hospitalised population in the region |
| `{region}_{death_type}` | Regional death counts by background, sepsis, non-sepsis infection, and drug-toxicity cause |
| `{region}_{age_group}_{death_type}` | Regional age-by-cause death counts |
| `{region}_{drug}_currently_on_drug` | Current regional drug exposure |
| `syndrome_{id}_population_{region}` | Regional active syndrome stock |
| `syndrome_{id}_deaths_sepsis_{region}` and `syndrome_{id}_deaths_infection_non_sepsis_{region}` | Regional syndrome-attributed infection death counts |



### C.4 Total Column Count

The CSV contains thousands of columns. The exact schema is generated by
`Simulation::export_summary_to_csv` and guarded by `tests/csv_invariants.rs`; any semantic
or structural change requires a schema-version decision and matching Python updates.
...
- A successful acquisition contributes one event to the relevant bacterium totals. The
  person-level acquisition scalar contributes at most one count per person-day.
- Acquisition attributes (resistance, carrier status, syndrome, care setting, and region) are
  snapshotted at acquisition, rather than reconstructed from mutable end-of-day state.
...
- Applied-activity numerator and denominator fields are captured together at the point where
  ...
  even when the prescribing-course flag has already ended.
...
- Vector-valued `*_by_bacteria` cells follow `BACTERIA_LIST` order. Their field name states
  whether entries are transition events, person-days, or another unit.
...
This appendix documents the person-level state and scientifically
meaningful person-level intermediate quantities used by the executable model.

The appendix uses two linked tables:

1. **Table D1** is a compact dictionary of meaning, lifetime, shape, and reset
   conventions.
2. **Table D2** is the corresponding update-rule catalogue of direct state
   inputs, configured parameters, timing, exceptions, and code authority.

The inclusion rules are:

- one conceptual row for an indexed family rather than one row per bacterium,
  drug, syndrome, or resistance mechanism;
- separate exception notes where indexed members follow materially different
  rules;
...
- rule-local values only when they have a model interpretation and directly
  control a transition, probability, hazard, activity, classification, or
  outcome.

Loop indices, aliases, buffers, counters, and cached parameter lookups are not
individual-level model variables and are excluded.

### D.1 Notation and Scope

- `b`, `d`, and `m` denote bacterium, drug, and resistance-mechanism indices.
...
- "Temporary" means recalculated during an update and not stored on
  `Individual`.
- `MISSING_EVENT_DATE` is `-1`; drug dates that use `i32::MIN` are identified
  explicitly.
- `INFECTION_EPS` is the common small positive threshold used to distinguish
  active infection or exposure from numerical zero.

### D.2 Table D1. Compact Variable Dictionary

| Variable | Lifetime and shape | Meaning and units | Initial value or sentinel | Reset or terminal behaviour | Process group |
|---|---|---|---|---|---|
| [`id`](#rule-id) | Persistent scalar | Unique individual identifier. | Assigned from `0` to population size minus one. | Never reset. | Demography |
| [`age`](#rule-age) | Persistent scalar | Age in days; negative values represent future cohort members. | Supplied at construction. | Increments daily while entering or active; frozen after death. | Demography |
| [`sex_at_birth`](#rule-sex-at-birth) | Persistent scalar category | Biological sex at birth, currently `"male"` or `"female"`. | Supplied at construction. | Never reset. | Demography |
| [`perceived_penicillin_allergy`](#rule-perceived-penicillin-allergy) | Persistent Boolean | Whether the person reports a penicillin allergy; used in drug selection. | Bernoulli draw at construction. | Never reset. | Demography and prescribing |
| [`region_living`](#rule-region-living) | Persistent scalar category | Home region. | Assigned during population construction. | Stable home region. | Location |
| [`region_cur_in`](#rule-region-cur-in) | Persistent scalar category | Current region, which may differ during travel. | Home region. | Returns to `region_living` when travel ends. | Location |
| [`days_visiting`](#rule-days-visiting) | Persistent scalar count | Days elapsed in the current non-home visit. | `0`. | Reset to `0` on return home. | Location |
| [`hospital_status`](#rule-hospital-status) | Persistent enum | Current inpatient status. | `NotInHospital`. | Changes on admission or discharge; frozen after death. | Care location |
| [`days_hospitalized`](#rule-days-hospitalized) | Persistent scalar count | Days in the current admission. | `0`. | Reset to `0` on admission and discharge. | Care location |
| [`date_last_infected[b]`](#rule-date-last-infected) | Persistent bacterium family | Start day of the current infection. | `MISSING_EVENT_DATE`. | Reset on infection clearance. | Active infection |
| [`date_last_infected_keep[b]`](#rule-date-last-infected-keep) | Persistent bacterium family | Most recent infection start day retained after clearance. | `MISSING_EVENT_DATE`. | Not reset by clearance. | Infection history |
| [`infectious_syndrome[b]`](#rule-infectious-syndrome) | Persistent bacterium family | Clinical syndrome ID for the active episode; IDs 1-10 are modelled. | `0`, meaning none. | Reset to `0` on clearance. | Active infection |
| [`level[b]`](#rule-level) | Persistent bacterium family | Non-negative unitless infection intensity. | `0`; set to a bacterium-specific value at acquisition. | Reset to `0` on clearance. | Active infection |
| [`predicted_infection_risk[b]`](#rule-predicted-infection-risk) | Persistent bacterium family | Current daily probability of acquiring infection. | `0`. | Recalculated daily and retained as a reporting snapshot. | Infection acquisition |
| [`clearance_hazard[b]`](#rule-clearance-hazard) | Persistent bacterium family | Daily immune-clearance probability, from `0` to `1`. | `0`. | Recalculated during active infection; reset on clearance. | Active infection |
| [`clearance_ready_day[b]`](#rule-clearance-ready-day) | Persistent bacterium family | First day on which immune clearance is eligible. | `-1`. | Set at acquisition or backfilled; reset on clearance. | Active infection |
| [`sepsis[b]`](#rule-sepsis) | Persistent bacterium family | Whether the infection is in a sepsis or other life-threatening state. | `false`. | Cleared when infection ends or sepsis recovers. | Sepsis |
| [`sepsis_onset_day[b]`](#rule-sepsis-onset-day) | Persistent bacterium family | Day on which the current or last sepsis episode began. | `-1`. | Set at onset; not cleared by sepsis recovery. | Sepsis |
| [`sepsis_episode_open[b]`](#rule-sepsis-episode-open) | Persistent bacterium family | Reporting tracker for an unresolved incident-sepsis episode. | `false`. | Closed after its outcome is classified. | Sepsis reporting |
| [`sepsis_episode_context_at_onset[b]`](#rule-sepsis-episode-context-at-onset) | Persistent bacterium family | Encoded antibiotic-use context at sepsis onset. | `0`. | Overwritten at the next episode onset. | Sepsis reporting |
| [`sepsis_episode_best_activity_at_onset[b]`](#rule-sepsis-episode-best-activity-at-onset) | Persistent bacterium family | Best active-drug `activity_r` immediately before onset. | `0`. | Overwritten at the next episode onset. | Sepsis reporting |
| [`sepsis_episode_effective_at_onset[b]`](#rule-sepsis-episode-effective-at-onset) | Persistent bacterium family | Whether therapy met the effective-activity threshold at onset. | `false`. | Overwritten at the next episode onset. | Sepsis reporting |
| [`sepsis_episode_first_effective_day[b]`](#rule-sepsis-episode-first-effective-day) | Persistent bacterium family | First day effective therapy was observed for the open episode. | `-1`. | Reset when a new episode opens. | Sepsis reporting |
| [`sepsis_episode_delay_bucket_recorded[b]`](#rule-sepsis-episode-delay-bucket-recorded) | Persistent bacterium family | Guard preventing duplicate effective-therapy delay classification. | `false`. | Reset when a new episode opens. | Sepsis reporting |
| [`sepsis_episode_region_at_onset[b]`](#rule-sepsis-episode-region-at-onset) | Persistent bacterium family | Encoded home region captured at onset. | `-1`. | Overwritten at the next episode onset. | Sepsis reporting |
| [`sepsis_episode_hospitalized_at_onset[b]`](#rule-sepsis-episode-hospitalized-at-onset) | Persistent bacterium family | Whether the person was hospitalised at onset. | `false`. | Overwritten at the next episode onset. | Sepsis reporting |
| [`sepsis_episode_age_group_at_onset[b]`](#rule-sepsis-episode-age-group-at-onset) | Persistent bacterium family | Encoded age group captured at onset. | `-1`. | Overwritten at the next episode onset. | Sepsis reporting |
| [`diagnostic_cascade_open[b]`](#rule-diagnostic-cascade-open) | Persistent bacterium family | Reporting tracker for an infection eligible for diagnostic-cascade follow-up. | `false`. | Closed when the infection episode ends. | Diagnostic reporting |
| [`diagnostic_cascade_entry_time_step[b]`](#rule-diagnostic-cascade-entry-time-step) | Persistent bacterium family | Day on which cascade eligibility began. | `-1`. | Reset when a new cascade episode opens. | Diagnostic reporting |
| [`diagnostic_cascade_entry_hospitalized[b]`](#rule-diagnostic-cascade-entry-hospitalized) | Persistent bacterium family | Hospital status captured at cascade entry. | `false`. | Overwritten at the next cascade entry. | Diagnostic reporting |
| [`diagnostic_cascade_bacterial_identification_recorded[b]`](#rule-diagnostic-cascade-bacterial-identification-recorded) | Persistent bacterium family | Guard preventing duplicate identification-stage counts. | `false`. | Reset when a new cascade episode opens. | Diagnostic reporting |
| [`diagnostic_cascade_resistance_testing_recorded[b]`](#rule-diagnostic-cascade-resistance-testing-recorded) | Persistent bacterium family | Guard preventing duplicate AST-stage counts. | `false`. | Reset when a new cascade episode opens. | Diagnostic reporting |
| [`diagnostic_cascade_targeted_treatment_recorded[b]`](#rule-diagnostic-cascade-targeted-treatment-recorded) | Persistent bacterium family | Guard preventing duplicate targeted-treatment counts. | `false`. | Reset when a new cascade episode opens. | Diagnostic reporting |
| [`diagnostic_cascade_effective_targeted_treatment_recorded[b]`](#rule-diagnostic-cascade-effective-targeted-treatment-recorded) | Persistent bacterium family | Guard preventing duplicate effective-targeted-treatment counts. | `false`. | Reset when a new cascade episode opens. | Diagnostic reporting |
| [`infection_prevented_by_drug[b]`](#rule-infection-prevented-by-drug) | Persistent bacterium family used as a daily event flag | Whether existing therapy prevented an attempted infection that day. | `false`. | Reset at the start of each day. | Infection acquisition |
| [`presence_microbiome[b]`](#rule-presence-microbiome) | Persistent bacterium family | Whether the bacterium is present in the carriage compartment. | `false`. | Cleared by carriage-clearance rules. | Carriage |
| [`microbiome_disruption_level`](#rule-microbiome-disruption-level) | Persistent scalar | Unitless accumulated ecological disruption from antibiotics. | `0`. | Decays over time but is not event-reset. | Carriage ecology |
| [`date_microbiome_acquired[b]`](#rule-date-microbiome-acquired) | Persistent bacterium family | Day current carriage began. | `MISSING_EVENT_DATE`. | Reset when carriage clears. | Carriage |
| [`microbiome_acquired_today[b]`](#rule-microbiome-acquired-today) | Persistent bacterium family used as a daily event flag | Whether carriage was acquired that day. | `false`. | Cleared after daily aggregation. | Carriage reporting |
| [`microbiome_acquired_on_drug_today[b]`](#rule-microbiome-acquired-on-drug-today) | Persistent bacterium family used as a daily event flag | Whether same-day carriage acquisition occurred under antibiotic exposure. | `false`. | Cleared after daily aggregation. | Carriage reporting |
| [`microbiome_cleared_today[b]`](#rule-microbiome-cleared-today) | Persistent bacterium family used as a daily event flag | Whether carriage cleared that day. | `false`. | Cleared after daily aggregation. | Carriage reporting |
| [`cleared_any_r_microbiome_categories[b][category]`](#rule-cleared-any-r-microbiome-categories) | Persistent bacterium-by-category counter family | Counts resistant infection clearances by concurrent carriage-resistance category. | All counts `0`. | Reset after aggregation. | Resistance reporting |
| [`vaccination_status[b]`](#rule-vaccination-status) | Persistent bacterium family | Whether the person has received a supported vaccine against `b`. | `false`. | Once true, remains true. | Vaccination |
| [`infection_has_caused_symptoms[b]`](#rule-infection-has-caused-symptoms) | Persistent bacterium family | Latched symptom or other clinical-testing indication for the active infection. | `false`. | Reset on infection clearance. | Clinical presentation |
| [`test_identified_infection[b]`](#rule-test-identified-infection) | Persistent bacterium family | Whether bacterial identification has completed for the active infection. | `false`. | Reset on clearance or absence of infection. | Diagnostics |
| [`test_for_resistance[b]`](#rule-test-for-resistance) | Persistent bacterium family | Whether the AST result panel is available. | `false`. | Reset when testing state is cleared. | Diagnostics |
| [`resistance_test_initiated_day[b]`](#rule-resistance-test-initiated-day) | Persistent bacterium family | Day AST was initiated; a non-negative value with no result means pending. | `-1`. | Reset when testing state is cleared. | Diagnostics |
| [`cur_use_drug[d]`](#rule-cur-use-drug) | Persistent drug family | Whether the drug is currently being taken. | `false`. | Set false when the course stops. | Treatment |
| [`drug_use_context[d]`](#rule-drug-use-context) | Persistent drug family enum | Context in which the current course began. | `None`. | Reset to `None` when the drug stops. | Treatment |
| [`cur_level_drug[d]`](#rule-cur-level-drug) | Persistent drug family | Unitless systemic exposure. | `0`. | Held during use, then decays to numerical zero. | Pharmacokinetics |
| [`date_drug_initiated[d]`](#rule-date-drug-initiated) | Persistent drug family | Start day of the current or most recent active course. | `i32::MIN`. | Reset to `i32::MIN` when the course stops. | Treatment |
| [`date_drug_initiated_keep[d]`](#rule-date-drug-initiated-keep) | Persistent drug family | Most recent drug initiation day retained after stopping. | `i32::MIN`. | Not reset on cessation. | Treatment history |
| [`ever_taken_drug[d]`](#rule-ever-taken-drug) | Persistent drug family | Whether the person has ever received the drug. | `false`. | Once true, remains true. | Treatment history |
| [`current_infection_related_death_risk`](#rule-current-infection-related-death-risk) | Persistent scalar reporting snapshot | Combined daily probability of infection-related death. | `0`. | Recalculated daily for a living active person. | Mortality |
| [`background_all_cause_mortality_rate`](#rule-background-all-cause-mortality-rate) | Persistent scalar reporting snapshot | Daily probability of background death. | Small active-person default; `0` before birth. | Recalculated from current age and covariates. | Mortality |
| [`infection_hospital_acquired[b]`](#rule-infection-hospital-acquired) | Persistent bacterium family | Whether the active infection was acquired during hospitalisation. | `false`. | Set at acquisition and reset on clearance. | Infection context |
| [`drug_toxicity_reservoir[d]`](#rule-drug-toxicity-reservoir) | Persistent drug family | Accumulated unitless toxicity burden attributable to each drug. | `0`. | Accumulates under exposure and decays after exposure. | Toxicity |
| [`current_toxicity_hazard`](#rule-current-toxicity-hazard) | Persistent scalar reporting snapshot | Sum of current per-drug toxicity reservoirs before person-level modifiers. | `0`. | Recalculated daily. | Toxicity |
| [`mortality_risk_current_toxicity`](#rule-mortality-risk-current-toxicity) | Persistent scalar reporting snapshot | Daily toxicity-death probability after person-level modifiers. | `0`. | Recalculated daily. | Toxicity and mortality |
| [`resistances[b][d].microbiome_r`](#rule-resistance-microbiome-r) | Persistent bacterium-by-drug family | Carriage resistance severity, bounded by `max_resistance_level`. | `0`. | Derived from `mechanism_microbiome`; cleared with carriage. | Resistance |
| [`resistances[b][d].test_r`](#rule-resistance-test-r) | Persistent bacterium-by-drug family | Completed AST-reported resistance result, from `0` to `1`. | `0`, also used while no result is available. | Reset when AST state is cleared; readiness is tracked separately. | Diagnostics and resistance |
| [`resistances[b][d].activity_r`](#rule-resistance-activity-r) | Persistent bacterium-by-drug family | Effective drug activity after potency, exposure, site penetration, and resistance; not a resistance fraction. | `0`. | Recalculated daily and reset when exposure or infection is absent. | Treatment response |
| [`resistances[b][d].any_r`](#rule-resistance-any-r) | Persistent bacterium-by-drug family | Active-infection resistance severity, bounded by `max_resistance_level`. | `0`. | Derived from `mechanism_any`; reset on infection clearance. | Resistance |
| [`mechanism_any[b]`](#rule-mechanism-any) | Persistent bacterium bitmask family | Mechanisms present in any represented active-infection strain. | Empty mask. | Updated by acquisition, emergence, HGT, and clearance. | Resistance |
| [`mechanism_majority[b]`](#rule-mechanism-majority) | Persistent bacterium bitmask family | Mechanisms in the represented majority infection strain. | Empty mask. | Updated by acquisition, promotion, reversion, and clearance. | Resistance |
| [`mechanism_microbiome[b]`](#rule-mechanism-microbiome) | Persistent bacterium bitmask family | Mechanisms represented in carriage. | Empty mask. | Updated by carriage acquisition, de novo carriage emergence, HGT, infection-carriage transfer, reversion, and clearance. | Resistance and carriage |
| [`how_resistance_acquired[b][d]`](#rule-how-resistance-acquired) | Optional persistent bacterium-by-drug family | Resistance-acquisition provenance category. | Empty matrix when provenance tracking is disabled; otherwise `None`. | Updated only in provenance-enabled runs and cleared with the corresponding compartment. | Optional diagnostics |
| [`infection_resolution_this_timestep[b][resolution]`](#rule-infection-resolution-this-timestep) | Persistent bacterium-by-resolution daily counter family | Counts infection resolutions by immune clearance, drug-assisted clearance, or death pathway. | All counts `0`. | Reset at the start of each day after aggregation. | Outcome reporting |
| [`day_7_since_last_infection_drug_used[b]`](#rule-day-7-since-last-infection-drug-used) | Persistent bacterium family of optional Booleans | Whether any antibiotic began within seven days of infection onset. | `None`. | Set once on day seven; reset for a new infection. | Treatment reporting |
| [`date_of_death`](#rule-date-of-death) | Persistent optional scalar | Simulation day of death. | `None`. | Terminal once set. | Mortality |
...
| [`immunodeficiency_type`](#rule-immunodeficiency-type) | Persistent optional enum | Current temporary or chronic severe immunodeficiency state. | `None`. | Changes through onset and recovery transitions. | Host state |
| [`bacteria_level_at_drug_start[b]`](#rule-bacteria-level-at-drug-start) | Persistent bacterium family of optional values | Infection intensity when the current treatment episode began. | `None`. | Cleared when treatment tracking ends. | Treatment response |
| [`days_on_current_treatment[b]`](#rule-days-on-current-treatment) | Persistent bacterium family count | Days elapsed in the tracked treatment episode. | `-1`, meaning no tracked treatment. | Reset to `-1` when tracking ends. | Treatment response |
| [`treatment_failure_assessed[b]`](#rule-treatment-failure-assessed) | Persistent bacterium family | Whether failure was assessed for the tracked course. | `false`. | Reset when a new tracked course begins or tracking ends. | Treatment response |
| [`drug_activity_response_multiplier[b]`](#rule-drug-activity-response-multiplier) | Persistent bacterium family | Infection-specific scaling of drug-driven level reduction. | Global usual-response multiplier. | Resampled when treatment begins; restored when tracking ends. | Treatment response |
| [`drug_stopped_with_infection_day[b]`](#rule-drug-stopped-with-infection-day) | Persistent bacterium family of optional dates | Day a drug stopped while infection remained active. | `None`. | Cleared after restart-window resolution or treatment reset. | Treatment failure |
| [`bacteria_level_at_drug_cessation[b]`](#rule-bacteria-level-at-drug-cessation) | Persistent bacterium family of optional values | Infection intensity when treatment stopped prematurely. | `None`. | Cleared with restart-window state. | Treatment failure |
| [`bacteria_on_selection_day`](#rule-bacteria-on-selection-day) | Persistent scalar index used as a daily snapshot | Bacterium that triggered the recorded drug-selection decision. | `-1`. | Reset when no selection is recorded. | Prescribing diagnostics |
| [`drug_score_on_selection_day[d]`](#rule-drug-score-on-selection-day) | Persistent drug family used as a daily snapshot | Drug-selection score at the recorded selection decision. | `-1`. | Replaced on selection; otherwise remains the no-selection sentinel. | Prescribing diagnostics |
| [`stopped_drug_index[b]`](#rule-stopped-drug-index) | Persistent bacterium family of optional drug indices | Specific drug stopped while infection remained active. | `None`. | Cleared with restart-window state. | Treatment failure |
| [`restart_window_assessed[b]`](#rule-restart-window-assessed) | Persistent bacterium family | Whether post-cessation restart eligibility has been assessed. | `false`. | Reset when a new cessation event is recorded. | Treatment failure |
| [`date_last_drug_failure[b]`](#rule-date-last-drug-failure) | Persistent bacterium family | Most recent treatment-failure day. | `-1`. | Retained across infection resolution. | Treatment history |
| [`toxicity_stopped_drug_day[d]`](#rule-toxicity-stopped-drug-day) | Persistent drug family | Most recent day the drug stopped because of toxicity. | `i32::MIN`. | Retained to inform future selection. | Toxicity and prescribing |
| [`current_number_of_drugs`](#rule-current-number-of-drugs) | Persistent scalar count | Number of drugs currently in use. | `0`. | Recalculated when courses start or stop. | Treatment |
| [`active_symptomatic_syndromes`](#rule-active-symptomatic-syndromes) | Temporary set for an individual-day | Distinct syndromes among active infections whose symptom indicator has latched. | Rebuilt empty when needed. | Discarded after the daily prescribing decision. | Prescribing context |
| [`under_medical_care`](#rule-under-medical-care) | Temporary Boolean for an individual-day | Proxy for nonspecific medical or supportive care. | Recalculated when needed. | Not stored. | Care context |
| [`severe_symptomatic_infection`](#rule-severe-symptomatic-infection) | Temporary Boolean for an individual-day | Whether any symptomatic active infection exceeds the admission threshold. | Recalculated daily. | Not stored. | Care context |
| [`serious_resistance_test_positive`](#rule-serious-resistance-test-positive) | Temporary Boolean for an individual-day | Whether completed AST shows resistance meeting the serious-resistance definition. | Recalculated daily. | Not stored. | Care context |
| [`immunodeficiency_transition_probability`](#rule-immunodeficiency-transition-probability) | Temporary probability family | Daily onset or recovery probability for temporary and chronic immunodeficiency. | Recalculated daily. | Discarded after the transition draw. | Host state |
| [`hospitalization_probability`](#rule-hospitalization-probability) | Temporary scalar probability | Daily probability of admission for a non-hospitalised person. | Recalculated daily. | Discarded after the admission draw. | Care location |
| [`travel_probability`](#rule-travel-probability) | Temporary scalar probability | Daily probability of beginning a non-home visit. | Recalculated while at home. | Discarded after the travel draw. | Location |
| [`sepsis_onset_probability[b]`](#rule-sepsis-onset-probability) | Temporary bacterium probability family | Daily probability that an active non-septic infection progresses to sepsis. | Recalculated daily. | Discarded after the onset draw. | Sepsis |
| [`antibiotic_initiation_probability`](#rule-antibiotic-initiation-probability) | Temporary scalar probability | Daily probability of starting at least one antibiotic. | Recalculated daily. | Discarded after the initiation draw. | Prescribing |
| [`drug_cessation_probability[d]`](#rule-drug-cessation-probability) | Temporary drug probability family | Daily probability of stopping each active drug. | Recalculated for active courses. | Discarded after the cessation draw. | Treatment |
| [`drug_selection_score[d]`](#rule-drug-selection-score) | Temporary drug family | Relative selection weight after activity, indication, history, resistance, availability, and policy effects. | Rebuilt for each selection event. | Discarded after sampling; optionally copied to the diagnostic snapshot. | Prescribing |
| [`toxicity_death_probability`](#rule-toxicity-death-probability) | Temporary scalar probability | Daily probability of death from accumulated drug toxicity. | Recalculated daily. | Discarded after the mortality draw; may become terminal death state. | Mortality |
| [`background_death_probability`](#rule-background-death-probability) | Temporary scalar probability | Daily probability of non-infection, non-toxicity background death. | Recalculated daily. | Discarded after the mortality draw; may become terminal death state. | Mortality |
| [`non_sepsis_infection_death_probability`](#rule-non-sepsis-infection-death-probability) | Temporary scalar probability | Daily probability of infection-related death outside the sepsis pathway. | Recalculated daily. | Discarded after the mortality draw; may become terminal death state. | Mortality |
| [`sepsis_death_probability`](#rule-sepsis-death-probability) | Temporary scalar probability | Daily probability of death from current sepsis states. | Recalculated daily. | Discarded after the mortality draw; may become terminal death state. | Mortality |
| [`sepsis_recovery_probability[b]`](#rule-sepsis-recovery-probability) | Temporary bacterium probability family | Daily probability that an eligible sepsis state resolves. | Recalculated after the minimum duration. | Discarded after the recovery draw. | Sepsis |
| [`microbiome_acquisition_probability[b]`](#rule-microbiome-acquisition-probability) | Temporary bacterium probability family | Daily probability of entering carriage. | Recalculated when not already carrying `b`. | Discarded after acquisition sampling. | Carriage |
| [`local_profile_sampling_probability[b]`](#rule-local-profile-sampling-probability) | Temporary bacterium probability family | Probability that a newly acquired carriage or infection profile is sampled from local persistent strain pressure. | Recalculated at acquisition. | Discarded after source selection. | Resistance persistence |
| [`microbiome_clearance_probability[b]`](#rule-microbiome-clearance-probability) | Temporary bacterium probability family | Daily probability of clearing carriage. | Recalculated during carriage. | Discarded after the clearance draw. | Carriage |
| [`infection_acquisition_probability[b]`](#rule-infection-acquisition-probability) | Temporary bacterium probability family | Daily probability that an uninfected person acquires active infection. | Recalculated daily and copied to `predicted_infection_risk[b]`. | Discarded after acquisition sampling. | Infection acquisition |
| [`incoming_infection_mechanism_mask[b]`](#rule-incoming-infection-mechanism-mask) | Temporary bacterium bitmask family | Resistance profile assembled for a candidate new infection. | Built only after an acquisition draw succeeds. | Either installed in infection state or discarded if therapy prevents establishment. | Resistance acquisition |
| [`existing_therapy_prevention_probability[b]`](#rule-existing-therapy-prevention-probability) | Temporary bacterium probability family | Probability that current effective therapy prevents an incoming infection from establishing. | Recalculated for candidate acquisition. | Discarded after prevention draws. | Infection acquisition |
| [`de_novo_emergence_probability[b,m]`](#rule-de-novo-emergence-probability) | Temporary bacterium-by-mechanism probability family | Daily probability that an absent mechanism emerges in active infection under applicable pressure. | Recalculated for eligible routes. | Discarded after one route-level draw. | Resistance evolution |
| [`minority_promotion_probability[b,m]`](#rule-minority-promotion-probability) | Temporary bacterium-by-mechanism probability family | Daily probability that a minority mechanism becomes majority under selecting drug pressure. | Recalculated for eligible minority routes. | Discarded after one route-level draw. | Resistance evolution |
| [`mechanism_reversion_probability[b,m]`](#rule-mechanism-reversion-probability) | Temporary bacterium-by-mechanism probability family | Daily probability of losing an unselected mechanism from the represented majority or carriage state. | Recalculated for eligible routes. | Discarded after one route-level draw. | Resistance evolution |
| [`hgt_probability[recipient_b,m]`](#rule-hgt-probability) | Temporary recipient-bacterium-by-mechanism probability family | Daily probability of horizontal transfer from eligible carried or infected donors. | Recalculated for eligible recipient routes. | Discarded after donor and transfer sampling. | Resistance evolution |
| [`new_bacteria_level[b]`](#rule-new-bacteria-level) | Temporary bacterium family | Proposed next infection intensity after growth and drug activity. | Recalculated for active infection. | Stored in `level[b]` or replaced by clearance. | Active infection |
| [`symptom_onset_probability[b]`](#rule-symptom-onset-probability) | Temporary bacterium probability family | Daily probability that an eligible active infection latches the symptom indicator. | Recalculated until symptoms occur. | Discarded after the onset draw. | Clinical presentation |
| [`bacterial_identification_probability[b]`](#rule-bacterial-identification-probability) | Temporary bacterium probability family | Daily probability that testing identifies the bacterium. | Recalculated while identification is eligible and incomplete. | Discarded after the identification draw. | Diagnostics |
| [`resistance_testing_probability[b]`](#rule-resistance-testing-probability) | Temporary bacterium probability family | Daily probability of initiating AST after bacterial identification. | Recalculated while AST is eligible and uninitiated. | Discarded after the initiation draw. | Diagnostics |
| [`infection_resolution_type[b]`](#rule-infection-resolution-type) | Temporary bacterium enum | Classification of a cleared or fatal infection outcome. | Constructed when an infection resolves. | Converted to a daily resolution counter and discarded. | Outcome reporting |
| [`effective_carriage_activity[b,d]`](#rule-effective-carriage-activity) | Temporary bacterium-by-drug family | Drug activity used by carriage-clearance rules after carriage resistance. | Recalculated during carriage under exposure. | Discarded after contributing to clearance log-odds. | Carriage and treatment |
...
| [`restart_treatment_eligibility[b]`](#rule-restart-treatment-eligibility) | Temporary bacterium Boolean | Whether persistent or rebounding infection after cessation meets restart conditions. | Recalculated within the configured restart window. | Converted to restart action or completed assessment. | Treatment failure |
| [`treatment_failure_indicator[b]`](#rule-treatment-failure-indicator) | Temporary bacterium Boolean | Whether tracked treatment has failed to reduce infection sufficiently by the assessment day. | Evaluated once per tracked treatment episode. | Converted to failure history and treatment changes. | Treatment failure |

### D.3 Table D2. Update-rule Catalogue

Parameter names containing `{bacterium}`, `{drug}`, `{mechanism}`, `{region}`,
`{syndrome}`, or `{age_group}` denote indexed parameter families rather than
additional individual-level variables. "Fixed rule" identifies an executable
constant or category rule rather than a configurable parameter.

| Variable | Direct individual-state inputs | Configured parameters or fixed inputs | Update timing and rule | Exceptions and notes | Executable authority |
|---|---|---|---|---|---|
| <a id="rule-id"></a>`id` | Population construction order. | None. | Assigned once when the population is constructed and never updated. | Identifier only; it has no biological effect. | `population::Population::new`; `population::Individual::new` |
...
| <a id="rule-sex-at-birth"></a>`sex_at_birth` | Initialisation RNG draw. | Fixed initialisation probability: 0.5 male and 0.5 female. | Assigned at population construction and retained for life. | Binary model abstraction. | `population::Population::new` |
| <a id="rule-perceived-penicillin-allergy"></a>`perceived_penicillin_allergy` | Initialisation RNG draw. | Fixed initialisation probability: 0.08. | Assigned at population construction and retained for life; it penalises selection of relevant drugs. | Represents perceived status, not confirmed immunological allergy. | `population::Population::new`; drug-selection rules in `rules` |
| <a id="rule-region-living"></a>`region_living` | Population age-region sampling distribution. | Population age-region distribution inputs. | Assigned at construction and retained as the home region. | Travel changes `region_cur_in`, not this field. | `population::sample_age_and_region_from_distribution`; `population::Population::new` |
...
| <a id="rule-level"></a>`level[b]` | Previous level, age, immunodeficiency, syndrome, infection duration, drug exposure, `any_r`, immune-clearance draw. | `{bacterium}_initial_infection_level`; `{bacterium}_base_bacteria_level_change`; `{bacterium}_max_level`; `bacteria_growth_age_multiplier_{age_group}`; `bacteria_growth_immunodeficiency_multiplier`; `syndrome_{id}_bacteria_growth_multiplier`; potency, penetration and activity parameters; clearance parameter families. | Initialised at acquisition; daily growth and effective drug activity form `new_bacteria_level[b]`, after which immune clearance or a sub-threshold level can resolve infection. | MDR-TB has explicit multidrug synergy rules; the numerical clearance threshold is fixed at 0.0001. | Infection-acquisition and active-infection blocks in `rules::apply_rules`; `config::ClearanceParameters` |
| <a id="rule-predicted-infection-risk"></a>`predicted_infection_risk[b]` | Current acquisition inputs summarised by `infection_acquisition_probability[b]`. | Acquisition parameter families listed for that temporary probability. | Replaced daily with the calculated probability for an eligible person-bacterium pair. | A diagnostic snapshot; it is not a second acquisition draw. | Infection-acquisition block in `rules::apply_rules` |
| <a id="rule-clearance-hazard"></a>`clearance_hazard[b]` | `level[b]`, age, immunodeficiency, infection duration, bacterium. | `default_clearance_base_log_odds`; `{bacterium}_clearance_log_odds_adjustment`; `clearance_age_log_odds_{age_group}`; `clearance_immunodeficient_log_odds`; `clearance_level_log_odds_per_unit`; fixed duration slope of 0.25 per day. | Recalculated and stored on active eligible infection days before the immune-clearance draw. | Zero outside an eligible active infection. | `config::ClearanceParameters::hazard_for`; active-infection block in `rules::apply_rules` |
| <a id="rule-clearance-ready-day"></a>`clearance_ready_day[b]` | Infection acquisition day. | Current executable delay rule. | Set when infection starts and consulted before immune clearance is allowed. | Loaded clearance-delay fields do not currently alter this update. | Infection-acquisition and active-infection blocks in `rules::apply_rules` |
| <a id="rule-sepsis"></a>`sepsis[b]` | Infection level and duration, age, syndrome, region, immunodeficiency, hospitalisation, `under_medical_care`, recovery and death events. | Bacterium-specific sepsis-onset families; age, syndrome, region and care modifiers; sepsis-recovery families. | A daily onset draw can set the flag; recovery, infection resolution, or death clears it. | *H. pylori* cannot initiate sepsis unless another active infection exists. | Sepsis onset, recovery and mortality blocks in `rules::apply_rules` |
...
| <a id="rule-sepsis-episode-open"></a>`sepsis_episode_open[b]` | `sepsis[b]` transitions and infection resolution. | None. | Opened at sepsis onset and closed at recovery, infection resolution, or death. | Output-accounting state; it does not alter clinical risk. | Sepsis episode bookkeeping in `rules` and `simulation` |
| <a id="rule-sepsis-episode-context-at-onset"></a>`sepsis_episode_context_at_onset[b]` | Drug contexts active at sepsis onset. | Fixed treatment-context categories. | Snapshotted once when the sepsis episode opens. | Reporting only. | Sepsis episode bookkeeping in `simulation` |
| <a id="rule-sepsis-episode-best-activity-at-onset"></a>`sepsis_episode_best_activity_at_onset[b]` | Drug exposure, potency, penetration and `any_r[b][d]` at onset. | Activity parameters listed for `activity_r`; fixed output threshold documented for effective therapy. | Stores the maximum applicable drug activity at episode onset. | Reporting only; it is not the dynamic treatment calculation. | Sepsis episode bookkeeping in `simulation` |
| <a id="rule-sepsis-episode-effective-at-onset"></a>`sepsis_episode_effective_at_onset[b]` | `sepsis_episode_best_activity_at_onset[b]`. | Fixed reporting threshold `EFFECTIVE_THERAPY_ACTIVITY_THRESHOLD = 0.5`. | Set once at episode onset according to the reporting threshold. | Output definition, not a configurable biological threshold. | Sepsis episode bookkeeping in `simulation` |
| <a id="rule-sepsis-episode-first-effective-day"></a>`sepsis_episode_first_effective_day[b]` | Daily best activity during an open episode. | Fixed reporting threshold 0.5. | Set on the first episode day that activity meets the threshold. | Remains missing if effective therapy is never observed. | Sepsis episode bookkeeping in `simulation` |
| <a id="rule-sepsis-episode-delay-bucket-recorded"></a>`sepsis_episode_delay_bucket_recorded[b]` | Onset day, first effective day, episode closure. | Fixed reporting delay buckets. | Latches after the episode contributes to one delay-to-effective-therapy category. | Prevents duplicate output counting. | Sepsis episode bookkeeping in `simulation` |
| <a id="rule-sepsis-episode-region-at-onset"></a>`sepsis_episode_region_at_onset[b]` | `region_cur_in` at sepsis onset. | Region categories. | Snapshotted when the episode opens. | Reporting only. | Sepsis episode bookkeeping in `simulation` |
| <a id="rule-sepsis-episode-hospitalized-at-onset"></a>`sepsis_episode_hospitalized_at_onset[b]` | `hospital_status` at sepsis onset. | None. | Snapshotted when the episode opens. | Reporting only. | Sepsis episode bookkeeping in `simulation` |
| <a id="rule-sepsis-episode-age-group-at-onset"></a>`sepsis_episode_age_group_at_onset[b]` | `age` at sepsis onset. | Fixed age-group boundaries. | Derived and stored when the episode opens. | Reporting category only. | Sepsis episode bookkeeping in `simulation` |
| <a id="rule-diagnostic-cascade-open"></a>`diagnostic_cascade_open[b]` | Active symptomatic infection and cascade lifecycle. | Cascade reporting contract. | Opened on qualifying cascade entry and closed when infection resolves or the cascade is finalised. | Output-accounting state; it does not trigger testing. | Diagnostic cascade bookkeeping in `simulation` |
| <a id="rule-diagnostic-cascade-entry-time-step"></a>`diagnostic_cascade_entry_time_step[b]` | Cascade opening, current time step. | None. | Snapshotted once when the cascade opens. | Reporting only. | Diagnostic cascade bookkeeping in `simulation` |
| <a id="rule-diagnostic-cascade-entry-hospitalized"></a>`diagnostic_cascade_entry_hospitalized[b]` | `hospital_status` at cascade entry. | None. | Snapshotted once when the cascade opens. | Reporting only. | Diagnostic cascade bookkeeping in `simulation` |
| <a id="rule-diagnostic-cascade-bacterial-identification-recorded"></a>`diagnostic_cascade_bacterial_identification_recorded[b]` | `test_identified_infection[b]`, cascade state. | None. | Latches after bacterial identification is counted for the episode. | Prevents duplicate output counting. | Diagnostic cascade bookkeeping in `simulation` |
| <a id="rule-diagnostic-cascade-resistance-testing-recorded"></a>`diagnostic_cascade_resistance_testing_recorded[b]` | Completed `test_r[b][d]` results, cascade state. | AST readiness and reporting rules. | Latches after resistance testing is counted for the episode. | Initiation alone is not a completed result. | Diagnostic cascade bookkeeping in `simulation` |
| <a id="rule-diagnostic-cascade-targeted-treatment-recorded"></a>`diagnostic_cascade_targeted_treatment_recorded[b]` | Bacterial identification and subsequent drug context. | Fixed targeted-treatment classification. | Latches after a targeted course is counted. | Reporting only. | Diagnostic cascade bookkeeping in `simulation` |
| <a id="rule-diagnostic-cascade-effective-targeted-treatment-recorded"></a>`diagnostic_cascade_effective_targeted_treatment_recorded[b]` | Targeted treatment and applied activity. | Fixed effective-therapy reporting definition. | Latches after effective targeted therapy is counted. | Reporting only. | Diagnostic cascade bookkeeping in `simulation` |
...
| <a id="rule-microbiome-acquired-today"></a>`microbiome_acquired_today[b]` | Successful carriage acquisition event. | None. | Reset at the start of the day and set for a same-day acquisition. | Daily reporting flag. | Daily reset and carriage-acquisition code in `rules` |
| <a id="rule-microbiome-acquired-on-drug-today"></a>`microbiome_acquired_on_drug_today[b]` | Same-day carriage acquisition and active drug exposure. | Fixed active-exposure criterion. | Set when acquisition occurs while qualifying drug exposure is present. | Daily reporting flag. | Carriage-acquisition code in `rules` |
| <a id="rule-microbiome-cleared-today"></a>`microbiome_cleared_today[b]` | Successful carriage clearance event. | None. | Reset at the start of the day and set on same-day clearance. | Daily reporting flag. | Daily reset and carriage-clearance code in `rules` |
| <a id="rule-cleared-any-r-microbiome-categories"></a>`cleared_any_r_microbiome_categories[b][category]` | Carriage resistance immediately before clearance. | Fixed resistance-category boundaries in the output contract. | Populated when carriage clears, before the carriage resistance state is reset. | Daily output categories; they do not influence clearance. | Carriage-clearance code in `rules`; aggregation in `simulation` |
| <a id="rule-vaccination-status"></a>`vaccination_status[b]` | Age crossing birth, simulation year, eligible bacterial vaccine, RNG draw. | `vaccine_{vaccine}_availability_year`; `vaccine_{vaccine}_birth_coverage_target`; `vaccine_{vaccine}_rollout_years`. | Eligible birth cohorts receive one coverage draw as they enter the active population; successful status remains true. | Only mapped bacterial vaccine targets are supported; this is birth-cohort vaccination, not an all-age campaign. | `rules::prepare_individual_for_active_day`; `config::VaccinationParameters` |
| <a id="rule-infection-has-caused-symptoms"></a>`infection_has_caused_symptoms[b]` | `level[b]`, duration since acquisition, bacterium. | `{bacterium}_symptom_onset_base_log_odds`; `{bacterium}_symptom_onset_threshold_level`; `{bacterium}_symptom_onset_delay_days`; `{bacterium}_symptom_onset_log_odds_per_level_unit`. | An eligible asymptomatic infection receives a daily onset draw; true is latched for that infection episode. | Operationally includes symptoms or another indication represented as presentation for testing; reset at resolution. | Symptom-onset block in `rules::apply_rules` |
| <a id="rule-test-identified-infection"></a>`test_identified_infection[b]` | Active symptomatic infection, infection duration, year, region, hospital status, immunodeficiency, sepsis. | `test_delay_days`; `bacterial_testing_available_from_day`; `{bacterium}_test_availability_year`; `bacterial_testing_base_rate_per_day`; adoption, hospital, region, immunosuppression, sepsis and policy testing multipliers. | Eligible unresolved infections receive a daily identification draw; a positive identification is latched for the episode. | Adoption follows a fixed 40-year sigmoid; reset at resolution. | Diagnostic-testing block in `rules::apply_rules` |
...
| <a id="rule-resistance-test-initiated-day"></a>`resistance_test_initiated_day[b]` | AST initiation event, current day. | `resistance_test_result_delay_days`. | Set on AST initiation; used to determine when `test_r` results become available; reset with the infection episode. | A sentinel represents no test initiated. | Resistance-testing block and result-readiness helper in `rules` |
...
| <a id="rule-date-drug-initiated"></a>`date_drug_initiated[d]` | Course-start event, current day. | None beyond initiation. | Set when a course starts and replaced by a sentinel when it stops. | Current-course date. | `rules::start_drug_course`; `rules::stop_drug_course` |
...
| <a id="rule-ever-taken-drug"></a>`ever_taken_drug[d]` | Course-start event. | None. | Latches true the first time the drug is started. | Never reset during a person's lifetime. | `rules::start_drug_course` |
...
| <a id="rule-infection-hospital-acquired"></a>`infection_hospital_acquired[b]` | `hospital_status` at successful infection acquisition. | None. | Snapshotted when infection starts and reset at resolution. | Acquisition setting is retained even if care location subsequently changes. | Infection-acquisition and resolution blocks in `rules::apply_rules` |
...
| <a id="rule-resistance-microbiome-r"></a>`resistances[b][d].microbiome_r` | `mechanism_microbiome[b]`, drug class. | `resistance_mechanism_{mechanism}_enhancement_{drug_class}` with mechanism-wide fallback; `max_resistance_level`. | Recomputed from the carriage mechanism mask after carriage resistance changes. | Continuous susceptibility reduction among carriage profiles; zero does not mean the bacterium is absent. | Resistance recalculation helpers in `rules`; `population::Resistance` |
| <a id="rule-resistance-test-r"></a>`resistances[b][d].test_r` | Completed AST timing, current `any_r[b][d]`, test-error draw. | `resistance_test_result_delay_days`; `test_r_error_prob`; `test_r_error_value`. | On result readiness, copies current resistance with the configured error process. | Zero is a valid result value; readiness must be established from test state and timing. | AST result block and readiness helper in `rules` |
...
| <a id="rule-resistance-any-r"></a>`resistances[b][d].any_r` | `mechanism_any[b]`, drug class. | `resistance_mechanism_{mechanism}_enhancement_{drug_class}` with mechanism-wide fallback; `max_resistance_level`. | Recomputed after infection mechanism acquisition, emergence, transfer, promotion or reversion. | Mechanism effects multiply remaining susceptibility and are bounded by `max_resistance_level`. | Resistance recalculation helpers in `rules`; `population::Resistance` |
| <a id="rule-mechanism-any"></a>`mechanism_any[b]` | Incoming infection mask, de novo emergence, HGT, minority promotion and reversion. | Mechanism applicability; de novo, HGT, promotion and reversion parameter families. | Bitmask updated when any represented infection strain carries or loses a mechanism. | Includes minority mechanisms; `mechanism_majority` is the dominant-profile subset. | Resistance acquisition and evolution blocks in `rules::apply_rules` |
| <a id="rule-mechanism-majority"></a>`mechanism_majority[b]` | Incoming majority mask, minority promotion and majority reversion. | `majority_r_evolution_rate_per_day_when_drug_present`; reversion families. | Updated when mechanisms establish in, enter, or leave the represented majority infection profile. | Must remain a subset of `mechanism_any[b]`. | Resistance acquisition and evolution blocks in `rules::apply_rules` |
| <a id="rule-mechanism-microbiome"></a>`mechanism_microbiome[b]` | Incoming carriage profile, de novo carriage emergence, infection-carriage transfer, HGT, and carriage reversion. | Carriage profile-source, emergence, transfer, HGT and reversion parameter families. | Set at carriage acquisition and updated during carriage resistance evolution; reset at clearance. | Separate from the active-infection masks. | Carriage acquisition, evolution and clearance blocks in `rules::apply_rules` |
| <a id="rule-how-resistance-acquired"></a>`how_resistance_acquired[b][d]` | Profile sampling, microbiome inheritance, de novo emergence and HGT events. | Fixed provenance categories; compile-time `TRACK_RESISTANCE_ACQUISITION_PROVENANCE`. | Updated when resistance first enters or materially changes the active infection profile. | The matrix is empty when tracking is disabled; when enabled it is a coarse last-event classification, not full causal provenance. | Resistance acquisition and evolution blocks in `rules`; output aggregation in `simulation` |
...
| <a id="rule-immunodeficiency-type"></a>`immunodeficiency_type` | Age, previous immunodeficiency state, startup and daily transition draws. | `immunosuppression_startup_seed_fraction`; `immunosuppression_chronic_probability_age_{age_group}`; daily incidence and recovery parameter families. | Seeded at construction, then updated by daily onset and recovery transitions. | Type categories represent the model's immunodeficiency states rather than diagnoses. | `population::Population::new`; immunodeficiency block in `rules::apply_rules` |
| <a id="rule-bacteria-level-at-drug-start"></a>`bacteria_level_at_drug_start[b]` | `level[b]` when a tracked treatment starts. | Treatment-failure eligibility rules. | Snapshotted at the start of a qualifying treatment episode. | Used as the baseline for subsequent failure assessment. | `rules::start_drug_course`; treatment-failure helpers |
...
| <a id="rule-drug-stopped-with-infection-day"></a>`drug_stopped_with_infection_day[b]` | Drug cessation while infection remains active. | Restart-window parameters. | Set when treatment stops before infection resolution. | Sentinel when no unresolved cessation is being tracked. | `rules::stop_drug_course`; restart-window logic |
| <a id="rule-bacteria-level-at-drug-cessation"></a>`bacteria_level_at_drug_cessation[b]` | `level[b]` at unresolved treatment cessation. | Restart-window parameters. | Snapshotted with `drug_stopped_with_infection_day[b]`. | Baseline for detecting persistence or rebound. | Treatment cessation and restart-window logic in `rules` |
| <a id="rule-bacteria-on-selection-day"></a>`bacteria_on_selection_day` | Bacterium index associated with a treatment-selection event. | Drug-selection rules. | Set to the triggering bacterium index for a selection event and otherwise reset to `-1`. | Treatment audit state; it is not hidden pathogen knowledge supplied to empiric selection. | Antibiotic-selection code in `rules::apply_rules` |
| <a id="rule-drug-score-on-selection-day"></a>`drug_score_on_selection_day[d]` | Candidate drug scores calculated at selection. | Potency threshold, empiric or targeted scores, preferences, surveillance, allergy, availability, toxicity memory, reserve and policy modifiers; `drug_selection_temperature`. | Stores the score used for the selection event. | Ordinary bacterium-drug initiation multipliers apply to targeted, not empiric, choice; empiric choice uses syndrome scores. | Drug-score and selection helpers in `rules` |
| <a id="rule-stopped-drug-index"></a>`stopped_drug_index[b]` | Drug stopped while an infection remains active. | Restart-window rules. | Records the relevant stopped drug for later restart assessment. | Sentinel when no drug is tracked. | Treatment cessation and restart-window logic in `rules` |
| <a id="rule-restart-window-assessed"></a>`restart_window_assessed[b]` | Cessation date, current day, persistent or rebounding level. | `restart_window_enabled`; `restart_window_days`; `restart_bacteria_level_threshold`; `restart_window_probability`. | Latches once the post-cessation restart window has been evaluated. | Prevents repeated assessment for the same cessation episode. | Restart-window logic in `rules::apply_rules` |
| <a id="rule-date-last-drug-failure"></a>`date_last_drug_failure[b]` | Confirmed treatment-failure event for bacterium `b`, current day. | `drug_failure_memory_days`. | Set when treatment failure is confirmed and retained as recent failure history. | A recent failure can support escalation to reserve candidates; the field is per bacterium, not per drug. | `rules::assess_treatment_failure`; drug-selection helpers |
...
| <a id="rule-active-symptomatic-syndromes"></a>`active_symptomatic_syndromes` | `level[b]`, `infection_has_caused_symptoms[b]`, `infectious_syndrome[b]`. | Fixed active-infection and nonzero-syndrome tests. | Built when empiric drug initiation is evaluated by collecting the distinct syndromes of active symptomatic infections. | It exposes syndrome information, not hidden bacterium identity, to empiric selection. | `rules::active_symptomatic_syndromes` |
...
| <a id="rule-serious-resistance-test-positive"></a>`serious_resistance_test_positive` | Completed `test_r[b][d]` results for serious resistance categories. | AST readiness rule; fixed serious-resistance drug/category mapping. | Recomputed before hospitalisation risk is evaluated. | Unperformed or pending AST is not interpreted as a negative result. | Hospitalisation and AST helpers in `rules` |
| <a id="rule-immunodeficiency-transition-probability"></a>`immunodeficiency_transition_probability` | Age, current immunodeficiency state, simulation time. | Age-specific immunodeficiency incidence, type-allocation and recovery parameter families. | Recomputed for eligible onset or recovery transitions and immediately consumed by an RNG draw. | A family of pathway-specific probabilities, not one stored scalar. | Immunodeficiency block in `rules::apply_rules` |
...
| <a id="rule-drug-selection-score"></a>`drug_selection_score[d]` | Treatment context, active syndromes, identified bacteria, potency, AST or surveillance resistance, allergy, availability, toxicity and failure history. | `minimal_potency_threshold_for_drug_selection`; syndrome empiric scores including `_before_{year}` overrides; targeted bacterium-drug initiation multipliers; clinical preference, reserve, surveillance, allergy, toxicity, policy and `drug_selection_temperature` parameters. | Candidate scores are built at each selection event and converted to weighted choice probabilities. | Empiric choice uses syndrome and calendar era but not hidden bacterium identity; ordinary bacterium-drug multipliers are targeted-only. | Drug-score and weighted-selection helpers in `rules` |
...
| <a id="rule-local-profile-sampling-probability"></a>`local_profile_sampling_probability[b]` | Acquisition setting, active local profile count and pathway context. | `local_mechanism_persistence_enabled`; `local_mechanism_persistence_virtual_profile_mass`; `local_mechanism_persistence_max_sampling_probability`; `community_profile_cache_retention`; `hospital_profile_cache_retention`; pathway and counterfactual multipliers. | Calculated when a new infection or carriage profile may be drawn from the regional care-setting cache. | Community and hospital source rules differ; persistence sampling is bounded and does not determine incidence. | `simulation::ResistanceProfileCache`; `rules::carriage_profile_sampling_probability`; acquisition blocks |
...
| <a id="rule-infection-acquisition-probability"></a>`infection_acquisition_probability[b]` | Infection absence, age, region, hospital status, vaccination, carriage, exposure context and policy state. | `{bacterium}_acquisition_log_odds_baseline`; age and region acquisition families; hospital, vaccination, carriage-versus-infection, pathway and policy multipliers. | Recomputed daily for each eligible uninfected bacterium and copied to `predicted_infection_risk[b]` before acquisition sampling. | Incidence is externally parameterised; profile dynamics determine the resistance of acquisitions, not their number. | Infection-acquisition block in `rules::apply_rules` |
| <a id="rule-incoming-infection-mechanism-mask"></a>`incoming_infection_mechanism_mask[b]` | Sampled local or exogenous profile, `mechanism_microbiome[b]`, current acquisition setting. | Profile-cache and ratchet parameters; `carrier_resistance_inheritance_probability`; `infection_from_microbiome_dampening`; mechanism applicability. | Constructed only after an infection-acquisition draw succeeds, then sanitised for the bacterium. | Discarded if existing therapy prevents establishment. | Infection profile assembly in `rules::apply_rules`; `simulation::ResistanceProfileCache` |
| <a id="rule-existing-therapy-prevention-probability"></a>`existing_therapy_prevention_probability[b]` | Current drug exposure, potency, syndrome penetration and resistance implied by the incoming mechanism mask. | `antibiotic_infection_prevention_efficacy`; potency, penetration and `max_resistance_level`. | Calculated for candidate acquisition under active therapy before prevention draws. | Prevents establishment; it does not reduce the externally specified acquisition hazard itself. | Infection-acquisition prevention block in `rules::apply_rules` |
...
| <a id="rule-minority-promotion-probability"></a>`minority_promotion_probability[b,m]` | Minority mechanism in `mechanism_any[b]`, absence from `mechanism_majority[b]`, selecting drug pressure. | `majority_r_evolution_rate_per_day_when_drug_present`; mechanism-drug applicability. | One draw is made per eligible minority mechanism under selecting pressure. | Promotion changes majority status, not whether any strain carries the mechanism. | `rules::promote_minority_mechanisms_once`; resistance-evolution block |
| <a id="rule-mechanism-reversion-probability"></a>`mechanism_reversion_probability[b,m]` | Present eligible mechanism, majority or carriage compartment, absence of current selection. | Mechanism-specific reversion-rate family; `run_pathway_reversion_rate_multiplier`; applicability and selection gates. | Recomputed for eligible unselected mechanisms before daily loss draws. | Infection and carriage masks are updated separately; selected mechanisms do not revert through this route. | Reversion blocks and parameter cache in `rules` |
| <a id="rule-hgt-probability"></a>`hgt_probability[recipient_b,m]` | Eligible donor and recipient compartments, donor mechanism, hospital setting, antibiotic pressure and donor majority status. | Donor-recipient HGT rate matrix; `hgt_hospital_multiplier`; antibiotic-context multipliers; `hgt_minority_donor_multiplier`; `run_pathway_hgt_multiplier`; counterfactual resistance multiplier. | Calculated for eligible donor-recipient-mechanism routes before transfer sampling. | Donor and recipient must share a represented infection or carriage compartment; MDR-TB is excluded and applicability gates are enforced. | HGT block in `rules::apply_rules`; HGT parameter cache |
| <a id="rule-new-bacteria-level"></a>`new_bacteria_level[b]` | Previous `level[b]`, growth modifiers, applied drug activity and response multiplier. | Initial and maximum level; base growth; age, immunodeficiency and syndrome growth multipliers; potency, penetration, activity and treatment-response parameters. | Built once per active infection day, bounded, then stored in `level[b]` unless resolution supersedes it. | MDR-TB multidrug synergy and background effectiveness are explicit exceptions. | Active-infection level block in `rules::apply_rules` |
| <a id="rule-symptom-onset-probability"></a>`symptom_onset_probability[b]` | `level[b]`, infection duration and current symptom flag. | `{bacterium}_symptom_onset_base_log_odds`; threshold, delay and per-level log-odds parameters. | Recomputed for eligible infections until the symptom indicator latches. | No further onset draws after symptoms have occurred in that episode. | Symptom-onset block in `rules::apply_rules` |
...
| <a id="rule-infection-resolution-type"></a>`infection_resolution_type[b]` | Resolution cause, treatment exposure, sepsis and fatal pathway. | Fixed resolution categories. | Constructed when an infection clears or ends in death and immediately converted to a daily counter. | Temporary classification avoids counting one episode in multiple resolution categories. | Resolution and mortality blocks in `rules::apply_rules` |
...
| <a id="rule-applied-activity-observation"></a>`applied_activity_observation[b]` | Active infection, all drug exposures, potency, penetration, `any_r`, and treatment contexts. | Same potency, penetration and resistance parameters as dynamic activity; fixed observation fields. | Aggregates applied, potential, pure and best activity when drug exposure exists, then emits a daily rule event. | Reporting object only; dynamic level change uses the underlying activity calculation directly. | `rules::applied_activity_observation`; event aggregation in `simulation` |
| <a id="rule-restart-treatment-eligibility"></a>`restart_treatment_eligibility[b]` | Cessation day and level, current level, stopped drug, prior assessment and current activity. | `restart_window_enabled`; `restart_window_days`; `restart_bacteria_level_threshold`; `restart_window_probability`; `minimal_potency_threshold_for_drug_selection`; fixed fallback high-level threshold 2.0 and active-level threshold 0.1. | Recomputed within the post-cessation window before any probabilistic restart action. | Evaluated once per tracked cessation episode. | Restart-window helper and treatment-selection code in `rules` |
