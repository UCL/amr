...
The stored profile library blends retained profiles with newly collected profiles each day. Every stored profile has the same marginal probability of retention. Both `community_profile_cache_retention` and `hospital_profile_cache_retention` are 0.999. For an existing stored profile this corresponds to an expected marginal half-life of roughly 693 daily refreshes. It is a retention parameter for the profile library, not a biological half-life for host carriage, a bacterial clone, or a resistance mechanism. Hospital and community groups can both become all-susceptible when that is what retained and newly observed active profiles contain. Established resistant genotypes are represented separately in the bounded local mechanism-persistence archive rather than being forced into the active-prevalence library.
...
The environmental-floor parameters represent explicitly specified agricultural, food-chain, wastewater, and other exogenous pathways that can reseed resistance when the local human circulating-profile library alone would be insufficient.
