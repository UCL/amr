...
| Incident bacterial sepsis | 30 million cases per year | Rudd KE et al., 2020 | Derived bacterial-subset assumption | Rudd et al. estimate all-cause sepsis; the model simulates bacterial infections only, so this is a bacterial subset anchor rather than a direct observed global count. The headline numerator counts each person once when they transition from no active sepsis to active sepsis; per-bacterium onset counters remain separate descriptive outputs. |
...
- a person-level incident-sepsis scalar, alongside per-bacterium sepsis prevalence and onset counters
