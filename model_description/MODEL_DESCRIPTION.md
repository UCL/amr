...
Second, independently of the exogenous floor path, the code retains an optional **universal 1% acquisition reseeding guard** through `calculate_resistance_floor(bacteria, drug, current_day)`. It is not a per-organism ramped or prevalence-triggered floor. When enabled, every new infection and each eligible drug receives a separate attempt after that drug's class has been introduced. A successful 1% check enters mechanism assignment; an applicable non-placeholder mechanism is then assigned with probability `mechanism_assignment_probability_on_any_r_gain` (currently 0.8), and `any_r` is derived from that mechanism. The rule operates whether current prevalence is low or high and does not directly clamp prevalence to 1%. The guard is provisionally disabled in the current default configuration (`resistance_floor_feature_enabled = 0.0`) while its necessity at production population sizes is assessed.

**Causal correctness guard — the mechanism must be represented in the retained global cache.** When enabled, the universal guard can assign a mechanism only if that mechanism is currently present in at least one stored profile for the bacterium across all regions and both community and hospital strata. Although the helper is named `mechanism_has_ever_emerged_globally`, it delegates to current retained-cache membership; there is no separate permanent registry of every historical emergence. The rule therefore cannot introduce a mechanism before it appears in the circulating cache, and it cannot resurrect one after it has disappeared from every retained profile.

When enabled for all organisms, the universal guard is attempted separately for eligible drugs and one mechanism can be reached through several drug entries, so its effect is not equivalent to setting a 1% organism-level resistance prevalence. With the guard disabled, resistance is generated and maintained through profile inheritance, de novo emergence, HGT, carriage, explicit environmental reseeding probabilities, and ratchet support through the exogenous pathway.
...
| Optional universal acquisition reseeding guard (disabled by default) | When enabled: every new infection, per drug entry whose class is introduced | Can record one applicable mechanism currently represented in the global cache in `mechanism_any` and `mechanism_majority` | Repeated 1% entry checks followed by mechanism assignment; not a direct 1% prevalence clamp |
...
At finite simulated population sizes, uncommon infections and mechanisms can be represented by very few active infection-days and retained profiles. The code retains an optional **universal acquisition reseeding guard**, configured under the resistance-floor parameter names and separate from the environmental and ratchet pathways described in Sections 7.7 and 7.8. When enabled, this guard reduces the chance of losing a mechanism while it remains represented in the retained profile cache; it is not a hard lower bound on resistance prevalence. It is provisionally disabled in the current default configuration so that the corrected cache and explicit biological persistence pathways can be evaluated without a population-invariant universal reinforcement rule.

| Parameter | Value | Function |
|-----------|-------|----------|
| `resistance_floor_feature_enabled` | 0.0 (off) | Master switch for this optional universal guard only |
| `resistance_floor_all_bacteria_enabled` | 1.0 (dormant) | Restores the previous all-bacteria scope if the master switch is re-enabled |
| `resistance_floor_default_level` | 0.01 (dormant) | Per-drug-entry probability used only when the guard is enabled |
| `bacteria_{name}_resistance_floor_enabled` | Per-organism | Per-species override when all-bacteria switch is 0.0 |

**How the acquisition reseeding guard works when enabled.** Every time a person acquires an infection, the model runs a four-step guard separately for each drug:
...
**Interpretation and current decision.** The configured 1% value is repeated by drug entry, followed by a separate 0.8 mechanism-assignment roll. It is therefore not necessarily weak at the organism level and its aggregate effect depends on drug-list and mechanism applicability. It also does not guarantee persistence: after a mechanism disappears from every retained profile, the cache-membership guard blocks this pathway from recreating it. Because its finite-population rationale becomes less compelling as the production population rises from 3 million towards 10 million, the current default leaves it off while transmission of sampled profiles, explicit environmental reseeding probabilities, the ratchet, emergence, HGT, and carriage operate without it.

**Default configuration — guard disabled.** `resistance_floor_feature_enabled = 0.0` prevents all universal guard attempts. `resistance_floor_all_bacteria_enabled = 1.0` and `resistance_floor_default_level = 0.01` remain dormant so that changing only the master switch recreates the previous guard-on configuration for matched comparisons. Explicit bacterium–mechanism environmental floors and the dynamic ratchet are not controlled by this master switch.

**Guard-on comparison.** Setting `resistance_floor_feature_enabled = 1.0` restores the previous behavior: with the dormant all-bacteria switch and 0.01 value, every organism receives a separate 1% attempt for each drug entry whose class has been introduced, subject to mechanism applicability and the current-cache causal check.

**Details for specific example bacteria**

Stenotrophomonas maltophilia. Intrinsic non-susceptibility to carbapenems, unprotected penicillins, 1st/2nd-generation cephalosporins, macrolides, and most aminoglycosides is encoded directly as near-zero potency values (`potency_when_no_r` ≤ 0.05). With the universal guard disabled, acquired resistance to TMP-SMX, fluoroquinolones, and tetracyclines is generated and maintained through the standard emergence, carriage, HGT, profile-transmission, ratchet, and explicit exogenous pathways where applicable.
...
The model includes a dedicated family of **environmental / exogenous mechanism floors** that apply only to the non-human fraction of community acquisition. These remain active and are distinct from the optional universal acquisition reseeding guard in Section 7.5, which is disabled by default.
...
**Diagnostic switch.** `run_pathway_ratchet_enabled` is 1.0 by default and preserves the ratchet behavior described above. Setting it to 0.0 disables only the dynamic ratchet contribution for component-ablation runs; static environmental floors remain active and the separate universal acquisition guard retains its independently configured state (currently disabled).
