...
**Implementation.** Once per simulated year, after that day's profile-cache update, the model pools retained community-profile entries across all regions and computes the marginal cache prevalence of each (bacteria, mechanism) pair. Hospital profiles are excluded because the ratchet acts only on exogenous community acquisitions. A bacterium must have at least 100 retained community profiles before that annual observation can increase its permanent peak; below this denominator the existing peak is left unchanged. Every retained community entry contributes equally to this statistic; it is not weighted to reproduce regional population sizes. The `peak_mechanism_prevalence[bacteria][mechanism]` table is updated upward-only from qualifying annual snapshots: once a sampled peak has been recorded, it is not allowed to fall.
