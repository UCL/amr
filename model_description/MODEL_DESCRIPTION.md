...
When an HGT event occurs, the mechanism must be classified as transferable and its executable host status must permit both the donor and recipient bacteria. A zero de novo emergence rate does not block receipt when the pair is `HgtOnly`; `ExcludedHost` does block both donation and receipt. The transferred mechanism is then recorded in each compartment where the recipient organism is present: `mechanism_any` for an active infection and `mechanism_microbiome` for asymptomatic carriage. If the recipient is present in both, both records are updated. A carriage-only transfer does not create phantom active-infection mechanism state. All HGT rates are scaled by the run-level `run_pathway_hgt_multiplier` (default 1.0) and by `counterfactual_resistance_multiplier`.
