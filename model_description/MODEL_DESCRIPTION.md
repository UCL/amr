...
Alongside the active library, the model keeps a permanent **local mechanism-persistence archive**. The first complete observed genotype carrying each mechanism is remembered separately for that region × care setting × bacterium. If a locally established mechanism later disappears from every active profile in that same slot, its archived genotype becomes latent. The archive records historical establishment; it is not included in active-cache prevalence and does not itself imply that the genotype is currently circulating. Section 7.5 describes the bounded route by which a latent local genotype can be sampled again.
...
When an infection or carriage acquisition samples the **local human reservoir**, the cache first checks whether that region × care setting × bacterium has any locally established mechanism that is absent from all of its active profiles. If so, all deduplicated latent archived genotypes together receive one profile-level reactivation draw. The default probability is `local_mechanism_persistence_reactivation_probability = 0.001`, equivalent to one virtual profile in a full 1000-profile reservoir. A successful draw selects one latent genotype and returns that complete profile in place of an active-cache profile. The draw is not repeated per mechanism or per drug, and archived mechanisms are never OR-ed independently onto another sampled genotype.

The archive is enabled by `local_mechanism_persistence_enabled = 1.0`. Establishment and reactivation are strictly local: an observation in one region or care setting does not seed another. A genotype remains latent only while at least one mechanism for which it is the archived establishment profile is absent from that slot's active cache. The archive does not alter the active prevalence calculation. It is sampled only on the human-reservoir branch, follows the same counterfactual gate as ordinary profile inheritance, and can contribute to either active infection or carriage. Successful incorporations are reported in the CSV field `local_persistence_profile_reactivations`.
...
| Local mechanism-persistence archive | Human-reservoir infection or carriage sampling when a locally established mechanism is absent from the active slot | Restores one complete archived genotype for that region × setting × bacterium | One bounded profile-level draw across all latent local genotypes; does not alter active-cache prevalence |
...
At finite simulated population sizes, an uncommon but genuinely established mechanism can disappear from every active infection and every retained profile in a local reservoir by chance. The model represents the much larger off-model population through a permanent local establishment archive rather than through the former universal per-drug acquisition guard. This is a rare profile-reintroduction pathway, not a hard lower bound on resistance prevalence.

| Parameter | Default | Function |
|-----------|--------:|----------|
| `local_mechanism_persistence_enabled` | 1.0 | Enables local historical establishment and reactivation |
| `local_mechanism_persistence_reactivation_probability` | 0.001 | One total profile-level draw when a sampled local slot has at least one latent archived genotype |

**Establishment and latency.** At each cache refresh, the first complete observed genotype carrying each mechanism is archived for that exact region × care setting × bacterium slot. The archive is permanent, but remains separate from the active profile reservoir. After the active cache has been refreshed, a mechanism is latent if it has an archived establishment profile but is absent from every active profile in that slot. Profiles associated with latent mechanisms are deduplicated, so one archived genotype carrying several missing mechanisms is still one candidate.

**Reactivation.** Whenever an infection or carriage acquisition is already taking the human-reservoir profile-sampling path, the model first checks that slot's precomputed latent candidates. All candidates together receive one Bernoulli draw at the configured probability. On success, one candidate is selected uniformly and its complete genotype is returned. The model does not repeat the draw for every drug or mechanism and does not combine archived bits with an independently sampled active genotype. At the default 0.001 probability, the archive has approximately the sampling weight of one virtual profile in a full 1000-profile reservoir.

**Scope and interpretation.** Establishment does not cross regions, bacteria, or community/hospital strata. The pathway is not used on the exogenous source branch. It follows the same resistance-counterfactual gate as ordinary profile inheritance and can contribute to both active infection and carriage. Archived profiles do not count toward active-cache prevalence, prescribing knowledge, or ratchet peak calculations. Once reactivated and present in an active infection, however, the genotype can re-enter the ordinary daily cache through the same majority-profile collection process as any other circulating strain.

**Diagnostics.** `local_persistence_profile_reactivations` counts complete archived profiles actually incorporated into new active infections or carriage during each output timestep. Draws rejected by the resistance counterfactual or active infections prevented by existing therapy are not counted.
