        ...
        for &drug in DRUG_SHORT_NAMES.iter() {
            for &bacteria in BACTERIA_LIST.iter() {
                ...
                // Note: Higher than pure mutation rates because represents combined selection + mutation + amplification
                 map.insert(format!("drug_{}_for_bacteria_{}_resistance_emergence_rate_per_day_baseline", drug, bacteria), 0.0);  // ***   0.00000000000000000000000000000001
            }
        }
    ...
    // pseudomonas_aeruginosa - Clinically apparent when burden high
        ...
        for &drug in DRUG_SHORT_NAMES.iter() {
            map.insert(
                ...
                0.00000001,  // 1e-8 - reduced from 0.2
            );
        }
        ...
        for &drug in acb_preserved_drugs.iter() {
            map.insert(
                ...
                0.000000001,  // 1e-9 - reserve agents even lower
            );
        }
        ...
        if BACTERIA_LIST.contains(&"bacteroides_fragilis") {
            for &drug in DRUG_SHORT_NAMES.iter() {
                map.insert(
                    ...
                    0.0000000001,  // 1e-10 - moderate base, controlled by mechanism multipliers
                );
            }
            ...
            for &drug in frontline_anaerobe_agents.iter() {
                map.insert(
                    ...
                    0.0000000001,  // 1e-10 - metronidazole resistance is rare, controlled by multipliers
                );
            }
            ...
            for &drug in inhibitor_protected_beta_lactams.iter() {
                map.insert(
                    ...
                    0.0000000001,  // 1e-10, controlled by multipliers
                );
            }
            ...
            for &drug in carbapenems.iter() {
                map.insert(
                    ...
                    0.00000000001,  // 1e-11 - carbapenem resistance is very rare, controlled by multipliers
                );
            }
        }

        // mycoplasma_genitalium - Tier 1 fix: reduce emergence (was showing 54pp error)
        // M. genitalium resistance is a real concern but should emerge slowly over years
        if BACTERIA_LIST.contains(&"mycoplasma_genitalium") {
            for &drug in DRUG_SHORT_NAMES.iter() {
                map.insert(
                    format!(
                    ...
                    0.00000000000001,  // 1e-14 - reduced from 1e-6
                );
            }
        ...
        for &drug in DRUG_SHORT_NAMES.iter() {
            map.insert(
                ...
                0.0000000000000000000000001,  // 1e-25 - effectively zero
            );
        }
        ...
        // enterococcus_faecium - Tier 2 fix: dramatically reduce emergence (was showing 40pp error)
        ...
        for &drug in DRUG_SHORT_NAMES.iter() {
            map.insert(
                ...
                0.00000000000000001,  // 1e-17 - dramatically reduced from 1000000
            );
        }
        ...
        // helicobacter_pylori - Tier 1 fix: dramatically reduce emergence (was showing 67pp error)
...
*/
        ...
        for &drug in DRUG_SHORT_NAMES.iter() {
            map.insert(format!("drug_{}_for_bacteria_neisseria_gonorrhoeae_resistance_emergence_rate_per_day_baseline", drug), 0.000000000001);  // 1e-12
        }
        ...
        for &drug in gonorrhea_3rd4th_gen_cephs.iter() {
            map.insert(format!("drug_{}_for_bacteria_neisseria_gonorrhoeae_resistance_emergence_rate_per_day_baseline", drug), 0.0000000000001);  // 1e-13 - ceftriaxone resistance very rare
        }
    ...
    map.insert("log_odds_microbiome_vs_infection".to_string(), 2.0); // Fallback: modest carriage boost if no bacteria-specific param
       ...
       // Stenotrophomonas maltophilia carries constitutive L1/L2 beta-lactamases and multi-drug efflux pumps
        ...
        map.insert(
            ...
            10000000000.0,
        ); // Plasmid-mediated quinolone protection is uncommon but possible
        map.insert(
            "bacteria_stenotrophomonas_maltophilia_mechanism_esbl_emergence_multiplier".to_string(),
            10000000000.0,
        ); // Occasional ESBL acquisition atop L2 beta-lactamase background
        map.insert(
            "bacteria_stenotrophomonas_maltophilia_mechanism_ampc_emergence_multiplier".to_string(),
            10000000000.0,
        ); // Adaptive L2 regulation drives broad-spectrum cephalosporin resistance
        map.insert(
            "bacteria_stenotrophomonas_maltophilia_mechanism_carbapenemase_emergence_multiplier"
                .to_string(),
            10000000000.0,
        ); // Metallo-beta-lactamase L1 variants are constitutive and expand under therapy
        map.insert(
            "bacteria_stenotrophomonas_maltophilia_mechanism_16s_methyltransferase_emergence_multiplier"
                .to_string(),
            10000000000.0,
        ); // ArmA/Rmt determinants occasionally co-localize with integrons in ICU isolates
        ...
        // Neisseria meningitidis rarely develops high-level resistance outside of sporadic cases
        map.insert(
        ...
        // Klebsiella pneumoniae remains highly resistant but not universally pan-resistant
        map.insert(
        ...
        // Moraxella catarrhalis increasingly expresses BRO beta-lactamases and macrolide efflux pumps
        map.insert(
        ...
        map.insert(
            "bacteria_campylobacter_jejuni_mechanism_erm_methylation_emergence_multiplier".to_string(),
            0.0000000000000001,
        );
...
*/
    ...
    map.insert("empiric_therapy_broad_spectrum_bonus".to_string(), 0.85); // Make broad-spectrum empiric choices noticeably less favored than narrow options
        ...
        map.insert("sepsis_death_base_log_odds".to_string(), -4.2); // Base log-odds (~1.5% daily mortality at baseline)
