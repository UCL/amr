...
lazy_static! {
    pub static ref PARAMETERS: HashMap<String, f64> = {
        ...
        map.insert("reserve_drug_score_penalty".to_string(), 0.2); // Reserve agents get 80% score haircut to keep total usage under stewardship targets
    ...
    // pseudomonas_aeruginosa - Clinically apparent when burden high
        ...
        // staphylococcus_aureus
        for &drug in DRUG_SHORT_NAMES.iter() {
            map.insert(format!("drug_{}_for_bacteria_staphylococcus_aureus_resistance_emergence_rate_per_day_baseline", drug), 0.03); 
        }
        ...
        // citrobacter_spp._
        for &drug in DRUG_SHORT_NAMES.iter() {
            map.insert(format!("drug_{}_for_bacteria_citrobacter_spp._resistance_emergence_rate_per_day_baseline", drug), 0.01); 
        }

        // clostridioides_difficile
        for &drug in DRUG_SHORT_NAMES.iter() {
            map.insert(format!("drug_{}_for_bacteria_clostridioides_difficile_resistance_emergence_rate_per_day_baseline", drug), 0.01); 
        }

        // enterobacter_cloacae
        for &drug in DRUG_SHORT_NAMES.iter() {
        map.insert(format!("drug_{}_for_bacteria_enterobacter_cloacae_resistance_emergence_rate_per_day_baseline", drug), 0.01); 
        }
        ...
        // enterococcus_faecalis
        for &drug in DRUG_SHORT_NAMES.iter() {
        map.insert(format!("drug_{}_for_bacteria_enterococcus_faecalis_resistance_emergence_rate_per_day_baseline", drug), 0.01); 
        }

        // enterococcus_faecium
        for &drug in DRUG_SHORT_NAMES.iter() {
        map.insert(format!("drug_{}_for_bacteria_enterococcus_faecium_resistance_emergence_rate_per_day_baseline", drug), 0.01); 
        }
        ...
        // listeria_monocytogenes
        for &drug in DRUG_SHORT_NAMES.iter() {
        map.insert(format!("drug_{}_for_bacteria_listeria_monocytogenes_resistance_emergence_rate_per_day_baseline", drug), 0.01); 
        }
        ...
        // serratia_spp.
        for &drug in DRUG_SHORT_NAMES.iter() {
        map.insert(format!("drug_{}_for_bacteria_serratia_spp._resistance_emergence_rate_per_day_baseline", drug), 0.02); 
        }
        ...
        // yersinia_enterocolitica
        for &drug in DRUG_SHORT_NAMES.iter() {
        map.insert(format!("drug_{}_for_bacteria_yersinia_enterocolitica_resistance_emergence_rate_per_day_baseline", drug), 0.02); 
        }

        // neisseria_meningitidis
        for &drug in DRUG_SHORT_NAMES.iter() {
        map.insert(format!("drug_{}_for_bacteria_neisseria_meningitidis_resistance_emergence_rate_per_day_baseline", drug), 0.01); 
        }
...
*/
    ...
    map.insert("log_odds_microbiome_vs_infection".to_string(), 2.0); // Fallback: modest carriage boost if no bacteria-specific param
        ...
        map.insert("resistance_emergence_pop_size_multiplier".to_string(), 1.0); // Debug knob to keep prevalence steady when population size changes - 0.5 million / pop size
