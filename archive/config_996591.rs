...
impl ClearanceParameters {
    fn from_map(map: &HashMap<String, f64>, num_bacteria: usize) -> Self {
        ...
        let base_daily_hazard = get_or_default(map, "default_clearance_hazard_after_delay", 0.020); // 0.045
...
*/
    ...
    map.insert("log_odds_microbiome_vs_infection".to_string(), 2.0); // Fallback: modest carriage boost if no bacteria-specific param
        ...
        map.insert("resistance_mechanism_erm_methylation_emergence_rate".to_string(), 0.0001); // Common in gram-positives
        map.insert("resistance_mechanism_esbl_emergence_rate".to_string(), 0.00002); // Requires specific gene mutations
        map.insert("resistance_mechanism_ampc_emergence_rate".to_string(), 0.0001); // Chromosomal or plasmid-mediated
        map.insert("resistance_mechanism_meca_emergence_rate".to_string(), 0.00002); // Requires SCCmec element acquisition
        map.insert("resistance_mechanism_carbapenemase_emergence_rate".to_string(), 0.00002); // Rare, high-level resistance genes
        map.insert("resistance_mechanism_van_type_emergence_rate".to_string(), 0.00002); // Complex vanA/vanB resistance cluster
        map.insert("resistance_mechanism_16s_methyltransferase_emergence_rate".to_string(), 0.0001); // Rare, high-level aminoglycoside resistance
...
*/
    ...
    map.insert("empiric_therapy_broad_spectrum_bonus".to_string(), 0.85); // Make broad-spectrum empiric choices noticeably less favored than narrow options
        ...
        map.insert("base_sepsis_death_risk_per_day".to_string(), 0.01); // Base 1% daily death risk for sepsis
        ...
        map.insert("sepsis_base_log_odds_of_recovery_per_day".to_string(), -1.0); // Base log odds (low baseline recovery probability ~12%)
