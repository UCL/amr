...
lazy_static! {
    ...
    // pseudomonas_aeruginosa - Clinically apparent when burden high
        ...
        // Corrections below target the largest class-level discrepancies (v531909).
        //
        //   Over-represented: Sulfonamides +16 pp, Tetracyclines +4.7 pp, Ceph 3G +4.3 pp,
        //                     Monobactams +2.9 pp, Anti-MRSA Ceph 5G +1.0 pp
        //   Under-represented: BLI combos -8.7 pp, Penicillins -3.5 pp, Macrolides -2.8 pp,
        //                      Nitrofurans -2.4 pp, Carbapenems -1.9 pp, Aminoglycosides -1.9 pp,
        //                      Glycopeptides -1.5 pp, Fosfomycin -1.0 pp, Nitroimidazoles -0.8 pp

        // --- 1. Sulfonamides: global penalty on both sulfanilamide + trim_sulf ---
        // sim 19.98% vs target 4.0% — sulfanilamide had no prior global penalty; trim_sulf
        // tightened from 0.2 → 0.08. Key organisms restored after global loop.
        for &bacteria in BACTERIA_LIST.iter() {
            map.insert(format!("drug_sulfanilamide_for_bacteria_{}_initiation_multiplier", bacteria), 0.05);
        }
        for &bacteria in BACTERIA_LIST.iter() {
            map.insert(format!("drug_trim_sulf_for_bacteria_{}_initiation_multiplier", bacteria), 0.08);
        }
        map.insert("drug_trim_sulf_for_bacteria_stenotrophomonas_maltophilia_initiation_multiplier".to_string(), 7.0); // First-line (restores earlier override)
        map.insert("drug_trim_sulf_for_bacteria_staphylococcus_epidermidis_initiation_multiplier".to_string(), 2.5);   // Oral step-down
        map.insert("drug_trim_sulf_for_bacteria_listeria_monocytogenes_initiation_multiplier".to_string(), 3.5);       // IV/oral alternative to ampicillin
        map.insert("drug_trim_sulf_for_bacteria_escherichia_coli_initiation_multiplier".to_string(), 1.2);             // UTI second-line
        map.insert("drug_trim_sulf_for_bacteria_klebsiella_pneumoniae_initiation_multiplier".to_string(), 0.5);        // UTI where susceptible
        map.insert("drug_trim_sulf_for_bacteria_proteus_spp._initiation_multiplier".to_string(), 0.5);                 // UTI where susceptible
        map.insert("drug_trim_sulf_for_bacteria_staphylococcus_aureus_initiation_multiplier".to_string(), 0.6);        // CA-MRSA oral step-down
        map.insert("drug_trim_sulf_for_bacteria_bordetella_pertussis_initiation_multiplier".to_string(), 0.3);         // Post-exposure / macrolide-allergy alternative

        // --- 2. Tetracyclines: global 0.4 penalty then restore key indications ---
        // sim 10.67% vs target 6.0% — broad default coverage drives overuse.
        for &bacteria in BACTERIA_LIST.iter() {
            map.insert(format!("drug_tetracycline_for_bacteria_{}_initiation_multiplier", bacteria), 0.4);
            map.insert(format!("drug_doxycycline_for_bacteria_{}_initiation_multiplier", bacteria), 0.4);
            map.insert(format!("drug_minocycline_for_bacteria_{}_initiation_multiplier", bacteria), 0.4);
        }
        ...
        // --- 3. Monobactams: tighten global penalty + reduce pen-allergy restores ---
        // sim 3.38% vs target 0.5% — global dropped from 0.05 → 0.02; Pseudomonas 0.4 → 0.12.
        for &bacteria in BACTERIA_LIST.iter() {
            map.insert(format!("drug_aztreonam_for_bacteria_{}_initiation_multiplier", bacteria), 0.02);
            map.insert(format!("drug_aztreonam_avibactam_for_bacteria_{}_initiation_multiplier", bacteria), 0.02);
        }
        map.insert("drug_aztreonam_for_bacteria_pseudomonas_aeruginosa_initiation_multiplier".to_string(), 0.12);      // Pen-allergy anti-pseudomonal
        map.insert("drug_aztreonam_for_bacteria_escherichia_coli_initiation_multiplier".to_string(), 0.06);
        map.insert("drug_aztreonam_for_bacteria_klebsiella_pneumoniae_initiation_multiplier".to_string(), 0.06);
        map.insert("drug_aztreonam_avibactam_for_bacteria_pseudomonas_aeruginosa_initiation_multiplier".to_string(), 0.12); // MBL/NDM strains
        map.insert("drug_aztreonam_avibactam_for_bacteria_acinetobacter_baumannii_initiation_multiplier".to_string(), 0.10);
        map.insert("drug_aztreonam_avibactam_for_bacteria_klebsiella_pneumoniae_initiation_multiplier".to_string(), 0.06);
        map.insert("drug_aztreonam_avibactam_for_bacteria_escherichia_coli_initiation_multiplier".to_string(), 0.06);

        // --- 4. Anti-MRSA Cephalosporins (5G): restrict ceftaroline to MRSA bacteraemia ---
        // sim 1.16% vs target 0.2% — no prior global penalty; set 0.02 and restore MRSA use.
        for &bacteria in BACTERIA_LIST.iter() {
            map.insert(format!("drug_ceftaroline_for_bacteria_{}_initiation_multiplier", bacteria), 0.02);
        }
        map.insert("drug_ceftaroline_for_bacteria_staphylococcus_aureus_initiation_multiplier".to_string(), 1.5);      // MRSA bloodstream / complicated SSTI
        map.insert("drug_ceftaroline_for_bacteria_streptococcus_pneumoniae_initiation_multiplier".to_string(), 0.3);   // MDR-CAP salvage
        map.insert("drug_ceftaroline_for_bacteria_staphylococcus_epidermidis_initiation_multiplier".to_string(), 0.5); // Device-associated CoNS

        // --- 5. BLI combos: further boost amox-clav and pip/tazo ---
        // sim 9.35% vs target 18.0% — raised from 4.0/3.0 to 7.0/5.0.
        for &bacteria in &[
            ...
            if BACTERIA_LIST.contains(&bacteria) {
                map.insert(format!("drug_amoxicillin_clavulanate_for_bacteria_{}_initiation_multiplier", bacteria), 7.0);
            }
        }
        for &bacteria in &[
            "citrobacter_spp.", "serratia_spp.", "morganella_spp.", "enterobacter_spp.",
            "enterobacter_cloacae", "streptococcus_pyogenes", "streptococcus_agalactiae",
        ] {
            if BACTERIA_LIST.contains(&bacteria) {
                map.insert(format!("drug_amoxicillin_clavulanate_for_bacteria_{}_initiation_multiplier", bacteria), 4.0);
            }
        }
        for &bacteria in &[
            ...
            if BACTERIA_LIST.contains(&bacteria) {
                map.insert(format!("drug_piperacillin_tazobactam_for_bacteria_{}_initiation_multiplier", bacteria), 5.0);
            }
        }

        // --- 6. Penicillins: increase amoxicillin for community first-line use ---
        // sim 14.52% vs target 18.0%
        for &bacteria in &[
            "streptococcus_pneumoniae", "streptococcus_pyogenes", "streptococcus_agalactiae",
            ...
            if BACTERIA_LIST.contains(&bacteria) {
                map.insert(format!("drug_amoxicillin_for_bacteria_{}_initiation_multiplier", bacteria), 4.0);
            }
        }

        // --- 7. Macrolides: increase respiratory/atypical boosts ---
        // sim 8.24% vs target 11.0%
        map.insert("drug_azithromycin_for_bacteria_streptococcus_pneumoniae_initiation_multiplier".to_string(), 5.0);
        map.insert("drug_clarithromycin_for_bacteria_streptococcus_pneumoniae_initiation_multiplier".to_string(), 5.0);
        map.insert("drug_erythromycin_for_bacteria_streptococcus_pneumoniae_initiation_multiplier".to_string(), 4.0);
        map.insert("drug_azithromycin_for_bacteria_moraxella_catarrhalis_initiation_multiplier".to_string(), 4.0);
        map.insert("drug_clarithromycin_for_bacteria_moraxella_catarrhalis_initiation_multiplier".to_string(), 4.0);
        map.insert("drug_azithromycin_for_bacteria_mycoplasma_pneumoniae_initiation_multiplier".to_string(), 7.0);
        map.insert("drug_clarithromycin_for_bacteria_mycoplasma_pneumoniae_initiation_multiplier".to_string(), 6.0);
        map.insert("drug_azithromycin_for_bacteria_bordetella_pertussis_initiation_multiplier".to_string(), 7.0);
        map.insert("drug_clarithromycin_for_bacteria_bordetella_pertussis_initiation_multiplier".to_string(), 6.0);
        map.insert("drug_erythromycin_for_bacteria_bordetella_pertussis_initiation_multiplier".to_string(), 6.0);
        map.insert("drug_azithromycin_for_bacteria_streptococcus_pyogenes_initiation_multiplier".to_string(), 4.0);
        map.insert("drug_clarithromycin_for_bacteria_streptococcus_pyogenes_initiation_multiplier".to_string(), 3.5);
        map.insert("drug_azithromycin_for_bacteria_haemophilus_influenzae_initiation_multiplier".to_string(), 5.5);   // Restores + boosts
        map.insert("drug_clarithromycin_for_bacteria_haemophilus_influenzae_initiation_multiplier".to_string(), 5.5);
        map.insert("drug_erythromycin_for_bacteria_haemophilus_influenzae_initiation_multiplier".to_string(), 5.0);

        // --- 8. Nitrofurantoin: aggressive UTI boost ---
        // sim 0.58% vs target 3.0% — raised from 8/5 to 20/12
        map.insert("drug_nitrofurantoin_for_bacteria_escherichia_coli_initiation_multiplier".to_string(), 20.0);       // First-line uncomplicated UTI
        map.insert("drug_nitrofurantoin_for_bacteria_klebsiella_pneumoniae_initiation_multiplier".to_string(), 12.0);  // Second-line UTI
        map.insert("drug_nitrofurantoin_for_bacteria_enterococcus_faecalis_initiation_multiplier".to_string(), 10.0);  // UTI (susceptible; common)
        map.insert("drug_nitrofurantoin_for_bacteria_enterococcus_faecium_initiation_multiplier".to_string(), 8.0);    // UTI
        map.insert("drug_nitrofurantoin_for_bacteria_staphylococcus_aureus_initiation_multiplier".to_string(), 6.0);   // UTI / catheter
        map.insert("drug_nitrofurantoin_for_bacteria_staphylococcus_epidermidis_initiation_multiplier".to_string(), 5.0);
        ...
        map.insert("drug_nitrofurantoin_for_bacteria_proteus_spp._initiation_multiplier".to_string(), 0.05);           // Largely resistant

        // --- 9. Carbapenems: raise per-organism overrides to 1.0-1.5 ---
        // sim 0.08% vs target 2.0% — raised from 0.25-0.30 to 1.0-1.5
        map.insert("drug_meropenem_for_bacteria_escherichia_coli_initiation_multiplier".to_string(), 1.5);
        map.insert("drug_meropenem_for_bacteria_klebsiella_pneumoniae_initiation_multiplier".to_string(), 1.5);
        map.insert("drug_meropenem_for_bacteria_pseudomonas_aeruginosa_initiation_multiplier".to_string(), 1.0);
        map.insert("drug_meropenem_for_bacteria_acinetobacter_baumannii_initiation_multiplier".to_string(), 1.2);
        map.insert("drug_meropenem_for_bacteria_enterobacter_spp._initiation_multiplier".to_string(), 1.0);
        map.insert("drug_meropenem_for_bacteria_enterobacter_cloacae_initiation_multiplier".to_string(), 1.0);
        map.insert("drug_meropenem_for_bacteria_citrobacter_spp._initiation_multiplier".to_string(), 0.8);
        map.insert("drug_meropenem_for_bacteria_serratia_spp._initiation_multiplier".to_string(), 0.8);
        map.insert("drug_imipenem_c_for_bacteria_escherichia_coli_initiation_multiplier".to_string(), 0.8);
        map.insert("drug_imipenem_c_for_bacteria_klebsiella_pneumoniae_initiation_multiplier".to_string(), 0.8);
        map.insert("drug_imipenem_c_for_bacteria_pseudomonas_aeruginosa_initiation_multiplier".to_string(), 0.6);
        map.insert("drug_imipenem_c_for_bacteria_acinetobacter_baumannii_initiation_multiplier".to_string(), 0.9);
        map.insert("drug_ertapenem_for_bacteria_escherichia_coli_initiation_multiplier".to_string(), 0.8);
        map.insert("drug_ertapenem_for_bacteria_klebsiella_pneumoniae_initiation_multiplier".to_string(), 0.8);
        map.insert("drug_ertapenem_for_bacteria_enterobacter_spp._initiation_multiplier".to_string(), 0.5);

        // --- 10. Aminoglycosides: aggressive boost for combination therapy ---
        // sim 0.10% vs target 2.0% — raised from 2-3 to 8-12
        map.insert("drug_gentamicin_for_bacteria_escherichia_coli_initiation_multiplier".to_string(), 10.0);
        map.insert("drug_gentamicin_for_bacteria_klebsiella_pneumoniae_initiation_multiplier".to_string(), 8.0);
        map.insert("drug_gentamicin_for_bacteria_staphylococcus_aureus_initiation_multiplier".to_string(), 8.0);       // Endocarditis synergy
        map.insert("drug_gentamicin_for_bacteria_enterococcus_faecalis_initiation_multiplier".to_string(), 12.0);      // High-level synergy endocarditis
        map.insert("drug_gentamicin_for_bacteria_pseudomonas_aeruginosa_initiation_multiplier".to_string(), 6.0);
        map.insert("drug_amikacin_for_bacteria_pseudomonas_aeruginosa_initiation_multiplier".to_string(), 10.0);
        map.insert("drug_amikacin_for_bacteria_acinetobacter_baumannii_initiation_multiplier".to_string(), 10.0);      // MDR combination
        map.insert("drug_amikacin_for_bacteria_klebsiella_pneumoniae_initiation_multiplier".to_string(), 8.0);
        map.insert("drug_tobramycin_for_bacteria_pseudomonas_aeruginosa_initiation_multiplier".to_string(), 10.0);
        map.insert("drug_tobramycin_for_bacteria_acinetobacter_baumannii_initiation_multiplier".to_string(), 6.0);

        // --- 11. Glycopeptides & oxazolidinones: additional boosts ---
        // sim 0.46% vs target 2.0% — teicoplanin for Enterococcus; vancomycin for Strep coverage
        map.insert("drug_teicoplanin_for_bacteria_enterococcus_faecalis_initiation_multiplier".to_string(), 3.5);
        map.insert("drug_teicoplanin_for_bacteria_enterococcus_faecium_initiation_multiplier".to_string(), 3.0);
        map.insert("drug_vancomycin_for_bacteria_streptococcus_pneumoniae_initiation_multiplier".to_string(), 2.0);    // Pen-resistant CAP
        map.insert("drug_vancomycin_for_bacteria_streptococcus_pyogenes_initiation_multiplier".to_string(), 1.5);      // Beta-lac allergy
        map.insert("drug_vancomycin_for_bacteria_streptococcus_agalactiae_initiation_multiplier".to_string(), 1.5);
        map.insert("drug_linezolid_for_bacteria_staphylococcus_aureus_initiation_multiplier".to_string(), 5.0);        // MRSA escalation
        map.insert("drug_linezolid_for_bacteria_enterococcus_faecium_initiation_multiplier".to_string(), 1.0);         // VRE (reserve)

        // --- 12. Fosfomycin: boost for UTI organisms ---
        // sim 0.0% vs target 1.0% — fosfomycin trometamol is first-line uncomplicated UTI (EMA/IDSA)
        map.insert("drug_fosfomycin_for_bacteria_escherichia_coli_initiation_multiplier".to_string(), 8.0);
        map.insert("drug_fosfomycin_for_bacteria_enterococcus_faecalis_initiation_multiplier".to_string(), 6.0);
        map.insert("drug_fosfomycin_for_bacteria_klebsiella_pneumoniae_initiation_multiplier".to_string(), 4.0);
        map.insert("drug_fosfomycin_for_bacteria_staphylococcus_aureus_initiation_multiplier".to_string(), 3.0);
        map.insert("drug_fosfomycin_for_bacteria_staphylococcus_epidermidis_initiation_multiplier".to_string(), 3.0);
        map.insert("drug_fosfomycin_for_bacteria_pseudomonas_aeruginosa_initiation_multiplier".to_string(), 0.05);     // Unreliable activity

        // --- 13. Nitroimidazoles: boost anaerobic / H. pylori coverage ---
        // sim 0.17% vs target 1.0% — C. difficile already at 6.0; add B. fragilis + H. pylori
        map.insert("drug_metronidazole_for_bacteria_bacteroides_fragilis_initiation_multiplier".to_string(), 10.0);    // First-line anaerobic coverage
        map.insert("drug_metronidazole_for_bacteria_helicobacter_pylori_initiation_multiplier".to_string(), 8.0);      // Triple/quadruple therapy component
        map.insert("drug_furazolidone_for_bacteria_helicobacter_pylori_initiation_multiplier".to_string(), 4.0);       // Quadruple therapy fallback
    ...
    map.insert("log_odds_microbiome_vs_infection".to_string(), 2.0); // Fallback: modest carriage boost if no bacteria-specific param
        ...
        map.insert("bacteria_klebsiella_pneumoniae_mechanism_mutation_folate_pathway_emergence_rate".to_string(), 0.000_3   ); // classes: sulf
        ...
        map.insert("bacteria_citrobacter_spp._mechanism_enzyme_16s_rrmt_emergence_rate".to_string(), 0.3   ); // classes: ag
        ...
        map.insert("bacteria_citrobacter_spp._mechanism_mutation_folate_pathway_emergence_rate".to_string(), 0.002   ); // classes: sulf
        ...
        map.insert("bacteria_citrobacter_spp._mechanism_enzyme_aac_aph_emergence_rate".to_string(), 0.3   ); // classes: ag
  ...
  // Enterobacter spp. — Gram-negative, Enterobacterales
       ...
       // Morganella spp. — Gram-negative, Enterobacterales
        ...
        map.insert("bacteria_serratia_spp._mechanism_mutation_gyra_primary_emergence_rate".to_string(), 0.04    ); // classes: fq
        ...
        map.insert("bacteria_serratia_spp._mechanism_global_efflux_pump_emergence_rate".to_string(), 0.02    ); // classes: fq, mls, tet, chl
       ...
       // A. baumannii — Gram-negative, NonFermenter
        ...
        map.insert("bacteria_helicobacter_pylori_mechanism_target_site_cfr_emergence_rate".to_string(), 0.3  ); // classes: oxa, mls, chl
        ...
        map.insert("bacteria_helicobacter_pylori_mechanism_global_efflux_pump_emergence_rate".to_string(), 0.3  ); // classes: fq, mls, tet, chl
        ...
        map.insert("bacteria_helicobacter_pylori_mechanism_global_porin_loss_emergence_rate".to_string(), 0.3  ); // classes: none (currently zeroed)
        map.insert("bacteria_helicobacter_pylori_mechanism_modification_mcr_1_emergence_rate".to_string(), 0.0); // tier 0
        map.insert("bacteria_helicobacter_pylori_mechanism_mutation_folate_pathway_emergence_rate".to_string(), 0.3     ); // classes: sulf
        map.insert("bacteria_helicobacter_pylori_mechanism_mutation_nitroreductase_emergence_rate".to_string(), 0.3 ); // classes: other (metronidazole, nitrofurantoin, furazolidone)
        ...
        map.insert("bacteria_helicobacter_pylori_mechanism_mutation_rpo_b_emergence_rate".to_string(), 0.3); // classes: other (rifampicin, fidaxomicin)
        ...
        map.insert("bacteria_helicobacter_pylori_mechanism_enzyme_bla_z_emergence_rate".to_string(), 0.3 ); // classes: pen; PBP1A proxy via blaZ; targets amox/amp only
        map.insert("bacteria_helicobacter_pylori_mechanism_enzyme_oxa_acinetobacter_emergence_rate".to_string(), 0.0); // tier 0
        map.insert("bacteria_helicobacter_pylori_mechanism_mutation_23s_rrna_emergence_rate".to_string(), 0.3 ); // classes: mls; primary clarithromycin R mechanism
        map.insert("bacteria_helicobacter_pylori_mechanism_efflux_tet_abc_emergence_rate".to_string(), 0.0); // tier 0
        map.insert("bacteria_helicobacter_pylori_mechanism_mutation_pbp_mosaic_emergence_rate".to_string(), 0.3 ); // classes: pen, bli, ceph, mono; low PBP1A-type amoxicillin resistance proxy
        ...
        map.insert("bacteria_staphylococcus_aureus_mechanism_target_site_pbp2a_meca_emergence_rate".to_string(), 0.001   ); // classes: pen, bli, ceph, carb; driving penicillins ~35-60%
        map.insert("bacteria_staphylococcus_aureus_mechanism_target_site_van_a_emergence_rate".to_string(), 0.000_1      ); // classes: glyc; targets ~10% for vanco/teico
        map.insert("bacteria_staphylococcus_aureus_mechanism_target_site_van_b_emergence_rate".to_string(), 0.000_01           ); // classes: glyc
        map.insert("bacteria_staphylococcus_aureus_mechanism_target_site_erm_b_emergence_rate".to_string(), 0.01        ); // classes: mls; macrolides ~40%
        map.insert("bacteria_staphylococcus_aureus_mechanism_target_site_cfr_emergence_rate".to_string(), 0.002        ); // classes: oxa, mls, chl; Linezolid 1%
        ...
        map.insert("bacteria_staphylococcus_aureus_mechanism_mutation_nitroreductase_emergence_rate".to_string(), 0.03         ); // classes: other (metronidazole, nitrofurantoin, furazolidone)
        map.insert("bacteria_staphylococcus_aureus_mechanism_enzyme_fos_a_emergence_rate".to_string(), 0.03        ); // classes: other (fosfomycin)
        map.insert("bacteria_staphylococcus_aureus_mechanism_mutation_mpr_f_emergence_rate".to_string(), 0.03         ); // classes: other (daptomycin)
        ...
        map.insert("bacteria_staphylococcus_aureus_mechanism_enzyme_bla_z_emergence_rate".to_string(), 0.001   ); // classes: pen; blaZ
        ...
        map.insert("bacteria_enterococcus_faecium_mechanism_target_site_van_a_emergence_rate".to_string(),   0.008   ); // classes: glyc
        map.insert("bacteria_enterococcus_faecium_mechanism_target_site_van_b_emergence_rate".to_string(),   0.008     ); // classes: glyc
        ...
        map.insert("bacteria_clostridioides_difficile_mechanism_as_yet_unknown_emergence_rate".to_string(), 0.0); // classes: broad placeholder; 
         ...
         // B. fragilis — Anaerobe
        // Band 8 (x37.5)
        map.insert("bacteria_bacteroides_fragilis_mechanism_enzyme_esbl_ctx_m_emergence_rate".to_string(), 0.003  ); // classes: pen, bli, ceph, mono
        map.insert("bacteria_bacteroides_fragilis_mechanism_enzyme_esbl_tem_emergence_rate".to_string(), 0.003   ); // classes: pen, bli, ceph, mono
        map.insert("bacteria_bacteroides_fragilis_mechanism_enzyme_esbl_shv_emergence_rate".to_string(), 0.003     ); // classes: pen, bli, ceph, mono
        ...
        map.insert("bacteria_bacteroides_fragilis_mechanism_enzyme_kpc_emergence_rate".to_string(), 0.000_03  ); // classes: pen, bli, ceph, carb, mono
        map.insert("bacteria_bacteroides_fragilis_mechanism_enzyme_ndm_vim_emergence_rate".to_string(), 0.000_03 ); // classes: pen, bli, ceph, carb, mono
        map.insert("bacteria_bacteroides_fragilis_mechanism_enzyme_oxa_48_emergence_rate".to_string(), 0.000_03  ); // classes: pen, bli, ceph, carb
